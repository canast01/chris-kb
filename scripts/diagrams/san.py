...
@kb_diagram(
    'brocade-fabric-os-arch-index',
    'docs/san/brocade/fabric-os/architecture/index.md',
    'Brocade Fabric OS Architecture — dual-fabric, principal switch, zoning, MAPS',
)
def brocade_fabric_os_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Brocade Fabric OS — FC SAN Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Dual-fabric core-edge topology; principal switch election via highest domain ID;'))
    lines.append(txt_row('WWN-based zoning, ISL trunks, Virtual Fabrics (FID), and MAPS monitoring.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Fabric Architecture'), bMid(L2, R2, 'Zoning and Name Service'))))
    lines.append(R(merge(bMid(L1, R1, 'Dual-fabric: A + B independent'), bMid(L2, R2, 'WWN zoning: recommended'))))
    lines.append(R(merge(bMid(L1, R1, 'Core: director-class switches'), bMid(L2, R2, 'Port zoning: for debugging'))))
    lines.append(R(merge(bMid(L1, R1, 'Edge: fixed-port switches'), bMid(L2, R2, 'Zone set: active + saved'))))
    lines.append(R(merge(bMid(L1, R1, 'Principal: elects domain IDs'), bMid(L2, R2, 'DNS: distributed name server'))))
    lines.append(R(merge(bMid(L1, R1, 'ISL: inter-switch link (trunk)'), bMid(L2, R2, 'FDMI: HBA attribute register'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Zoning enforced at login: initiator can only see targets in the same zone.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Virtual Fabrics (FID)'), bMid(L2, R2, 'MAPS Health Monitoring'))))
    lines.append(R(merge(bMid(L1, R1, 'Partition one switch: 2+ FIDs'), bMid(L2, R2, 'Rules: port, fabric, perf'))))
    lines.append(R(merge(bMid(L1, R1, 'Each FID: own domain ID + zone'), bMid(L2, R2, 'Policies: default + custom'))))
    lines.append(R(merge(bMid(L1, R1, 'Base fabric: management only'), bMid(L2, R2, 'Actions: email/SNMP/fence'))))
    lines.append(R(merge(bMid(L1, R1, 'Chassis context: cross-FID view'), bMid(L2, R2, 'Dashboard: mapspoliccopy'))))
    lines.append(R(merge(bMid(L1, R1, 'Requires Advanced License'), bMid(L2, R2, 'CRC errors: key SAN metric'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Brocade G720 / G730 directors (bladed) or X7 fixed switches; dual power;'))
    lines.append(txt_row('FC SFP+ optics (32G/64G); physical A/B fabric separation via separate cables.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('FOS           = Fabric OS; Brocade/Broadcom FC switch operating system'))
    lines.append(txt_row('Principal switch= elects domain IDs for the fabric; highest WWN or priority'))
    lines.append(txt_row('Domain ID     = 1-byte fabric-unique switch identifier; 1-239 range'))
    lines.append(txt_row('WWN           = World Wide Name; 64-bit unique ID for HBAs and switch ports'))
    lines.append(txt_row('Zone          = access control group; one initiator + one or more targets'))
    lines.append(txt_row('Zone set      = collection of zones activated on the fabric'))
    lines.append(txt_row('ISL           = Inter-Switch Link; FC connection between switches'))
    lines.append(txt_row('Trunk         = ISL trunk group; multiple ISLs load-balanced as one'))
    lines.append(txt_row('FID           = Fabric ID; Virtual Fabric partition identifier'))
    lines.append(txt_row('MAPS          = Monitoring and Alerting Policy Suite; proactive health checks'))
    lines.append(txt_row('DNS           = Distributed Name Server; tracks WWNs logged in to fabric'))
    lines.append(txt_row('FDMI          = Fabric Device Management Interface; HBA attributes'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'brocade-sannav-arch-index',
    'docs/san/brocade/sannav/architecture/index.md',
    'SANnav Architecture — Management Portal vs Global View, Linux appliance, HTTPS/SNMP',
)
def brocade_sannav_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'SANnav — SAN Management Platform Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Two variants: Management Portal (per-fabric ops) and Global View (multi-portal);'))
    lines.append(txt_row('Linux virtual appliances; communicates with switches via HTTPS and SNMP v3.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Deployment Variants'), bMid(L2, R2, 'Platform Architecture'))))
    lines.append(R(merge(bMid(L1, R1, 'Management Portal: per-fabric'), bMid(L2, R2, 'Linux VM (RHEL/CentOS base)'))))
    lines.append(R(merge(bMid(L1, R1, 'Global View: multi-portal UI'), bMid(L2, R2, 'PostgreSQL: config + event DB'))))
    lines.append(R(merge(bMid(L1, R1, 'Portal: one per fabric pair'), bMid(L2, R2, 'Elasticsearch: search + logs'))))
    lines.append(R(merge(bMid(L1, R1, 'GV: aggregates all portals'), bMid(L2, R2, 'REST API v3: northbound'))))
    lines.append(R(merge(bMid(L1, R1, 'DR: passive standby supported'), bMid(L2, R2, 'HTTPS 443: switch comms'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Management Portal manages switches directly; Global View aggregates read-only view.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Switch Integration'), bMid(L2, R2, 'Monitoring and Alerts'))))
    lines.append(R(merge(bMid(L1, R1, 'HTTPS: REST/supportsave'), bMid(L2, R2, 'MAPS: rules + policies'))))
    lines.append(R(merge(bMid(L1, R1, 'SNMP v3: trap receiver'), bMid(L2, R2, 'Dashboard: port health'))))
    lines.append(R(merge(bMid(L1, R1, 'Syslog: optional from switch'), bMid(L2, R2, 'CRC/signal alerts: fabric'))))
    lines.append(R(merge(bMid(L1, R1, 'SSH: supportsave collect'), bMid(L2, R2, 'Email/SNMP: notification'))))
    lines.append(R(merge(bMid(L1, R1, 'Certificate: switch TLS verify'), bMid(L2, R2, 'Inventory: live + historical'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('VMware or KVM VM (8 vCPU, 32 GB RAM, 500 GB disk minimum); management network'))
    lines.append(txt_row('access to all FC switches on TCP 443 and UDP 162 (SNMP).'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Management Portal= per-fabric SANnav instance; full fabric management'))
    lines.append(txt_row('Global View   = multi-portal aggregator; read-only cross-fabric dashboard'))
    lines.append(txt_row('SNMP v3       = secure traps from switches; authPriv encryption'))
    lines.append(txt_row('Elasticsearch = full-text search engine; powers SANnav log search'))
    lines.append(txt_row('PostgreSQL    = relational DB; config, inventory, and event storage'))
    lines.append(txt_row('MAPS          = Monitoring and Alerting Policy Suite; pushed from switch'))
    lines.append(txt_row('Supportsave   = FOS support bundle; SANnav can trigger and collect'))
    lines.append(txt_row('DR standby    = passive SANnav VM; promoted if active fails'))
    lines.append(txt_row('REST API v3   = SANnav northbound API; fabric config and monitoring'))
    lines.append(txt_row('CRC error     = Cyclic Redundancy Check error; key FC signal health metric'))
    lines.append(txt_row('Port health   = per-port state including CRC, signal, congestion'))
    lines.append(txt_row('Inventory     = real-time switch + host + storage discovered topology'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'cisco-dcnm-arch-index',
    'docs/san/cisco/cisco-dcnm/architecture/index.md',
    'Cisco DCNM Architecture — SAN management appliance, superseded by NDFC on Nexus Dashboard',
)
def cisco_dcnm_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Cisco DCNM Architecture — Last Standalone SAN Manager'))
    lines.append(txt_row())
    lines.append(txt_row('DCNM 11.x is the last standalone SAN management appliance; superseded by NDFC'))
    lines.append(txt_row('(Nexus Dashboard Fabric Controller) on Nexus Dashboard 12.0+.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'DCNM 11.x Architecture'), bMid(L2, R2, 'Managed Domains'))))
    lines.append(R(merge(bMid(L1, R1, 'Standalone Linux appliance (OVA)'), bMid(L2, R2, 'SAN: Cisco MDS fabrics'))))
    lines.append(R(merge(bMid(L1, R1, 'PostgreSQL: config + topology'), bMid(L2, R2, 'LAN: Nexus switching fabric'))))
    lines.append(R(merge(bMid(L1, R1, 'Elasticsearch: log search'), bMid(L2, R2, 'Media: IP LAN for video'))))
    lines.append(R(merge(bMid(L1, R1, 'REST/SSH: switch discovery'), bMid(L2, R2, 'DCNM licences per device'))))
    lines.append(R(merge(bMid(L1, R1, 'SNMP: event ingestion'), bMid(L2, R2, 'Web UI: central management'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('DCNM 11.x end-of-support: migrate to Nexus Dashboard + NDFC for ongoing support.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Migration Path to NDFC'), bMid(L2, R2, 'DCNM vs NDFC Differences'))))
    lines.append(R(merge(bMid(L1, R1, 'Deploy Nexus Dashboard 3-node'), bMid(L2, R2, 'NDFC: app on ND platform'))))
    lines.append(R(merge(bMid(L1, R1, 'Install NDFC on ND cluster'), bMid(L2, R2, 'NDFC: multi-tenant ND account'))))
    lines.append(R(merge(bMid(L1, R1, 'Re-discover switches in NDFC'), bMid(L2, R2, 'NDFC: REST API v1 compatible'))))
    lines.append(R(merge(bMid(L1, R1, 'Export: templates + configs'), bMid(L2, R2, 'ND: shared identity + scale'))))
    lines.append(R(merge(bMid(L1, R1, 'Cutover: update SNMP targets'), bMid(L2, R2, 'DCNM: deprecated as of 12.0'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('DCNM: Linux OVA VM (16 vCPU, 64 GB RAM, 500 GB disk); management network'))
    lines.append(txt_row('access to all Cisco MDS and Nexus switches on TCP 22 (SSH) and 443.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('DCNM          = Data Center Network Manager; Cisco SAN/LAN management v11'))
    lines.append(txt_row('NDFC          = Nexus Dashboard Fabric Controller; replacement for DCNM'))
    lines.append(txt_row('Nexus Dashboard= app-hosting platform; runs NDFC, NDI, NDO as apps'))
    lines.append(txt_row('OVA           = Open Virtualization Appliance; VMware import format'))
    lines.append(txt_row('EOS           = End of Support; DCNM 11.x reached EOS; migrate to NDFC'))
    lines.append(txt_row('SAN Insights  = DCNM feature; IO latency analytics per initiator-target pair'))
    lines.append(txt_row('LAN discovery = DCNM autodiscovery via CDP/LLDP from seed switch'))
    lines.append(txt_row('SNMP          = trap receiver in DCNM for switch alerts'))
    lines.append(txt_row('Template      = DCNM config template; import/export to NDFC'))
    lines.append(txt_row('vRF           = virtual routing and forwarding; DCNM overlay construct'))
    lines.append(txt_row('ND account    = identity in Nexus Dashboard; DCNM → NDFC user migration'))
    lines.append(txt_row('Seed switch   = first switch discovered; DCNM expands via CDP'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'cisco-mds-arch-index',
    'docs/san/cisco/mds/architecture/index.md',
    'Cisco MDS Architecture — NX-OS, VSAN, domain IDs, ISSU, FC switching',
)
def cisco_mds_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Cisco MDS Architecture — NX-OS FC Switching'))
    lines.append(txt_row())
    lines.append(txt_row('MDS 9000 NX-OS FC switches; VSANs create logical fabrics on shared hardware;'))
    lines.append(txt_row('each VSAN has own name server, zone DB, and domain ID; ISSU for zero-downtime.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Platform Types'), bMid(L2, R2, 'VSAN Architecture'))))
    lines.append(R(merge(bMid(L1, R1, 'Fixed: 9132T/9148T/9396T'), bMid(L2, R2, 'VSAN: logical fabric on 1 switch'))))
    lines.append(R(merge(bMid(L1, R1, 'Director: 9706/9710/9718'), bMid(L2, R2, 'Separate NS/zone DB per VSAN'))))
    lines.append(R(merge(bMid(L1, R1, 'ISSU: zero-downtime upgrade'), bMid(L2, R2, 'Port VSAN assignment: per port'))))
    lines.append(R(merge(bMid(L1, R1, 'Dual supervisor (directors)'), bMid(L2, R2, 'Domain ID: per VSAN unique'))))
    lines.append(R(merge(bMid(L1, R1, 'Linecard: modular port density'), bMid(L2, R2, 'VSAN 1: default; avoid in prod'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('VSAN isolation: devices in different VSANs cannot communicate directly.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'FC Services'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'FCNS: fabric name server'), bMid(L2, R2, 'DCNM/NDFC: central mgmt'))))
    lines.append(R(merge(bMid(L1, R1, 'FSPF: fabric shortest path'), bMid(L2, R2, 'CLI: NX-OS SSH access'))))
    lines.append(R(merge(bMid(L1, R1, 'FLOGI: fabric login process'), bMid(L2, R2, 'SNMP: v3 for monitoring'))))
    lines.append(R(merge(bMid(L1, R1, 'FDISC: N-port virtualisation'), bMid(L2, R2, 'NTP: clock sync required'))))
    lines.append(R(merge(bMid(L1, R1, 'TE port: trunked E port ISL'), bMid(L2, R2, 'AAA: TACACS+ or RADIUS'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('FC switches in dedicated SAN racks; A and B fabric physical separation;'))
    lines.append(txt_row('32G/64G SFP+ optics; separate out-of-band management on mgmt0 port.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('NX-OS         = Cisco network OS for MDS (and Nexus) switches'))
    lines.append(txt_row('VSAN          = Virtual SAN; logical fabric partition; own name server'))
    lines.append(txt_row('Domain ID     = 1-byte fabric-unique switch ID per VSAN'))
    lines.append(txt_row('FCNS          = Fibre Channel Name Server; tracks WWN logins per VSAN'))
    lines.append(txt_row('FSPF          = Fabric Shortest Path First; FC routing protocol'))
    lines.append(txt_row('ISSU          = In-Service Software Upgrade; zero-downtime NX-OS upgrade'))
    lines.append(txt_row('TE port       = Trunked E port; ISL carrying multiple VSANs'))
    lines.append(txt_row('FLOGI         = Fabric Login; HBA logs in to get FC address (FCID)'))
    lines.append(txt_row('FCID          = Fabric-assigned 3-byte address; domain.area.port'))
    lines.append(txt_row('Zoning        = access control; same concept as Brocade; per VSAN'))
    lines.append(txt_row('DCNM/NDFC     = Cisco fabric manager; discover, zone, monitor MDS'))
    lines.append(txt_row('Director      = high-end chassis MDS; dual supervisors; ISSU capable'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'cisco-nexus-dashboard-arch-index',
    'docs/san/cisco/nexus-dashboard/architecture/index.md',
    'Nexus Dashboard Architecture — 3/5-node cluster, NDFC/NDI/NDO apps, API gateway',
)
def cisco_nexus_dashboard_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Cisco Nexus Dashboard — App-Hosting Platform'))
    lines.append(txt_row())
    lines.append(txt_row('3-node or 5-node cluster with shared identity, multi-site connectivity, and API'))
    lines.append(txt_row('gateway; NDFC, NDI, and NDO run as hosted applications on the cluster.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Cluster Architecture'), bMid(L2, R2, 'Hosted Applications'))))
    lines.append(R(merge(bMid(L1, R1, '3-node: HA quorum'), bMid(L2, R2, 'NDFC: SAN+LAN fabric control'))))
    lines.append(R(merge(bMid(L1, R1, '5-node: scale + apps separation'), bMid(L2, R2, 'NDI: network insights'))))
    lines.append(R(merge(bMid(L1, R1, 'Kubernetes: shared app runtime'), bMid(L2, R2, 'NDO: multi-site orchestrator'))))
    lines.append(R(merge(bMid(L1, R1, 'Etcd: cluster state store'), bMid(L2, R2, 'Apps: install from app store'))))
    lines.append(R(merge(bMid(L1, R1, 'Shared: identity + secrets'), bMid(L2, R2, 'License: per-app subscription'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Each app (NDFC/NDI/NDO) is independent; all share ND auth and multi-site links.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Network Connectivity'), bMid(L2, R2, 'Multi-Site and API'))))
    lines.append(R(merge(bMid(L1, R1, 'Data network: switch discovery'), bMid(L2, R2, 'Multi-site: inter-ND link'))))
    lines.append(R(merge(bMid(L1, R1, 'Mgmt network: OOB access'), bMid(L2, R2, 'REST API: per-app + platform'))))
    lines.append(R(merge(bMid(L1, R1, 'SSH/HTTPS to Nexus/MDS'), bMid(L2, R2, 'SAML/LDAP: SSO integration'))))
    lines.append(R(merge(bMid(L1, R1, 'SNMP: trap receiving'), bMid(L2, R2, 'Webhook: alert forwarding'))))
    lines.append(R(merge(bMid(L1, R1, 'NTP: all nodes sync required'), bMid(L2, R2, 'Syslog: event forwarding'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Nexus Dashboard hardware appliance (NDC) or VMware/KVM VMs; 3-node minimum;'))
    lines.append(txt_row('management + data networks; outbound HTTPS to Cisco app store for installs.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('ND            = Nexus Dashboard; Cisco app-hosting platform for DC management'))
    lines.append(txt_row('NDFC          = Nexus Dashboard Fabric Controller; SAN/LAN fabric management'))
    lines.append(txt_row('NDI           = Nexus Dashboard Insights; telemetry + network assurance'))
    lines.append(txt_row('NDO           = Nexus Dashboard Orchestrator; ACI multi-site policy'))
    lines.append(txt_row('3-node        = minimum HA cluster; tolerates 1 node failure'))
    lines.append(txt_row('5-node        = production recommended; separates infra and app nodes'))
    lines.append(txt_row('Kubernetes    = ND runs K8s internally; apps are pods on the cluster'))
    lines.append(txt_row('Etcd          = distributed KV for cluster state; used by ND Kubernetes'))
    lines.append(txt_row('Data network  = second NIC; device discovery + telemetry collection'))
    lines.append(txt_row('Multi-site    = ND instances linked across sites for global view'))
    lines.append(txt_row('App store     = Cisco app catalogue; install NDFC/NDI/NDO from UI'))
    lines.append(txt_row('NDC           = Nexus Dashboard Cluster; Cisco hardware appliance option'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'cisco-mds-deploy-index',
    'docs/san/cisco/mds/deploy/index.md',
    'Cisco MDS Deployment — initial setup, VSAN creation, zoning, NDFC discovery',
)
def cisco_mds_deploy_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Cisco MDS Deployment — Initial Setup to Production'))
    lines.append(txt_row())
    lines.append(txt_row('Day-0 setup: license, NTP, AAA; create VSANs and assign ports; zone initiators'))
    lines.append(txt_row('to targets per VSAN; discover in NDFC; validate I/O with fctrace and fcping.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Phase 1: Day-0 Switch Setup'), bMid(L2, R2, 'Phase 2: VSAN and Zoning'))))
    lines.append(R(merge(bMid(L1, R1, 'Install NX-OS license'), bMid(L2, R2, 'Create VSAN per fabric role'))))
    lines.append(R(merge(bMid(L1, R1, 'Configure NTP servers'), bMid(L2, R2, 'Assign FC ports to VSAN'))))
    lines.append(R(merge(bMid(L1, R1, 'Set TACACS+/RADIUS AAA'), bMid(L2, R2, 'Create device aliases (WWN)'))))
    lines.append(R(merge(bMid(L1, R1, 'Enable features: fcoe/npv/etc'), bMid(L2, R2, 'Create zones: 1 init + 1 tgt'))))
    lines.append(R(merge(bMid(L1, R1, 'Configure mgmt IP + VRF'), bMid(L2, R2, 'Activate zone set per VSAN'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Device aliases persist across reboots; always use aliases not raw WWNs in zones.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Phase 3: ISL and NDFC'), bMid(L2, R2, 'Phase 4: Validation'))))
    lines.append(R(merge(bMid(L1, R1, 'Configure TE port ISL trunks'), bMid(L2, R2, 'show flogi database: HBA check'))))
    lines.append(R(merge(bMid(L1, R1, 'Verify allowed VSANs on ISL'), bMid(L2, R2, 'fcping: initiator to target'))))
    lines.append(R(merge(bMid(L1, R1, 'Add switch to NDFC discovery'), bMid(L2, R2, 'fctrace: path hop tracing'))))
    lines.append(R(merge(bMid(L1, R1, 'Sync zone DB via NDFC'), bMid(L2, R2, 'show zoneset active: confirm'))))
    lines.append(R(merge(bMid(L1, R1, 'Enable SNMP v3 for NDFC'), bMid(L2, R2, 'Host: rescan HBA after zoning'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('MDS switches in SAN racks; cabled to host HBAs and storage ports; separate'))
    lines.append(txt_row('A and B fabric physical paths; management network cable to mgmt0 port.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('VSAN          = Virtual SAN; logical fabric; keep prod storage on dedicated VSAN'))
    lines.append(txt_row('Zone          = access control pair: one initiator (HBA) + one or more targets'))
    lines.append(txt_row('Device alias  = human-readable WWN label; use instead of raw hex in zones'))
    lines.append(txt_row('Zone set      = collection of zones activated as a group on a VSAN'))
    lines.append(txt_row('TE port       = trunked E port; ISL carrying multiple VSANs between switches'))
    lines.append(txt_row('FLOGI         = Fabric Login; HBA login event; first step to reach storage'))
    lines.append(txt_row('fcping        = FC-layer ping; confirms path between initiator and target WWN'))
    lines.append(txt_row('fctrace       = FC path trace (like traceroute); shows hop-by-hop FCID path'))
    lines.append(txt_row('NDFC          = Nexus Dashboard Fabric Controller; central MDS management'))
    lines.append(txt_row('AAA           = Authentication, Authorization, Accounting; TACACS+ preferred'))
    lines.append(txt_row('NTP           = required; fabric domain elections can fail if clocks diverge'))
    lines.append(txt_row('Feature       = NX-OS capability flag; must enable (e.g. feature npv) before use'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
