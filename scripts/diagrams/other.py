...
@kb_diagram(
    'nutanix-arch-index',
    'docs/virtualization/nutanix/architecture/index.md',
    'Nutanix Architecture — AOS, AHV, Prism, CVM, distributed storage',
)
def nutanix_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Nutanix Architecture — AOS HCI Cluster'))
    lines.append(txt_row())
    lines.append(txt_row('Each Nutanix node runs AHV hypervisor + CVM (Controller VM) for local storage;'))
    lines.append(txt_row('DSF pools all node disks into one distributed datastore; Prism manages cluster.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Node Architecture'), bMid(L2, R2, 'Distributed Storage Fabric'))))
    lines.append(R(merge(bMid(L1, R1, 'AHV: Type-1 hypervisor (KVM)'), bMid(L2, R2, 'DSF: pools all node disks'))))
    lines.append(R(merge(bMid(L1, R1, 'CVM: runs AOS + storage svc'), bMid(L2, R2, 'RF2/RF3: replica factor'))))
    lines.append(R(merge(bMid(L1, R1, 'CVM: reserved 4 vCPU / 12 GB'), bMid(L2, R2, 'Erasure coding: 4+2 / 8+2'))))
    lines.append(R(merge(bMid(L1, R1, 'Local SSD: CVM cache tier'), bMid(L2, R2, 'OpLog: write buffer per node'))))
    lines.append(R(merge(bMid(L1, R1, 'HDD: persistent capacity tier'), bMid(L2, R2, 'Medusa: metadata key-value'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('CVM intercepts all storage I/O from VMs via iSCSI/NFS; handles dedup/compression.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Management Plane (Prism)'), bMid(L2, R2, 'Cluster Services'))))
    lines.append(R(merge(bMid(L1, R1, 'Prism Element: per-cluster UI'), bMid(L2, R2, 'Zookeeper: cluster config'))))
    lines.append(R(merge(bMid(L1, R1, 'Prism Central: multi-cluster'), bMid(L2, R2, 'Cassandra: metric storage'))))
    lines.append(R(merge(bMid(L1, R1, 'REST API v2/v3: automation'), bMid(L2, R2, 'Genesis: service manager'))))
    lines.append(R(merge(bMid(L1, R1, 'RBAC: role-based access'), bMid(L2, R2, 'Cerebro: replication engine'))))
    lines.append(R(merge(bMid(L1, R1, 'LCM: lifecycle manager'), bMid(L2, R2, 'Stargate: I/O path handler'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Nutanix NX or OEM (Dell XC, HPE DX, Lenovo HX) nodes; 10/25 GbE switches;'))
    lines.append(txt_row('IPMI/iDRAC/iLO for OOB management; Foundation for bare-metal imaging.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('AOS           = Acropolis OS; Nutanix hyperconverged OS on each node'))
    lines.append(txt_row('AHV           = Acropolis Hypervisor; KVM-based Type-1; replaces ESXi/Hyper-V'))
    lines.append(txt_row('CVM           = Controller VM; runs storage services per node; always on'))
    lines.append(txt_row('DSF           = Distributed Storage Fabric; cluster-wide virtual datastore'))
    lines.append(txt_row('RF2/RF3       = Replication Factor; 2 or 3 copies per block across nodes'))
    lines.append(txt_row('Prism Element = single-cluster management UI/API'))
    lines.append(txt_row('Prism Central = multi-cluster; required for Flow, Calm, Karbon, Leap'))
    lines.append(txt_row('OpLog         = per-node SSD write buffer; flush to extent store async'))
    lines.append(txt_row('Medusa        = distributed key-value metadata store'))
    lines.append(txt_row('Stargate      = storage I/O handler in CVM; serves iSCSI/NFS to VMs'))
    lines.append(txt_row('LCM           = Lifecycle Manager; AOS/AHV/firmware updates'))
    lines.append(txt_row('Foundation    = Nutanix imaging tool; bare-metal cluster setup'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'nutanix-ops-index',
    'docs/virtualization/nutanix/operations/index.md',
    'Nutanix Operations — NCC health checks, admin tasks, backup, CLI',
)
def nutanix_ops_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Nutanix Operations — Health Checks and Admin Tasks'))
    lines.append(txt_row())
    lines.append(txt_row('Daily: NCC health checks, alert review, capacity monitoring; weekly: NCC full run,'))
    lines.append(txt_row('LCM check; as-needed: upgrades, disk repair, DR test.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Daily Operations'), bMid(L2, R2, 'Weekly Operations'))))
    lines.append(R(merge(bMid(L1, R1, 'NCC: ncc health_checks run_all'), bMid(L2, R2, 'NCC full: all plugin groups'))))
    lines.append(R(merge(bMid(L1, R1, 'Alerts: Prism alert dashboard'), bMid(L2, R2, 'LCM: check for updates'))))
    lines.append(R(merge(bMid(L1, R1, 'Capacity: storage % consumed'), bMid(L2, R2, 'Cluster health: HA status'))))
    lines.append(R(merge(bMid(L1, R1, 'Stargate: disk health green'), bMid(L2, R2, 'Cluster history: any issues'))))
    lines.append(R(merge(bMid(L1, R1, 'CVM status: allssh status'), bMid(L2, R2, 'Replication: Cerebro status'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('NCC result: PASS = healthy; WARN = investigate; FAIL = action required.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Administrative Tasks'), bMid(L2, R2, 'CLI Reference'))))
    lines.append(R(merge(bMid(L1, R1, 'VM create: Prism or REST API'), bMid(L2, R2, 'acli: AHV management CLI'))))
    lines.append(R(merge(bMid(L1, R1, 'Disk replace: hot-swap + repair'), bMid(L2, R2, 'ncli: cluster + storage CLI'))))
    lines.append(R(merge(bMid(L1, R1, 'Node add: expand cluster'), bMid(L2, R2, 'ncc: health check runner'))))
    lines.append(R(merge(bMid(L1, R1, 'Snapshot: per-VM or PD'), bMid(L2, R2, 'allssh: run cmd all CVMs'))))
    lines.append(R(merge(bMid(L1, R1, 'DR test: Leap failover test'), bMid(L2, R2, 'lcm: LCM CLI for upgrades'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Nutanix nodes with CVM on each; Prism Central VM for multi-cluster ops;'))
    lines.append(txt_row('10/25 GbE network; IPMI for OOB; Foundation VM for imaging.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('NCC           = Nutanix Cluster Check; health check framework; 100+ plugins'))
    lines.append(txt_row('acli          = Acropolis CLI; AHV VM management (create/delete/update)'))
    lines.append(txt_row('ncli          = Nutanix CLI; cluster, storage, network management'))
    lines.append(txt_row('allssh        = run command across all CVMs simultaneously'))
    lines.append(txt_row('PD            = Protection Domain; group of VMs for snapshot/replication'))
    lines.append(txt_row('LCM           = Lifecycle Manager; AOS + AHV + firmware upgrade orchestrator'))
    lines.append(txt_row('Stargate      = storage I/O daemon; check health via Prism > Hardware'))
    lines.append(txt_row('CVM rolling restart= safe way to restart services across all CVMs'))
    lines.append(txt_row('Leap          = Nutanix DR product; orchestrated failover to Nutanix cluster'))
    lines.append(txt_row('Cerebro       = replication engine in CVM; handles PD and Leap replication'))
    lines.append(txt_row('HA reserve    = capacity reserved for failed node; Prism > Cluster > HA'))
    lines.append(txt_row('Disk repair   = replace failed disk; CVM auto-rebuilds redundancy'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'nutanix-sec-index',
    'docs/virtualization/nutanix/security/index.md',
    'Nutanix Security — OS hardening, SSH lockdown, AD RBAC, encryption',
)
def nutanix_sec_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Nutanix Security — Hardening and Access Control'))
    lines.append(txt_row())
    lines.append(txt_row('OS hardening, SSH lockdown, AD integration, RBAC, and data-at-rest encryption;'))
    lines.append(txt_row('aligned with Nutanix Security Configuration Guide.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Cluster Hardening'), bMid(L2, R2, 'Access Control'))))
    lines.append(R(merge(bMid(L1, R1, 'Lockdown mode: SSH key-only'), bMid(L2, R2, 'AD: Prism Central integration'))))
    lines.append(R(merge(bMid(L1, R1, 'Port firewall: iptables on CVM'), bMid(L2, R2, 'RBAC: built-in roles + custom'))))
    lines.append(R(merge(bMid(L1, R1, 'STIG: Nutanix STIG available'), bMid(L2, R2, 'MFA: TOTP or SAML IdP'))))
    lines.append(R(merge(bMid(L1, R1, 'NCC: security check plugins'), bMid(L2, R2, 'Audit: user action logging'))))
    lines.append(R(merge(bMid(L1, R1, 'TLS 1.2+ enforced on APIs'), bMid(L2, R2, 'No shared service accounts'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Lockdown mode removes password SSH; key-only access via nutanix or root user.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Encryption'), bMid(L2, R2, 'Network Security'))))
    lines.append(R(merge(bMid(L1, R1, 'At-rest: AOS native encryption'), bMid(L2, R2, 'CVM firewall: default-deny'))))
    lines.append(R(merge(bMid(L1, R1, 'KMS: external key manager'), bMid(L2, R2, 'Prism: TLS port 9440'))))
    lines.append(R(merge(bMid(L1, R1, 'SEDs: self-encrypting drives'), bMid(L2, R2, 'Storage: iSCSI/NFS to AHV'))))
    lines.append(R(merge(bMid(L1, R1, 'Instant Secure Erase on retire'), bMid(L2, R2, 'Flow: micro-seg for AHV VMs'))))
    lines.append(R(merge(bMid(L1, R1, 'Backup encryption: PD snaps'), bMid(L2, R2, 'VLAN isolation: mgmt network'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Nutanix nodes with SED option for hardware encryption; separate management VLAN;'))
    lines.append(txt_row('AD/LDAP reachable from Prism Central on management network.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Lockdown mode = CVM setting; disables password SSH; key authentication only'))
    lines.append(txt_row('STIG          = Security Technical Implementation Guide; DoD hardening baseline'))
    lines.append(txt_row('AOS encryption= software-based at-rest encryption; no special hardware needed'))
    lines.append(txt_row('SED           = Self-Encrypting Drive; hardware encryption; key managed by KMS'))
    lines.append(txt_row('KMS           = Key Management Server; external; required for SED management'))
    lines.append(txt_row('Flow          = Nutanix micro-segmentation; AHV VM network policy enforcement'))
    lines.append(txt_row('Prism RBAC    = role-based access; Cluster Admin, VM Admin, Monitor built-in'))
    lines.append(txt_row('SAML IdP      = SSO via Okta/ADFS/Azure AD; Prism Central setting'))
    lines.append(txt_row('CVM firewall  = iptables rules on each CVM; default-deny inbound'))
    lines.append(txt_row('ISE           = Instant Secure Erase; cryptographic erase on drive retirement'))
    lines.append(txt_row('NCC security  = NCC plugins: SSH, TLS, open ports, default passwords'))
    lines.append(txt_row('Audit log     = Prism Central user activity log; API calls and UI actions'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'nutanix-ts-index',
    'docs/virtualization/nutanix/troubleshooting/index.md',
    'Nutanix Troubleshooting — NCC interpretation, log locations, GSS escalation',
)
def nutanix_ts_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Nutanix Troubleshooting — Diagnostics and Escalation'))
    lines.append(txt_row())
    lines.append(txt_row('Start with NCC; review Prism alerts; check logs; collect logbay bundle before'))
    lines.append(txt_row('contacting Nutanix GSS; include NCC output and cluster config details.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Diagnostic Flow'), bMid(L2, R2, 'Common Issues'))))
    lines.append(R(merge(bMid(L1, R1, '1. Run NCC health_checks all'), bMid(L2, R2, 'CVM down: cluster degraded'))))
    lines.append(R(merge(bMid(L1, R1, '2. Review Prism alerts'), bMid(L2, R2, 'Disk failure: auto-rebuild'))))
    lines.append(R(merge(bMid(L1, R1, '3. Check cluster health tab'), bMid(L2, R2, 'Network: CVM SSH unreachable'))))
    lines.append(R(merge(bMid(L1, R1, '4. Collect logbay bundle'), bMid(L2, R2, 'Storage: RF2 under threshold'))))
    lines.append(R(merge(bMid(L1, R1, '5. Open GSS case if unresolved'), bMid(L2, R2, 'Upgrade stuck: LCM task fail'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Never restart all CVMs simultaneously — use rolling restart via Prism or ncli.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Log Locations'), bMid(L2, R2, 'GSS Escalation'))))
    lines.append(R(merge(bMid(L1, R1, '/home/nutanix/data/logs/'), bMid(L2, R2, 'portal.nutanix.com case'))))
    lines.append(R(merge(bMid(L1, R1, 'Stargate: stargate.INFO'), bMid(L2, R2, 'Severity: P1 (down) to P4'))))
    lines.append(R(merge(bMid(L1, R1, 'Cassandra: cassandra.INFO'), bMid(L2, R2, 'Pre-collect: logbay bundle'))))
    lines.append(R(merge(bMid(L1, R1, 'Genesis: genesis.out'), bMid(L2, R2, 'Include: cluster ID + version'))))
    lines.append(R(merge(bMid(L1, R1, 'NCC: ncc_output.log'), bMid(L2, R2, 'AOS + AHV version required'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('CVM SSH for log access; Prism for GUI diagnostics; IPMI/iDRAC for OOB;'))
    lines.append(txt_row('logbay uploads directly to Nutanix FTP for GSS analysis.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('NCC           = Nutanix Cluster Check; run: ncc health_checks run_all'))
    lines.append(txt_row('logbay        = support bundle tool; collects all CVM logs into one bundle'))
    lines.append(txt_row('GSS           = Global Support Services; Nutanix support team'))
    lines.append(txt_row('P1 severity   = cluster down or data loss risk; 24/7 Nutanix response'))
    lines.append(txt_row('Cluster ID    = unique identifier; found in Prism > Cluster Details'))
    lines.append(txt_row('CVM restart   = restart storage services on one node; safe if rolling'))
    lines.append(txt_row('Stargate FATAL= storage I/O path failure; high priority; check logs now'))
    lines.append(txt_row('RF alert      = replication factor degraded; disk/node failure needs attention'))
    lines.append(txt_row('Cassandra     = metadata store; failures affect cluster operations broadly'))
    lines.append(txt_row('Genesis       = service manager on CVM; restart resolves many CVM issues'))
    lines.append(txt_row('LCM task fail = upgrade failed; check LCM logs; may need manual pre-upgrade'))
    lines.append(txt_row('Prism alerts  = Prism > Alerts; filter by critical before checking logs'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'nutanix-internals-index',
    'docs/virtualization/nutanix/internals/index.md',
    'Nutanix Internals — AOS data path, OpLog, Cassandra, Stargate microservices',
)
def nutanix_internals_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Nutanix Internals — AOS Data Path and Microservices'))
    lines.append(txt_row())
    lines.append(txt_row('VM write hits Stargate in local CVM; buffered in OpLog (SSD); Curator manages'))
    lines.append(txt_row('replication RF2/RF3 to remote CVMs; Medusa stores metadata in Cassandra.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Write Data Path'), bMid(L2, R2, 'Read Data Path'))))
    lines.append(R(merge(bMid(L1, R1, 'VM write -> Stargate (local CVM)'), bMid(L2, R2, 'Metadata: Medusa lookup'))))
    lines.append(R(merge(bMid(L1, R1, 'OpLog: SSD write buffer'), bMid(L2, R2, 'Hit local node: direct read'))))
    lines.append(R(merge(bMid(L1, R1, 'Replica: async to remote CVM'), bMid(L2, R2, 'Remote node: Stargate RPC'))))
    lines.append(R(merge(bMid(L1, R1, 'Flush: OpLog -> extent store'), bMid(L2, R2, 'Cache: in-memory + SSD'))))
    lines.append(R(merge(bMid(L1, R1, 'Extent store: HDD capacity'), bMid(L2, R2, 'Dedup: fingerprint lookup'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('OpLog ensures write latency < 500 us; flush to HDD is asynchronous background.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'AOS Microservices'), bMid(L2, R2, 'Cluster Services'))))
    lines.append(R(merge(bMid(L1, R1, 'Stargate: I/O path handler'), bMid(L2, R2, 'Zookeeper: distributed config'))))
    lines.append(R(merge(bMid(L1, R1, 'Curator: background tasks'), bMid(L2, R2, 'Cassandra: metadata + KV'))))
    lines.append(R(merge(bMid(L1, R1, 'Medusa: metadata service'), bMid(L2, R2, 'Chronos: job scheduler'))))
    lines.append(R(merge(bMid(L1, R1, 'Cerebro: replication engine'), bMid(L2, R2, 'Ergon: async task engine'))))
    lines.append(R(merge(bMid(L1, R1, 'Genesis: service lifecycle mgr'), bMid(L2, R2, 'Prism: UI + API gateway'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('NVMe/SSD for OpLog + cache; HDD for extent store; 10/25 GbE for CVM-to-CVM'))
    lines.append(txt_row('replication and Stargate RPC; dedicated storage VLAN recommended.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Stargate      = I/O path handler in CVM; serves NFS/iSCSI to AHV VMs'))
    lines.append(txt_row('OpLog         = SSD write buffer; absorbs burst writes; async flush to HDD'))
    lines.append(txt_row('Extent store  = persistent HDD storage; data written after OpLog flush'))
    lines.append(txt_row('Medusa        = metadata service; tracks where each data block lives'))
    lines.append(txt_row('Curator       = background maintenance; dedup, compression, rebalance'))
    lines.append(txt_row('Cerebro       = replication engine; handles PD snapshots and Leap DR'))
    lines.append(txt_row('Genesis       = service manager; starts/stops CVM services'))
    lines.append(txt_row('Cassandra     = distributed KV store; cluster metadata and Medusa backend'))
    lines.append(txt_row('Zookeeper     = distributed coordination; leader election and config'))
    lines.append(txt_row('RF2           = 2 copies of every block across 2 different CVMs'))
    lines.append(txt_row('Fingerprint   = content hash used for inline deduplication comparison'))
    lines.append(txt_row('Ergon         = async task framework; tracks long-running ops (clones, moves)'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'linux-arch-index',
    'docs/compute/linux/architecture/index.md',
    'Linux Architecture Overview — distributions, kernel, systemd, package management',
)
def linux_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux Platform Architecture Overview'))
    lines.append(txt_row())
    lines.append(txt_row('Linux underpins most infrastructure; RHEL/CentOS for enterprise, Ubuntu for cloud;'))
    lines.append(txt_row('kernel + systemd + package manager form the core; network via NetworkManager or nmcli.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Distributions'), bMid(L2, R2, 'Core Components'))))
    lines.append(R(merge(bMid(L1, R1, 'RHEL: enterprise production'), bMid(L2, R2, 'Kernel: hardware abstraction'))))
    lines.append(R(merge(bMid(L1, R1, 'CentOS Stream: upstream test'), bMid(L2, R2, 'systemd: service manager'))))
    lines.append(R(merge(bMid(L1, R1, 'Ubuntu: cloud + DevOps'), bMid(L2, R2, 'glibc: C standard library'))))
    lines.append(R(merge(bMid(L1, R1, 'Debian: stable/minimal'), bMid(L2, R2, 'PAM: auth stack'))))
    lines.append(R(merge(bMid(L1, R1, 'SLES: SAP + HPC workloads'), bMid(L2, R2, 'NetworkManager: networking'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Choose distro based on vendor support matrix; RHEL for VMware, vSAN, SAP, Oracle.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Package Management'), bMid(L2, R2, 'Storage and Security'))))
    lines.append(R(merge(bMid(L1, R1, 'RPM/DNF: RHEL/CentOS/SLES'), bMid(L2, R2, 'LVM: logical volumes'))))
    lines.append(R(merge(bMid(L1, R1, 'APT/dpkg: Debian/Ubuntu'), bMid(L2, R2, 'ext4/XFS: filesystems'))))
    lines.append(R(merge(bMid(L1, R1, 'Subscription: RHEL + SLES'), bMid(L2, R2, 'SELinux/AppArmor: MAC'))))
    lines.append(R(merge(bMid(L1, R1, 'Repos: local mirror or CDN'), bMid(L2, R2, 'firewalld/nftables: FW'))))
    lines.append(R(merge(bMid(L1, R1, 'Errata: security + bug fixes'), bMid(L2, R2, 'SSH: remote access (22)'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('x86-64 or ARM64 bare metal or VM; NIC (1/10/25GbE); local disk or SAN/NAS;'))
    lines.append(txt_row('IPMI/iDRAC/iLO for OOB management; firmware and BIOS managed separately.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('RHEL           = Red Hat Enterprise Linux; subscription required; 10-year support'))
    lines.append(txt_row('systemd        = Linux init system and service manager; PID 1; unit files'))
    lines.append(txt_row('DNF            = package manager for RHEL 8+; replaces yum'))
    lines.append(txt_row('LVM            = Logical Volume Manager; flexible disk partitioning'))
    lines.append(txt_row('SELinux        = Security-Enhanced Linux; Mandatory Access Control by policy'))
    lines.append(txt_row('PAM            = Pluggable Authentication Modules; Linux auth stack'))
    lines.append(txt_row('glibc          = GNU C Library; fundamental library all Linux programs use'))
    lines.append(txt_row('NetworkManager = service that manages network interfaces and connections'))
    lines.append(txt_row('firewalld      = RHEL firewall daemon; zones and rules; wraps nftables/iptables'))
    lines.append(txt_row('XFS            = default filesystem for RHEL 7+; high performance, no shrink'))
    lines.append(txt_row('Subscription   = RHEL/SLES entitlement; required for security patches'))
    lines.append(txt_row('Errata         = security advisory with associated package updates'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'mysql-arch-index',
    'docs/compute/linux/mysql/architecture/index.md',
    'MySQL Architecture — InnoDB, replication, MySQL Router, InnoDB Cluster',
)
def mysql_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'MySQL — Relational Database Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Open-source RDBMS; InnoDB storage engine; primary-replica replication; MySQL'))
    lines.append(txt_row('Router for load balancing; InnoDB Cluster (Group Replication) for HA.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Engine Architecture'), bMid(L2, R2, 'InnoDB Internals'))))
    lines.append(R(merge(bMid(L1, R1, 'InnoDB: default engine'), bMid(L2, R2, 'Buffer pool: main memory cache'))))
    lines.append(R(merge(bMid(L1, R1, 'MyISAM: read-heavy legacy'), bMid(L2, R2, 'Redo log: crash recovery'))))
    lines.append(R(merge(bMid(L1, R1, 'MEMORY: temp tables in RAM'), bMid(L2, R2, 'Undo log: MVCC rollback'))))
    lines.append(R(merge(bMid(L1, R1, 'MVCC: concurrent reads'), bMid(L2, R2, 'Doublewrite: write safety'))))
    lines.append(R(merge(bMid(L1, R1, 'ACID: transaction guarantees'), bMid(L2, R2, 'Row locking: not table lock'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('InnoDB Buffer Pool should be 70-80% of available RAM for optimal performance.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Replication Topologies'), bMid(L2, R2, 'HA: InnoDB Cluster'))))
    lines.append(R(merge(bMid(L1, R1, 'Primary-Replica: async'), bMid(L2, R2, 'Group Replication: 3+ nodes'))))
    lines.append(R(merge(bMid(L1, R1, 'GTID: global tx tracking'), bMid(L2, R2, 'MySQL Router: conn routing'))))
    lines.append(R(merge(bMid(L1, R1, 'Semi-sync: 1 replica acked'), bMid(L2, R2, 'MySQL Shell: admin interface'))))
    lines.append(R(merge(bMid(L1, R1, 'GTID auto-position: simpler'), bMid(L2, R2, 'Auto-failover: Router sees'))))
    lines.append(R(merge(bMid(L1, R1, 'Binlog: base of replication'), bMid(L2, R2, 'Paxos: consensus protocol'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Linux VMs or bare metal; local NVMe for data directory preferred; NFS for backups;'))
    lines.append(txt_row('separate disk for binlogs and redo logs; dedicated network for replication.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('InnoDB         = default MySQL storage engine; ACID compliant; row locking'))
    lines.append(txt_row('Buffer Pool    = InnoDB memory cache for data and indexes; set to 70-80% RAM'))
    lines.append(txt_row('MVCC           = Multi-Version Concurrency Control; readers do not block writers'))
    lines.append(txt_row('GTID           = Global Transaction ID; unique per-transaction; simplifies failover'))
    lines.append(txt_row('Binlog         = binary log; records all changes; basis of replication'))
    lines.append(txt_row('Redo log       = InnoDB WAL; ensures crash recovery; innodb_log_files_in_group'))
    lines.append(txt_row('Group Replication= multi-primary or single-primary; Paxos consensus across nodes'))
    lines.append(txt_row('MySQL Router   = connection router; redirects writes to primary, reads to replicas'))
    lines.append(txt_row('MySQL Shell    = admin tool; configureCluster, addInstance, status checks'))
    lines.append(txt_row('Semi-sync      = primary waits for at least 1 replica to ACK before commit'))
    lines.append(txt_row('InnoDB Cluster = Group Replication + Router + Shell; HA solution for MySQL'))
    lines.append(txt_row('Doublewrite    = InnoDB write-safety buffer; prevents torn page on crash'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'postgresql-arch-index',
    'docs/compute/linux/postgresql/architecture/index.md',
    'PostgreSQL Architecture — MVCC, streaming replication, Patroni, pgBouncer',
)
def postgresql_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'PostgreSQL — Advanced Open-Source RDBMS Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('MVCC concurrency; streaming replication + WAL archiving for HA; Patroni for'))
    lines.append(txt_row('automated failover; pgBouncer for connection pooling; PITR for recovery.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Core Architecture'), bMid(L2, R2, 'Process Model'))))
    lines.append(R(merge(bMid(L1, R1, 'MVCC: concurrent read+write'), bMid(L2, R2, 'Postmaster: main process'))))
    lines.append(R(merge(bMid(L1, R1, 'WAL: write-ahead log'), bMid(L2, R2, 'Per-connection: backend fork'))))
    lines.append(R(merge(bMid(L1, R1, 'PGDATA: data directory'), bMid(L2, R2, 'Autovacuum: background GC'))))
    lines.append(R(merge(bMid(L1, R1, 'Shared buffers: memory cache'), bMid(L2, R2, 'Walwriter: WAL flush'))))
    lines.append(R(merge(bMid(L1, R1, 'Tablespace: disk layout'), bMid(L2, R2, 'Checkpointer: dirty flush'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('shared_buffers at 25% of RAM; effective_cache_size at 75% for query planner.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'HA and Replication'), bMid(L2, R2, 'Connection and Pooling'))))
    lines.append(R(merge(bMid(L1, R1, 'Streaming: primary to standby'), bMid(L2, R2, 'pgBouncer: conn pooler'))))
    lines.append(R(merge(bMid(L1, R1, 'WAL shipping: archive-based'), bMid(L2, R2, 'Transaction: PgBouncer mode'))))
    lines.append(R(merge(bMid(L1, R1, 'Patroni: auto failover'), bMid(L2, R2, 'pg_hba.conf: access control'))))
    lines.append(R(merge(bMid(L1, R1, 'etcd/Consul: DCS leader lock'), bMid(L2, R2, 'SSL: encrypt client conns'))))
    lines.append(R(merge(bMid(L1, R1, 'PITR: point-in-time restore'), bMid(L2, R2, 'HAProxy: frontend LB'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Linux VMs; local NVMe for PGDATA (latency-sensitive); separate mount for WAL;'))
    lines.append(txt_row('etcd cluster (3 nodes) required for Patroni; pgBouncer as separate VM.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PostgreSQL     = advanced open-source RDBMS; supports JSON, extensions, partitioning'))
    lines.append(txt_row('MVCC           = Multi-Version Concurrency Control; reads never block writes'))
    lines.append(txt_row('WAL            = Write-Ahead Log; pg_wal directory; basis of streaming replication'))
    lines.append(txt_row('Patroni        = HA manager for PostgreSQL; uses etcd/Consul for leader election'))
    lines.append(txt_row('etcd           = distributed KV store; Patroni uses it to hold leader lock'))
    lines.append(txt_row('pgBouncer      = connection pooler; reduces connection overhead on busy servers'))
    lines.append(txt_row('PGDATA         = PostgreSQL data directory; contains base/, pg_wal/, pg_tblspc/'))
    lines.append(txt_row('Streaming replication= primary sends WAL stream to standby in real time'))
    lines.append(txt_row('PITR           = Point-in-Time Recovery; restore to any moment using WAL archive'))
    lines.append(txt_row('Autovacuum     = background process; reclaims space from updated/deleted rows'))
    lines.append(txt_row('shared_buffers = PostgreSQL memory cache; set to 25% RAM; key tuning parameter'))
    lines.append(txt_row('DCS            = Distributed Configuration Store; etcd/Consul/ZooKeeper for Patroni'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'windows-arch-index',
    'docs/compute/windows-server/architecture/index.md',
    'Windows Server Architecture Overview — roles, AD DS, Hyper-V, Group Policy',
)
def windows_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Windows Server — Platform Architecture Overview'))
    lines.append(txt_row())
    lines.append(txt_row('Microsoft server OS; roles and features architecture; AD DS for identity;'))
    lines.append(txt_row('Group Policy for management; Hyper-V for virtualisation; WinRM for remote ops.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Core Roles'), bMid(L2, R2, 'Management Layer'))))
    lines.append(R(merge(bMid(L1, R1, 'AD DS: domain identity'), bMid(L2, R2, 'Group Policy: GPO'))))
    lines.append(R(merge(bMid(L1, R1, 'DNS: name resolution'), bMid(L2, R2, 'WinRM: remote PowerShell'))))
    lines.append(R(merge(bMid(L1, R1, 'DHCP: IP address service'), bMid(L2, R2, 'Server Manager: local UI'))))
    lines.append(R(merge(bMid(L1, R1, 'File Services: SMB shares'), bMid(L2, R2, 'WSUS: patch management'))))
    lines.append(R(merge(bMid(L1, R1, 'Hyper-V: virtualisation'), bMid(L2, R2, 'SCCM/Intune: config mgmt'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Roles are modular; install only what is needed; each role has its own services.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Storage and Networking'), bMid(L2, R2, 'Security Components'))))
    lines.append(R(merge(bMid(L1, R1, 'NTFS: primary filesystem'), bMid(L2, R2, 'Windows Defender: AV/EDR'))))
    lines.append(R(merge(bMid(L1, R1, 'ReFS: resilient filesystem'), bMid(L2, R2, 'Windows Firewall: host FW'))))
    lines.append(R(merge(bMid(L1, R1, 'iSCSI initiator: SAN attach'), bMid(L2, R2, 'BitLocker: disk encryption'))))
    lines.append(R(merge(bMid(L1, R1, 'Failover Clustering: WSFC'), bMid(L2, R2, 'Kerberos: auth protocol'))))
    lines.append(R(merge(bMid(L1, R1, 'SMB 3.0: file sharing'), bMid(L2, R2, 'TLS 1.3: wire encryption'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('x86-64 servers; TPM 2.0 required for BitLocker; BIOS/UEFI; NIC team for HA;'))
    lines.append(txt_row('iDRAC/iLO OOB management; Windows Server requires CAL for each connecting user.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('AD DS          = Active Directory Domain Services; user and computer identity'))
    lines.append(txt_row('Group Policy   = GPO; domain-wide config pushed to computers and users'))
    lines.append(txt_row('WinRM          = Windows Remote Management; PowerShell remote sessions'))
    lines.append(txt_row('Hyper-V        = Windows hypervisor; type-1; VMs run as partitions'))
    lines.append(txt_row('WSFC           = Windows Server Failover Cluster; SQL Always On, Hyper-V HA'))
    lines.append(txt_row('NTFS           = NT File System; permissions, journaling, compression'))
    lines.append(txt_row('ReFS           = Resilient File System; checksums, no repair needed; Storage Spaces'))
    lines.append(txt_row('SMB 3.0        = Server Message Block; Windows file sharing protocol'))
    lines.append(txt_row('BitLocker      = Microsoft full-disk encryption; requires TPM 2.0'))
    lines.append(txt_row('Kerberos       = AD authentication protocol; ticket-based; default in domain'))
    lines.append(txt_row('WSUS           = Windows Server Update Services; internal patch distribution'))
    lines.append(txt_row('CAL            = Client Access License; required per user/device connecting'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'ad-arch-index',
    'docs/compute/windows-server/active-directory/architecture/index.md',
    'Active Directory Architecture — AD DS, domain controllers, FSMO roles, replication',
)
def ad_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Active Directory — Identity and Directory Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('AD DS on Windows DCs; LDAP + Kerberos; NTDS.dit database; multi-master replication;'))
    lines.append(txt_row('Sites and Services control topology; FSMO roles for single-master operations.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Domain Controller'), bMid(L2, R2, 'Directory Structure'))))
    lines.append(R(merge(bMid(L1, R1, 'NTDS.dit: AD database'), bMid(L2, R2, 'Forest: top-level boundary'))))
    lines.append(R(merge(bMid(L1, R1, 'SYSVOL: GPO + scripts'), bMid(L2, R2, 'Domain: admin boundary'))))
    lines.append(R(merge(bMid(L1, R1, 'Kerberos KDC: auth service'), bMid(L2, R2, 'OU: Organizational Unit'))))
    lines.append(R(merge(bMid(L1, R1, 'LDAP 389/636: directory'), bMid(L2, R2, 'Trust: cross-domain auth'))))
    lines.append(R(merge(bMid(L1, R1, 'DNS integrated: SRV records'), bMid(L2, R2, 'Schema: object definitions'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('FSMO roles must be placed on reliable DCs; PDC Emulator is the most critical.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'FSMO Roles (5 total)'), bMid(L2, R2, 'Replication'))))
    lines.append(R(merge(bMid(L1, R1, 'Schema Master: 1 per forest'), bMid(L2, R2, 'Multi-master: all DCs equal'))))
    lines.append(R(merge(bMid(L1, R1, 'Domain Naming: 1 per forest'), bMid(L2, R2, 'USN: update sequence number'))))
    lines.append(R(merge(bMid(L1, R1, 'PDC Emulator: 1 per domain'), bMid(L2, R2, 'KCC: auto-topology build'))))
    lines.append(R(merge(bMid(L1, R1, 'RID Master: 1 per domain'), bMid(L2, R2, 'Sites: control WAN repl'))))
    lines.append(R(merge(bMid(L1, R1, 'Infra Master: 1 per domain'), bMid(L2, R2, 'SYSVOL: DFS-R replication'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('2+ Domain Controllers per domain (HA); Windows Server 2019/2022 VMs; dedicated'))
    lines.append(txt_row('NIC; time sync critical (Kerberos 5-min skew tolerance); DNS on same VMs.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('AD DS          = Active Directory Domain Services; the role providing identity'))
    lines.append(txt_row('DC             = Domain Controller; server running AD DS; holds NTDS.dit'))
    lines.append(txt_row('NTDS.dit       = Jet database file storing all AD objects and attributes'))
    lines.append(txt_row('SYSVOL         = shared folder on DCs; holds GPOs and logon scripts'))
    lines.append(txt_row('FSMO           = Flexible Single Master Operations; 5 special AD roles'))
    lines.append(txt_row('PDC Emulator   = handles password changes, time sync, legacy NT auth'))
    lines.append(txt_row('RID Master     = issues pools of RIDs to DCs so they can create unique SIDs'))
    lines.append(txt_row('KCC            = Knowledge Consistency Checker; automatically builds repl topology'))
    lines.append(txt_row('USN            = Update Sequence Number; tracks changes on each DC'))
    lines.append(txt_row('Site           = AD Sites and Services; maps to IP subnets; controls repl cost'))
    lines.append(txt_row('Trust          = relationship between domains/forests; enables cross-domain auth'))
    lines.append(txt_row('DFS-R          = Distributed File System Replication; replicates SYSVOL'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'sql-server-arch-index',
    'docs/compute/windows-server/sql-server/architecture/index.md',
    'SQL Server Architecture — Always On AG, FCI, SSMS, buffer pool, tempdb',
)
def sql_server_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'SQL Server — Enterprise RDBMS Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Microsoft SQL Server; Always On Availability Groups for HA/DR; FCI for local HA;'))
    lines.append(txt_row('buffer pool as main memory cache; tempdb performance critical for concurrency.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Engine Architecture'), bMid(L2, R2, 'Memory Components'))))
    lines.append(R(merge(bMid(L1, R1, 'SQL Server Database Engine'), bMid(L2, R2, 'Buffer pool: data cache'))))
    lines.append(R(merge(bMid(L1, R1, 'SQL Server Agent: jobs'), bMid(L2, R2, 'Plan cache: query plans'))))
    lines.append(R(merge(bMid(L1, R1, 'SSRS: report server'), bMid(L2, R2, 'Column store: in-memory'))))
    lines.append(R(merge(bMid(L1, R1, 'SSAS: analysis services'), bMid(L2, R2, 'tempdb: work area'))))
    lines.append(R(merge(bMid(L1, R1, 'SSIS: ETL integration'), bMid(L2, R2, 'Max server memory: required'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('tempdb contention is the #1 performance issue; use multiple data files (1 per CPU).'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'HA: Always On AG'), bMid(L2, R2, 'HA: Failover Cluster (FCI)'))))
    lines.append(R(merge(bMid(L1, R1, 'Primary: reads + writes'), bMid(L2, R2, 'Shared storage: SAN LUN'))))
    lines.append(R(merge(bMid(L1, R1, 'Secondaries: read replicas'), bMid(L2, R2, 'WSFC: Windows cluster'))))
    lines.append(R(merge(bMid(L1, R1, 'Listener: virtual IP/name'), bMid(L2, R2, 'One instance at a time'))))
    lines.append(R(merge(bMid(L1, R1, 'Sync or async per secondary'), bMid(L2, R2, 'AG+FCI: combine for best'))))
    lines.append(R(merge(bMid(L1, R1, 'Auto-failover: sync only'), bMid(L2, R2, 'DTC: distributed tx'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Windows Server VMs (min 4 vCPU, 16 GB RAM); local NVMe for tempdb; SAN LUN for'))
    lines.append(txt_row('user DBs (FCI) or local/SAN per replica (AG); WSFC quorum disk or cloud witness.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Always On AG   = Availability Group; HA/DR for user databases; most common SQL HA'))
    lines.append(txt_row('FCI            = Failover Cluster Instance; WSFC + shared storage; HA for instance'))
    lines.append(txt_row('Listener       = virtual network name for AG; clients connect here'))
    lines.append(txt_row('Buffer pool    = SQL Server memory cache for data pages; largest memory consumer'))
    lines.append(txt_row('tempdb         = system DB for temp tables, sort spills; shared across all sessions'))
    lines.append(txt_row('SSMS           = SQL Server Management Studio; primary admin GUI'))
    lines.append(txt_row('SQL Agent      = job scheduler; runs maintenance plans, SSIS packages, backups'))
    lines.append(txt_row('WSFC           = Windows Server Failover Cluster; required for both AG and FCI'))
    lines.append(txt_row('Max server memory= set to leave 10-15% RAM for OS; prevents OOM'))
    lines.append(txt_row('Sync replica   = AG secondary in sync mode; 0 RPO; auto-failover eligible'))
    lines.append(txt_row('Async replica  = AG secondary with lag; lower network cost; manual failover only'))
    lines.append(txt_row('Quorum         = WSFC vote mechanism; need majority to operate; use cloud witness'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'jira-arch-index',
    'docs/itsm/jira/architecture/index.md',
    'Jira Architecture — Data Center cluster, PostgreSQL, shared filesystem, load balancer',
)
def jira_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Jira — Project Tracking Platform Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Jira Data Center: clustered Java app; PostgreSQL backend; shared NFS for attachments;'))
    lines.append(txt_row('load balancer in front; Jira Cloud is SaaS alternative managed by Atlassian.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Center Architecture'), bMid(L2, R2, 'Storage Components'))))
    lines.append(R(merge(bMid(L1, R1, '2+ nodes: active-active'), bMid(L2, R2, 'PostgreSQL: primary DB'))))
    lines.append(R(merge(bMid(L1, R1, 'Hazelcast: cluster cache'), bMid(L2, R2, 'Oracle/SQL Server: also ok'))))
    lines.append(R(merge(bMid(L1, R1, 'Load balancer: sticky session'), bMid(L2, R2, 'NFS: shared home dir'))))
    lines.append(R(merge(bMid(L1, R1, 'REST API: external automation'), bMid(L2, R2, 'Attachments on NFS share'))))
    lines.append(R(merge(bMid(L1, R1, 'Tomcat: embedded web server'), bMid(L2, R2, 'Lucene: local search index'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Each node has own Lucene index; must be rebuilt if a node falls behind.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Key Concepts'), bMid(L2, R2, 'Cloud vs Data Center'))))
    lines.append(R(merge(bMid(L1, R1, 'Project: work container'), bMid(L2, R2, 'Cloud: Atlassian-managed SaaS'))))
    lines.append(R(merge(bMid(L1, R1, 'Issue: individual work item'), bMid(L2, R2, 'DC: self-hosted on prem'))))
    lines.append(R(merge(bMid(L1, R1, 'Workflow: state transitions'), bMid(L2, R2, 'DC: unlimited customisation'))))
    lines.append(R(merge(bMid(L1, R1, 'Board: sprint/kanban view'), bMid(L2, R2, 'DC: own DR/backup control'))))
    lines.append(R(merge(bMid(L1, R1, 'JQL: Jira Query Language'), bMid(L2, R2, 'Cloud: auto-upgrades'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('2+ Linux VMs (8 vCPU, 32 GB RAM each); NFS server or NAS for shared home;'))
    lines.append(txt_row('PostgreSQL cluster; load balancer (HAProxy or F5); internet access for app store.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Jira           = Atlassian project tracking; issues, boards, sprints, workflows'))
    lines.append(txt_row('Data Center    = Jira DC; self-hosted clustered edition; license required'))
    lines.append(txt_row('Hazelcast      = distributed in-memory cache used by Jira DC cluster members'))
    lines.append(txt_row('Lucene         = search indexing library; Jira uses per-node index replicas'))
    lines.append(txt_row('NFS home       = shared filesystem; all nodes mount same path for attachments'))
    lines.append(txt_row('JQL            = Jira Query Language; SQL-like issue search'))
    lines.append(txt_row('Workflow       = state machine for issues (To Do → In Progress → Done)'))
    lines.append(txt_row('App store      = Atlassian Marketplace; plugins extend Jira functionality'))
    lines.append(txt_row('Sticky session = load balancer directs user to same node for session state'))
    lines.append(txt_row('PostgreSQL     = preferred database; Oracle and SQL Server also supported'))
    lines.append(txt_row('Tomcat         = Java servlet container; Jira runs embedded within Tomcat'))
    lines.append(txt_row('REST API       = Jira REST v2/v3; create/update issues, query boards'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'servicenow-arch-index',
    'docs/itsm/servicenow/architecture/index.md',
    'ServiceNow Architecture — SaaS ITSM, instance model, CMDB, integration hub',
)
def servicenow_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'ServiceNow — Enterprise ITSM Platform Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('SaaS-only ITSM; dedicated customer instance; CMDB tracks CI relationships;'))
    lines.append(txt_row('Integration Hub for third-party connections; release cadence: quarterly.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Instance Architecture'), bMid(L2, R2, 'Core Modules'))))
    lines.append(R(merge(bMid(L1, R1, 'Dedicated: own DB + compute'), bMid(L2, R2, 'ITSM: incident + change'))))
    lines.append(R(merge(bMid(L1, R1, 'SaaS: ServiceNow-hosted'), bMid(L2, R2, 'CMDB: CI and topology'))))
    lines.append(R(merge(bMid(L1, R1, 'Multi-instance: no sharing'), bMid(L2, R2, 'Service Catalog: requests'))))
    lines.append(R(merge(bMid(L1, R1, 'Prod + non-prod instances'), bMid(L2, R2, 'Knowledge: KB articles'))))
    lines.append(R(merge(bMid(L1, R1, 'MID Server: on-prem bridge'), bMid(L2, R2, 'Asset: inventory tracking'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('MID Server is a Java agent on-prem; required for discovery and integration.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Integration Patterns'), bMid(L2, R2, 'Release Model'))))
    lines.append(R(merge(bMid(L1, R1, 'REST API: table API'), bMid(L2, R2, 'Quarterly: named by city'))))
    lines.append(R(merge(bMid(L1, R1, 'Integration Hub: connectors'), bMid(L2, R2, 'Current: Yokohama (2025)'))))
    lines.append(R(merge(bMid(L1, R1, 'MID Server: on-prem push'), bMid(L2, R2, '2 versions supported at once'))))
    lines.append(R(merge(bMid(L1, R1, 'Event Management: SNMP/syslog'), bMid(L2, R2, 'Subproduction clone'))))
    lines.append(R(merge(bMid(L1, R1, 'LDAP/SAML: user sync + SSO'), bMid(L2, R2, 'Update set: change bundle'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Entirely SaaS; MID Server VM required on-prem for discovery and integrations;'))
    lines.append(txt_row('network: instance on HTTPS 443; MID Server needs outbound 443 to instance URL.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('ServiceNow     = enterprise ITSM SaaS; incident, change, problem, service catalog'))
    lines.append(txt_row('Instance       = dedicated ServiceNow environment; own URL (company.service-now.com)'))
    lines.append(txt_row('CMDB           = Configuration Management DB; CIs and their relationships'))
    lines.append(txt_row('CI             = Configuration Item; any managed asset in CMDB'))
    lines.append(txt_row('MID Server     = on-prem Java agent; bridges ServiceNow cloud to your network'))
    lines.append(txt_row('Integration Hub= low-code connector platform; 200+ pre-built integrations'))
    lines.append(txt_row('Update set     = package of changes; moved between non-prod and prod instances'))
    lines.append(txt_row('Table API      = REST API for every ServiceNow table (incidents, CIs, etc.)'))
    lines.append(txt_row('Discovery      = automated CI discovery via MID Server + probes'))
    lines.append(txt_row('ITSM           = IT Service Management; incident, change, problem, request'))
    lines.append(txt_row('Quarterly release= new named version every 3 months (Utah, Vancouver, Xanadu...)'))
    lines.append(txt_row('Subproduction  = dev/test instance; cloned from prod for safe change testing'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
