...
@kb_diagram('linux', 'docs/compute/linux/index.md', 'Linux Overview')
def _linux_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2, L3, R3 = 3, 33, 36, 66, 69, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Overview'))
    lines.append(txt_row())
    lines.append(txt_row('Linux is the open-source OS kernel underpinning servers, containers, and cloud infrastructure.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2), bTop(L3, R3))))
    lines.append(R(merge(bMid(L1, R1, 'Core Subsystems'), bMid(L2, R2, 'Key Distributions'), bMid(L3, R3, 'Use Cases'))))
    lines.append(R(merge(bMid(L1, R1, 'Kernel: process + memory'), bMid(L2, R2, 'RHEL/CentOS: enterprise'), bMid(L3, R3, 'Web/app servers'))))
    lines.append(R(merge(bMid(L1, R1, 'Filesystem: VFS + ext4/xfs'), bMid(L2, R2, 'Ubuntu: cloud + desktop'), bMid(L3, R3, 'Container hosts'))))
    lines.append(R(merge(bMid(L1, R1, 'Network: TCP/IP + iptables'), bMid(L2, R2, 'Debian: stable base'), bMid(L3, R3, 'HPC workloads'))))
    lines.append(R(merge(bMid(L1, R1, 'Systemd: service + boot'), bMid(L2, R2, 'SUSE: SAP + Kubernetes'), bMid(L3, R3, 'Embedded/IoT'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2), bBot(L3, R3))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · CPU · RAM · NIC · storage disks/LUNs'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Kernel       = core OS code managing hardware, memory, processes, drivers'))
    lines.append(txt_row('VFS          = Virtual File System; abstraction layer over real filesystems'))
    lines.append(txt_row('Systemd      = init system and service manager; PID 1 on most modern distros'))
    lines.append(txt_row('RHEL         = Red Hat Enterprise Linux; subscription-based enterprise distro'))
    lines.append(txt_row('CentOS       = RHEL-compatible free rebuild; CentOS Stream is upstream of RHEL'))
    lines.append(txt_row('iptables     = netfilter firewall rules; nftables is modern replacement'))
    lines.append(txt_row('ext4         = default Linux filesystem; journalled, supports large files'))
    lines.append(txt_row('xfs          = high-performance journalled FS; default on RHEL 7+'))
    lines.append(txt_row('cgroups      = control groups; limit CPU/memory/IO per process group'))
    lines.append(txt_row('namespaces   = isolate PID/net/mount/user views; foundation of containers'))
    lines.append(txt_row('SELinux      = mandatory access control; labels processes and files'))
    lines.append(txt_row('systemctl    = command to start/stop/enable/status systemd services'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-ops-cli', 'docs/compute/linux/operations/cli-reference/index.md', 'Linux CLI Reference')
def _linux_ops_cli():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    ...
    lines.append(title_border(W2, 'Linux — CLI Reference'))
    lines.append(txt_row())
    lines.append(txt_row('Essential Linux CLI commands grouped by area: files, processes, network, and services.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'File and Filesystem'), bMid(L2, R2, 'Process Management'))))
    lines.append(R(merge(bMid(L1, R1, 'ls -lah / find / locate'), bMid(L2, R2, 'ps aux / top / htop'))))
    lines.append(R(merge(bMid(L1, R1, 'cp / mv / rm / mkdir'), bMid(L2, R2, 'kill -9 <pid> / pkill'))))
    lines.append(R(merge(bMid(L1, R1, 'chmod / chown / chgrp'), bMid(L2, R2, 'nice / renice / ionice'))))
    lines.append(R(merge(bMid(L1, R1, 'df -h / du -sh / lsblk'), bMid(L2, R2, 'systemctl start/stop/status'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  File ops are atomic where possible; use systemctl over service for systemd'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Network'), bMid(L2, R2, 'Text and Logs'))))
    lines.append(R(merge(bMid(L1, R1, 'ip addr / ip route / ss -tulpn'), bMid(L2, R2, 'grep / awk / sed / cut'))))
    lines.append(R(merge(bMid(L1, R1, 'ping / traceroute / dig'), bMid(L2, R2, 'tail -f / journalctl -f'))))
    lines.append(R(merge(bMid(L1, R1, 'curl / wget / nc'), bMid(L2, R2, 'sort / uniq / wc -l'))))
    lines.append(R(merge(bMid(L1, R1, 'iptables -L / nft list'), bMid(L2, R2, 'less / vim / cat / tee'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · TTY/SSH terminal · NIC · storage'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('ss -tulpn    = socket stats: TCP/UDP listening sockets with PID/process'))
    lines.append(txt_row('ip addr      = show/manage IP addresses; replaces ifconfig'))
    lines.append(txt_row('ip route     = show/manage routing table; replaces route'))
    lines.append(txt_row('journalctl   = query systemd journal; -f follows, -u filters by unit'))
    lines.append(txt_row('lsblk        = list block devices in tree view; shows mounts'))
    lines.append(txt_row('df -h        = disk free in human-readable units per filesystem'))
    lines.append(txt_row('du -sh       = disk usage of directory recursively, summarised'))
    lines.append(txt_row('chmod        = change file permissions: chmod 755 file'))
    lines.append(txt_row('chown        = change file owner/group: chown user:group file'))
    lines.append(txt_row('nice/renice  = set process priority (-20 highest, 19 lowest)'))
    lines.append(txt_row('ionice       = set I/O scheduler class for a process'))
    lines.append(txt_row('tee          = reads stdin, writes to both stdout and file'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-ops-common-issues', 'docs/compute/linux/operations/common-issues/index.md', 'Linux Common Issues')
def _linux_ops_common():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Common Issues'))
    lines.append(txt_row())
    lines.append(txt_row('Most frequent Linux operational issues: disk full, high load, OOM kills, and network errors.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Disk and Filesystem'), bMid(L2, R2, 'Performance'))))
    lines.append(R(merge(bMid(L1, R1, 'Disk full → df -h, find large'), bMid(L2, R2, 'High load → top, iostat'))))
    lines.append(R(merge(bMid(L1, R1, 'Inode exhausted → df -i'), bMid(L2, R2, 'High mem → free -h, ps aux'))))
    lines.append(R(merge(bMid(L1, R1, 'Mount fails → fstab syntax'), bMid(L2, R2, 'OOM kill → /var/log/messages'))))
    lines.append(R(merge(bMid(L1, R1, 'FS corrupt → fsck (unmounted)'), bMid(L2, R2, 'Swap usage → swapon -s'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  Disk and inode issues are separate; high load can be CPU or I/O wait'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Network'), bMid(L2, R2, 'Service and Boot'))))
    lines.append(R(merge(bMid(L1, R1, 'No route → ip route check'), bMid(L2, R2, 'Service fail → systemctl status'))))
    lines.append(R(merge(bMid(L1, R1, 'DNS fails → resolv.conf'), bMid(L2, R2, 'Boot hang → systemd-analyze'))))
    lines.append(R(merge(bMid(L1, R1, 'Port blocked → ss, iptables'), bMid(L2, R2, 'Unit not found → systemctl enable'))))
    lines.append(R(merge(bMid(L1, R1, 'SSH refused → sshd status'), bMid(L2, R2, 'Kernel panic → dmesg | tail'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · CPU · RAM · NIC · storage disks'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Inode        = metadata entry for a file; exhausted when too many small files'))
    lines.append(txt_row('OOM kill     = Out of Memory killer terminates process to free RAM'))
    lines.append(txt_row('I/O wait     = CPU idle waiting for disk; high iowait = storage bottleneck'))
    lines.append(txt_row('fsck         = filesystem check; must run on unmounted filesystem'))
    lines.append(txt_row('fstab        = /etc/fstab; lists filesystems to mount at boot'))
    lines.append(txt_row('resolv.conf  = /etc/resolv.conf; nameserver config for DNS resolution'))
    lines.append(txt_row('swapon -s    = list swap devices and usage'))
    lines.append(txt_row('systemd-analyze= shows boot time per unit; blame shows slowest units'))
    lines.append(txt_row('dmesg        = kernel ring buffer; shows hardware and boot messages'))
    lines.append(txt_row('ss           = socket statistics; replacement for netstat'))
    lines.append(txt_row('iostat       = I/O statistics per disk; part of sysstat package'))
    lines.append(txt_row('Kernel panic = fatal error; system halts; check dmesg before reboot'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
def _linux_sec():
    ...
    L1, R1, L2, R2, L3, R3 = 3, 33, 36, 66, 69, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Security'))
    lines.append(txt_row())
    lines.append(txt_row('Linux security layers: access control, authentication, encryption, and hardening.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2), bTop(L3, R3))))
    lines.append(R(merge(bMid(L1, R1, 'Access Control'), bMid(L2, R2, 'Authentication'), bMid(L3, R3, 'Hardening'))))
    lines.append(R(merge(bMid(L1, R1, 'DAC: chmod/chown'), bMid(L2, R2, 'SSH key-based auth'), bMid(L3, R3, 'Patch: yum/apt update'))))
    lines.append(R(merge(bMid(L1, R1, 'MAC: SELinux/AppArmor'), bMid(L2, R2, 'PAM: pluggable auth'), bMid(L3, R3, 'Firewall: iptables/nft'))))
    lines.append(R(merge(bMid(L1, R1, 'sudo: privilege escalation'), bMid(L2, R2, 'SSSD: AD/LDAP join'), bMid(L3, R3, 'CIS benchmark: baseline'))))
    lines.append(R(merge(bMid(L1, R1, 'ACLs: getfacl/setfacl'), bMid(L2, R2, 'MFA: TOTP via PAM'), bMid(L3, R3, 'auditd: syscall logging'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2), bBot(L3, R3))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · TPM chip · UEFI Secure Boot · HSM (optional)'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('DAC          = Discretionary Access Control; owner sets permissions (chmod/chown)'))
    lines.append(txt_row('MAC          = Mandatory Access Control; kernel enforces labels (SELinux/AppArmor)'))
    lines.append(txt_row('SELinux      = Security-Enhanced Linux; type enforcement, context labels'))
    lines.append(txt_row('AppArmor     = path-based MAC; simpler than SELinux; default on Ubuntu/Debian'))
    lines.append(txt_row('PAM          = Pluggable Authentication Modules; auth pipeline for login/sudo'))
    lines.append(txt_row('SSSD         = System Security Services Daemon; integrates with AD/LDAP'))
    lines.append(txt_row('sudo         = run command as another user (usually root); logged in /var/log/secure'))
    lines.append(txt_row('auditd       = kernel audit subsystem; logs file access, syscalls, user actions'))
    lines.append(txt_row('ACL          = Access Control List; per-user/group permissions beyond owner/group'))
    lines.append(txt_row('CIS          = Center for Internet Security; publishes Linux hardening benchmarks'))
    lines.append(txt_row('Secure Boot  = UEFI feature verifying bootloader signature before execution'))
    lines.append(txt_row('TPM          = Trusted Platform Module; hardware for key storage and measured boot'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-sec-access', 'docs/compute/linux/security/access-control/index.md', 'Linux Access Control')
def _linux_sec_access():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Access Control'))
    lines.append(txt_row())
    lines.append(txt_row('Linux access control: DAC permissions, ACLs, sudo, and privilege management.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'File Permissions (DAC)'), bMid(L2, R2, 'sudo Configuration'))))
    lines.append(R(merge(bMid(L1, R1, 'Mode: rwxrwxrwx owner/group/other'), bMid(L2, R2, '/etc/sudoers: NOPASSWD rules'))))
    lines.append(R(merge(bMid(L1, R1, 'chmod 750: rwxr-x---'), bMid(L2, R2, 'visudo: safe edit with validation'))))
    lines.append(R(merge(bMid(L1, R1, 'SUID: run as file owner'), bMid(L2, R2, 'sudo -l: list user allowances'))))
    lines.append(R(merge(bMid(L1, R1, 'Sticky bit: dir delete restrict'), bMid(L2, R2, 'sudoers.d: drop-in includes'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  Standard permissions first; ACLs for exceptions; sudo for admin escalation'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'ACLs'), bMid(L2, R2, 'Capability Control'))))
    lines.append(R(merge(bMid(L1, R1, 'setfacl -m u:user:rwx file'), bMid(L2, R2, 'getcap / setcap: fine-grained'))))
    lines.append(R(merge(bMid(L1, R1, 'getfacl file: view all ACLs'), bMid(L2, R2, 'CAP_NET_BIND: port < 1024'))))
    lines.append(R(merge(bMid(L1, R1, 'Default ACL on dir: inherit'), bMid(L2, R2, 'Drop root caps via capsh'))))
    lines.append(R(merge(bMid(L1, R1, 'mask: effective permission limit'), bMid(L2, R2, 'Ambient caps in systemd units'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · LDAP/AD for user provisioning · audit logging'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('DAC          = Discretionary Access Control; owner controls permissions'))
    lines.append(txt_row('SUID         = Set User ID bit; process runs as file owner, not caller'))
    lines.append(txt_row('Sticky bit   = on directory: only owner can delete their own files'))
    lines.append(txt_row('ACL          = Access Control List; per-user permissions beyond owner/group'))
    lines.append(txt_row('setfacl      = set ACL entries; -m modifies, -x removes, -b removes all'))
    lines.append(txt_row('getfacl      = display ACL entries for a file or directory'))
    lines.append(txt_row('Default ACL  = ACL on directory inherited by new files created within'))
    lines.append(txt_row('mask         = maximum effective ACL permission; limits named user/group ACEs'))
    lines.append(txt_row('Capability   = fine-grained privilege; alternative to SUID root'))
    lines.append(txt_row('CAP_NET_BIND = allows binding to port < 1024 without root'))
    lines.append(txt_row('visudo       = validates /etc/sudoers before writing; prevents lock-out'))
    lines.append(txt_row('sudoers.d    = /etc/sudoers.d/ directory; separate include files per role'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-sec-auth', 'docs/compute/linux/security/authentication/index.md', 'Linux Authentication')
def _linux_sec_auth():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    ...
    lines.append(title_border(W2, 'Linux — Authentication'))
    lines.append(txt_row())
    lines.append(txt_row('Linux authentication: SSH keys, PAM modules, SSSD for AD/LDAP, and MFA enforcement.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'SSH Key Authentication'), bMid(L2, R2, 'PAM Authentication'))))
    lines.append(R(merge(bMid(L1, R1, 'Generate: ssh-keygen -t ed25519'), bMid(L2, R2, '/etc/pam.d/sshd: auth stack'))))
    lines.append(R(merge(bMid(L1, R1, 'Deploy: ssh-copy-id user@host'), bMid(L2, R2, 'pam_unix: local passwd'))))
    lines.append(R(merge(bMid(L1, R1, '~/.ssh/authorized_keys: pub key'), bMid(L2, R2, 'pam_google_auth: TOTP MFA'))))
    lines.append(R(merge(bMid(L1, R1, 'PasswordAuthentication no in sshd'), bMid(L2, R2, 'pam_ldap: LDAP bind'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  SSH key auth preferred; disable password; use PAM for MFA + LDAP'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'SSSD (AD/LDAP Integration)'), bMid(L2, R2, 'Local Account Management'))))
    lines.append(R(merge(bMid(L1, R1, 'realm join domain.example.com'), bMid(L2, R2, 'useradd / usermod / userdel'))))
    lines.append(R(merge(bMid(L1, R1, '/etc/sssd/sssd.conf: config'), bMid(L2, R2, 'passwd / chpasswd: set password'))))
    lines.append(R(merge(bMid(L1, R1, 'sssctl user-checks: diagnostics'), bMid(L2, R2, 'chage: password expiry policy'))))
    lines.append(R(merge(bMid(L1, R1, 'Kerberos tickets: kinit/klist'), bMid(L2, R2, '/etc/shadow: hashed passwords'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · AD domain controllers · LDAP server · NTP'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('ed25519      = modern elliptic-curve SSH key type; preferred over RSA'))
    lines.append(txt_row('authorized_keys= file listing public keys allowed to authenticate via SSH'))
    lines.append(txt_row('PAM          = Pluggable Authentication Modules; stacked auth policy'))
    lines.append(txt_row('pam_google_auth= PAM module adding TOTP (Google Authenticator) second factor'))
    lines.append(txt_row('SSSD         = System Security Services Daemon; caches AD/LDAP credentials'))
    lines.append(txt_row('realm join   = joins Linux host to AD domain; configures Kerberos + SSSD'))
    lines.append(txt_row('Kerberos     = ticket-based network auth; realm join enables it on Linux'))
    lines.append(txt_row('kinit        = obtain Kerberos ticket-granting ticket'))
    lines.append(txt_row('chage        = change password aging info: expiry, min/max days'))
    lines.append(txt_row('/etc/shadow  = stores hashed passwords; root-readable only'))
    lines.append(txt_row('sssctl       = SSSD control tool; check user/group lookup and auth'))
    lines.append(txt_row('PasswordAuthentication= sshd_config option; set to no to disable SSH passwords'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-sec-enc', 'docs/compute/linux/security/encryption/index.md', 'Linux Encryption')
def _linux_sec_enc():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    ...
    lines.append(title_border(W2, 'Linux — Encryption'))
    lines.append(txt_row())
    lines.append(txt_row('Linux encryption: LUKS disk encryption, TLS for services, GPG for files, and key management.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Disk Encryption (LUKS)'), bMid(L2, R2, 'TLS / PKI'))))
    lines.append(R(merge(bMid(L1, R1, 'cryptsetup luksFormat /dev/sdX'), bMid(L2, R2, 'openssl req: generate CSR'))))
    lines.append(R(merge(bMid(L1, R1, 'cryptsetup open: mapped device'), bMid(L2, R2, 'certbot: Let Encrypt TLS cert'))))
    lines.append(R(merge(bMid(L1, R1, '/etc/crypttab: unlock at boot'), bMid(L2, R2, 'update-ca-certificates: trust'))))
    lines.append(R(merge(bMid(L1, R1, 'Tang/Clevis: network auto-unlock'), bMid(L2, R2, 'stunnel: TLS wrapper for TCP'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  LUKS for data at rest; TLS for data in transit; GPG for file-level'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'GPG File Encryption'), bMid(L2, R2, 'SSH Transport Encryption'))))
    lines.append(R(merge(bMid(L1, R1, 'gpg --gen-key: create keypair'), bMid(L2, R2, 'AES-256-GCM: session cipher'))))
    lines.append(R(merge(bMid(L1, R1, 'gpg -e -r recipient file'), bMid(L2, R2, 'ed25519: host key algorithm'))))
    lines.append(R(merge(bMid(L1, R1, 'gpg -d file.gpg: decrypt'), bMid(L2, R2, 'known_hosts: host key trust'))))
    lines.append(R(merge(bMid(L1, R1, 'gpg-agent: key in memory'), bMid(L2, R2, 'StrictHostKeyChecking yes'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · TPM · HSM (optional) · CA infrastructure'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('LUKS         = Linux Unified Key Setup; standard for block device encryption'))
    lines.append(txt_row('cryptsetup   = tool to create/manage LUKS containers and dm-crypt devices'))
    lines.append(txt_row('crypttab     = /etc/crypttab; lists encrypted volumes to unlock at boot'))
    lines.append(txt_row('Tang/Clevis  = network-bound disk encryption; auto-unlock via NBDE server'))
    lines.append(txt_row('GPG          = GNU Privacy Guard; PGP-compatible asymmetric encryption'))
    lines.append(txt_row('CSR          = Certificate Signing Request; submitted to CA for signed cert'))
    lines.append(txt_row('certbot      = Let Encrypt ACME client; automates cert issuance/renewal'))
    lines.append(txt_row('update-ca-certificates= adds CA cert to system trust store (Debian/Ubuntu)'))
    lines.append(txt_row('stunnel      = wraps plain TCP in TLS; useful for legacy app encryption'))
    lines.append(txt_row('known_hosts  = ~/.ssh/known_hosts; stores trusted server public key hashes'))
    lines.append(txt_row('StrictHostKeyChecking= reject connection if host key not in known_hosts'))
    lines.append(txt_row('dm-crypt     = kernel device mapper; LUKS sits on top as a key manager'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-sec-hard', 'docs/compute/linux/security/hardening/index.md', 'Linux Hardening')
def _linux_sec_hard():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Hardening'))
    lines.append(txt_row())
    lines.append(txt_row('Linux server hardening: CIS benchmark, kernel tuning, service reduction, and audit logging.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'OS Baseline'), bMid(L2, R2, 'Network Hardening'))))
    lines.append(R(merge(bMid(L1, R1, 'Patch: auto-update critical CVEs'), bMid(L2, R2, 'Firewall: default-deny inbound'))))
    lines.append(R(merge(bMid(L1, R1, 'Remove unused packages/services'), bMid(L2, R2, 'sshd: port 22 → custom port'))))
    lines.append(R(merge(bMid(L1, R1, 'Disable root SSH login'), bMid(L2, R2, 'AllowUsers: restrict SSH users'))))
    lines.append(R(merge(bMid(L1, R1, 'CIS-CAT or OpenSCAP: scan'), bMid(L2, R2, 'TCP wrappers: hosts.allow/deny'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  Patch first; reduce attack surface; enforce audit trail'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Kernel Parameters (sysctl)'), bMid(L2, R2, 'Audit and Logging'))))
    lines.append(R(merge(bMid(L1, R1, 'net.ipv4.ip_forward=0'), bMid(L2, R2, 'auditd: enable at boot'))))
    lines.append(R(merge(bMid(L1, R1, 'net.ipv4.tcp_syncookies=1'), bMid(L2, R2, 'auditctl: add watch rules'))))
    lines.append(R(merge(bMid(L1, R1, 'kernel.randomize_va_space=2'), bMid(L2, R2, 'rsyslog: forward to SIEM'))))
    lines.append(R(merge(bMid(L1, R1, 'fs.suid_dumpable=0'), bMid(L2, R2, 'logrotate: retention policy'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · Secure Boot · TPM · SIEM log destination'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('CIS benchmark= Center for Internet Security Linux hardening guide'))
    lines.append(txt_row('OpenSCAP     = open-source SCAP scanner; checks system against policy profiles'))
    lines.append(txt_row('sysctl       = runtime kernel parameter adjustment; persist in /etc/sysctl.d/'))
    lines.append(txt_row('ASLR         = Address Space Layout Randomisation; randomize_va_space=2'))
    lines.append(txt_row('SYN cookies  = mitigate TCP SYN flood; tcp_syncookies=1'))
    lines.append(txt_row('ip_forward   = route packets between interfaces; disable on non-routers'))
    lines.append(txt_row('auditd       = kernel audit daemon; logs file/syscall/network events'))
    lines.append(txt_row('auditctl     = add/remove audit rules at runtime'))
    lines.append(txt_row('rsyslog      = syslog daemon; can forward events to SIEM over TCP/TLS'))
    lines.append(txt_row('logrotate    = manages log file rotation, compression, and retention'))
    lines.append(txt_row('suid_dumpable= 0 disables core dumps for SUID programs'))
    lines.append(txt_row('AllowUsers   = sshd_config whitelist; only listed users can SSH in'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
def _linux_trouble():
    ...
    L1, R1, L2, R2, L3, R3 = 3, 33, 36, 66, 69, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Troubleshooting'))
    lines.append(txt_row())
    lines.append(txt_row('Linux troubleshooting: system performance, storage, network, and service failures.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2), bTop(L3, R3))))
    lines.append(R(merge(bMid(L1, R1, 'Performance'), bMid(L2, R2, 'Storage'), bMid(L3, R3, 'Network'))))
    lines.append(R(merge(bMid(L1, R1, 'top / htop: CPU breakdown'), bMid(L2, R2, 'df -h: filesystem usage'), bMid(L3, R3, 'ping / traceroute'))))
    lines.append(R(merge(bMid(L1, R1, 'vmstat / sar: memory trend'), bMid(L2, R2, 'iostat: disk I/O stats'), bMid(L3, R3, 'ss -tulpn: open ports'))))
    lines.append(R(merge(bMid(L1, R1, 'perf / strace: deep profiling'), bMid(L2, R2, 'dmesg: disk errors'), bMid(L3, R3, 'tcpdump: packet capture'))))
    lines.append(R(merge(bMid(L1, R1, 'OOM → dmesg | grep oom'), bMid(L2, R2, 'fsck: filesystem repair'), bMid(L3, R3, 'dig: DNS resolution'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2), bBot(L3, R3))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · CPU · RAM · NIC · storage disks'))
    ...
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('vmstat       = virtual memory stats; shows CPU, swap, I/O wait over interval'))
    lines.append(txt_row('sar          = System Activity Reporter; historical performance data'))
    lines.append(txt_row('perf         = Linux performance analysis tool; CPU profiling, tracing'))
    lines.append(txt_row('strace       = traces syscalls of a process; reveals file/network ops'))
    lines.append(txt_row('tcpdump      = packet capture; filters by interface, port, host'))
    lines.append(txt_row('iostat       = I/O statistics per disk; part of sysstat package'))
    lines.append(txt_row('fsck         = filesystem check/repair; run on unmounted filesystem'))
    lines.append(txt_row('dmesg        = kernel ring buffer; hardware events, disk errors, OOM kills'))
    lines.append(txt_row('OOM          = Out of Memory; kernel kills process to free RAM'))
    lines.append(txt_row('iowait       = CPU time waiting for I/O; high value = disk bottleneck'))
    lines.append(txt_row('load average = 1/5/15-min runqueue depth; > nCPU = saturation'))
    lines.append(txt_row('tcpdump -nn  = no hostname/port resolution; faster capture output'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
def _linux_trouble_common():
    ...
    lines.append(title_border(W2, 'Linux — Troubleshooting Common Issues'))
    lines.append(txt_row())
    lines.append(txt_row('Common Linux issues: SSH failures, boot problems, OOM kills, and permission errors.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'SSH Issues'), bMid(L2, R2, 'Boot Problems'))))
    lines.append(R(merge(bMid(L1, R1, 'Connection refused: sshd down?'), bMid(L2, R2, 'Hang: systemd-analyze blame'))))
    lines.append(R(merge(bMid(L1, R1, 'Permission denied: check key'), bMid(L2, R2, 'grub rescue: boot partition'))))
    lines.append(R(merge(bMid(L1, R1, 'Host key changed: known_hosts'), bMid(L2, R2, 'Emergency mode: root fs ro'))))
    lines.append(R(merge(bMid(L1, R1, 'MaxSessions: too many logins'), bMid(L2, R2, 'Kernel panic: dmesg serial'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  SSH issues: check daemon, then key, then host; boot: single-user or rescue'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Permission Errors'), bMid(L2, R2, 'Memory / OOM'))))
    lines.append(R(merge(bMid(L1, R1, 'Operation not permitted: capabilities'), bMid(L2, R2, 'OOM kill: dmesg | grep oom'))))
    lines.append(R(merge(bMid(L1, R1, 'Access denied: SELinux context'), bMid(L2, R2, 'Adjust /proc/sys/vm/overcommit'))))
    lines.append(R(merge(bMid(L1, R1, 'Cannot write: check owner/mode'), bMid(L2, R2, 'Add swap: dd + mkswap'))))
    lines.append(R(merge(bMid(L1, R1, 'sudo: not in sudoers: add user'), bMid(L2, R2, 'Tune vm.swappiness value'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · serial console (for boot) · SSH client'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Connection refused= port closed or sshd not running; check systemctl status sshd'))
    lines.append(txt_row('known_hosts  = remove stale entry: ssh-keygen -R hostname'))
    lines.append(txt_row('Emergency mode= systemd fallback; root filesystem mounted read-only for repair'))
    lines.append(txt_row('GRUB rescue  = minimal grub shell; set root/prefix then insmod + boot'))
    lines.append(txt_row('SELinux      = context mismatch causes silent denial; check audit.log'))
    lines.append(txt_row('OOM killer   = selects victim by oom_score; protect with oom_score_adj'))
    lines.append(txt_row('overcommit   = 0=heuristic, 1=always, 2=never; 2 prevents OOM but limits malloc'))
    lines.append(txt_row('swappiness   = 0-100; lower = prefer RAM; 60 default; 10 for database hosts'))
    lines.append(txt_row('Capabilities = sudo not required; use CAP_NET_ADMIN etc. for specific ops'))
    lines.append(txt_row('MaxSessions  = sshd_config; default 10 mux sessions per connection'))
    lines.append(txt_row('single-user  = boot target for recovery; append single to kernel cmdline'))
    lines.append(txt_row('serial console= kernel console on COM1; needed when SSH/VGA unavailable'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-trouble-diag', 'docs/compute/linux/troubleshooting/diagnostics/index.md', 'Linux Diagnostics')
def _linux_trouble_diag():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Diagnostics'))
    lines.append(txt_row())
    lines.append(txt_row('Linux diagnostic tools and techniques for CPU, memory, I/O, network, and kernel issues.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'CPU and Memory'), bMid(L2, R2, 'Disk I/O'))))
    lines.append(R(merge(bMid(L1, R1, 'top -H: per-thread view'), bMid(L2, R2, 'iostat -x 1: extended stats'))))
    lines.append(R(merge(bMid(L1, R1, 'perf top: hot functions'), bMid(L2, R2, 'iotop: per-process I/O'))))
    lines.append(R(merge(bMid(L1, R1, 'vmstat 1 10: memory trend'), bMid(L2, R2, 'blktrace / blkparse: tracing'))))
    lines.append(R(merge(bMid(L1, R1, 'strace -p <pid>: syscall trace'), bMid(L2, R2, 'smartctl -a /dev/sdX: health'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  Start broad (top/vmstat); narrow with perf/strace for root cause'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Network Diagnostics'), bMid(L2, R2, 'Kernel and System'))))
    lines.append(R(merge(bMid(L1, R1, 'tcpdump -i eth0 port 443'), bMid(L2, R2, 'dmesg -T: timestamped msgs'))))
    lines.append(R(merge(bMid(L1, R1, 'nmap -sV host: port scan'), bMid(L2, R2, 'journalctl -xe: recent errors'))))
    lines.append(R(merge(bMid(L1, R1, 'netstat -s: protocol counters'), bMid(L2, R2, 'eBPF / bpftrace: deep trace'))))
    lines.append(R(merge(bMid(L1, R1, 'ethtool -S eth0: NIC counters'), bMid(L2, R2, 'coredump: systemd-coredump'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · NIC · storage controller · serial console'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('perf top     = live profiling; shows functions consuming most CPU cycles'))
    lines.append(txt_row('strace -p    = attach to running process; shows each syscall with args/return'))
    lines.append(txt_row('iotop        = like top but shows I/O per process; requires root'))
    lines.append(txt_row('blktrace     = block I/O tracer; shows complete I/O lifecycle per request'))
    lines.append(txt_row('smartctl     = query drive SMART data; -a shows all attributes'))
    lines.append(txt_row('tcpdump      = raw packet capture; -w file saves for Wireshark analysis'))
    lines.append(txt_row('ethtool -S   = NIC statistics; shows errors, drops, ring buffer usage'))
    lines.append(txt_row('netstat -s   = per-protocol counters; TCP retransmits visible here'))
    lines.append(txt_row('eBPF         = extended Berkeley Packet Filter; safe kernel tracing'))
    lines.append(txt_row('bpftrace     = high-level eBPF scripting language for kernel/user tracing'))
    lines.append(txt_row('coredump     = crash memory snapshot; analysed with gdb or crash tool'))
    lines.append(txt_row('journalctl -xe= show most recent journal with explanations (-x)'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('linux-trouble-esc', 'docs/compute/linux/troubleshooting/escalation/index.md', 'Linux Escalation')
def _linux_trouble_esc():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Linux — Escalation'))
    lines.append(txt_row())
    lines.append(txt_row('Linux escalation paths: kernel panics, hardware failures, data corruption, security incidents.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Kernel Panic / Crash'), bMid(L2, R2, 'Hardware Failure'))))
    lines.append(R(merge(bMid(L1, R1, '1. Capture dmesg / vmcore'), bMid(L2, R2, '1. Check dmesg for errors'))))
    lines.append(R(merge(bMid(L1, R1, '2. Analyse with crash tool'), bMid(L2, R2, '2. Run smartctl on disks'))))
    lines.append(R(merge(bMid(L1, R1, '3. Report kernel BZ if kernel bug'), bMid(L2, R2, '3. Replace failed component'))))
    lines.append(R(merge(bMid(L1, R1, '4. Patch or workaround'), bMid(L2, R2, '4. Restore from backup if data'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('  Kernel issues → vendor support; hardware → ops team + replacement parts'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Corruption'), bMid(L2, R2, 'Security Incident'))))
    lines.append(R(merge(bMid(L1, R1, '1. Stop writes immediately'), bMid(L2, R2, '1. Isolate host from network'))))
    lines.append(R(merge(bMid(L1, R1, '2. Snapshot / backup image'), bMid(L2, R2, '2. Preserve logs + memory'))))
    lines.append(R(merge(bMid(L1, R1, '3. fsck on unmounted FS'), bMid(L2, R2, '3. Escalate to security team'))))
    lines.append(R(merge(bMid(L1, R1, '4. Restore from last good backup'), bMid(L2, R2, '4. Forensic image + analysis'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · backup storage · IPMI/iDRAC · SIEM'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('vmcore       = kernel crash dump; saved to /var/crash by kdump service'))
    lines.append(txt_row('kdump        = kernel crash dump mechanism; second kernel captures vmcore'))
    lines.append(txt_row('crash tool   = analyses vmcore with debug symbols; bt command shows stack'))
    lines.append(txt_row('kernel BZ    = kernel bug report to bugzilla.kernel.org or vendor tracker'))
    lines.append(txt_row('smartctl     = SMART disk health; Reallocated_Sector_Ct indicates bad blocks'))
    lines.append(txt_row('fsck         = filesystem check/repair; must run on unmounted filesystem'))
    lines.append(txt_row('Forensic image= dd or dcfldd to preserve disk state; chain of custody'))
    lines.append(txt_row('Isolate      = remove network access; preserve state without shutdown'))
    lines.append(txt_row('Preserve logs= copy /var/log before shutdown; volatile data lost on reboot'))
    lines.append(txt_row('IPMI/iDRAC   = out-of-band management; access console without network'))
    lines.append(txt_row('fsck -n      = dry-run check only; -y auto-fixes; use interactively'))
    lines.append(txt_row('Snapshot     = VM snapshot or LVM snapshot before repair attempts'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('windows-server', 'docs/compute/windows-server/index.md', 'Windows Server Overview')
def _windows_server():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2, L3, R3 = 3, 33, 36, 66, 69, 99
    lines = []
    lines.append(title_border(W2, 'Windows Server — Overview'))
    lines.append(txt_row())
    lines.append(txt_row('Windows Server provides enterprise OS, Active Directory, Hyper-V, and file services.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2), bTop(L3, R3))))
    lines.append(R(merge(bMid(L1, R1, 'Core Roles'), bMid(L2, R2, 'Editions'), bMid(L3, R3, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'AD DS: domain services'), bMid(L2, R2, 'Standard: 2 VM guests'), bMid(L3, R3, 'Server Manager UI'))))
    lines.append(R(merge(bMid(L1, R1, 'DNS / DHCP / NPS'), bMid(L2, R2, 'Datacenter: unlimited VMs'), bMid(L3, R3, 'Windows Admin Centre'))))
    lines.append(R(merge(bMid(L1, R1, 'Hyper-V: virtualisation'), bMid(L2, R2, 'Core: no GUI, minimal'), bMid(L3, R3, 'PowerShell + WinRM'))))
    lines.append(R(merge(bMid(L1, R1, 'IIS / File / Print'), bMid(L2, R2, 'Essentials: SMB 25 users'), bMid(L3, R3, 'Group Policy (GPO)'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2), bBot(L3, R3))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical or virtual server · CPU · RAM · NIC · NTFS/ReFS storage'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('AD DS        = Active Directory Domain Services; central identity and policy store'))
    lines.append(txt_row('Hyper-V      = Microsoft hypervisor built into Windows Server; type-1'))
    lines.append(txt_row('WinRM        = Windows Remote Management; PS remoting and remote admin'))
    lines.append(txt_row('GPO          = Group Policy Object; policy applied to OUs in AD'))
    lines.append(txt_row('Server Core  = minimal install without GUI; managed remotely via PS/WAC'))
    lines.append(txt_row('Windows Admin Centre= browser-based GUI for managing servers and clusters'))
    lines.append(txt_row('NPS          = Network Policy Server; RADIUS for 802.1X / VPN auth'))
    lines.append(txt_row('ReFS         = Resilient File System; alternative to NTFS; integrity streams'))
    lines.append(txt_row('IIS          = Internet Information Services; web/app server role'))
    lines.append(txt_row('NTFS         = New Technology File System; default FS with ACLs + journaling'))
    lines.append(txt_row('SMB          = Server Message Block; file sharing protocol; 3.x on Server 2016+'))
    lines.append(txt_row('CAL          = Client Access Licence; required per user or device connecting'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
def _windows_sec_index():
    ...
    lines.append(txt_row())
    lines.append(txt_row('Security pillars: identity and access control, authentication hardening, encryption, and OS hardening.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2), bTop(L3, R3))))
