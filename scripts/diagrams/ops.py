...
def ts_high_cpu():
    ...
    lines.append(R(sections(IV_L, IV_R, [PD1, PD2, PD3, PD4], ['IRQ load', 'cat /proc/interrupts', 'One CPU pinned', 'irqbalance', 'IRQ spread'])))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
...
def ts_vm_perf():
    ...
    lines.append(R(sections(IV_L, IV_R, [PD1, PD2, PD3, PD4], ['Mem slow', 'm → MCTL', '> 0 MB', 'Overcommit', 'Add RAM or reservation'])))
    lines.append(R(sections(IV_L, IV_R, [PD1, PD2, PD3, PD4], ['Disk slow', 'd → DAVG', '> 10ms', 'Array latency', 'See storage-latency'])))
    lines.append(R(sections(IV_L, IV_R, [PD1, PD2, PD3, PD4], ['Net drops', 'n → Drp/s', '> 0', 'NIC saturate', 'Increase vNIC or QoS'])))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
