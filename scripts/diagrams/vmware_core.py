...
@kb_diagram(
    'vmware-internals-cert-chain',
    'docs/virtualization/vmware/internals/certificate-chain/index.md',
    'VMware Certificate Chain — VMCA, STS, Machine SSL, Solution User Certs',
)
def vmware_internals_cert_chain():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'VMware Certificate Chain — VMCA and STS'))
    lines.append(txt_row())
    lines.append(txt_row('VMCA issues Machine SSL and Solution User certs; STS token-signing cert is'))
    lines.append(txt_row('separate and not issued by VMCA — expired STS causes total SSO failure.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'VMCA Certificate Hierarchy'), bMid(L2, R2, 'STS Token Signing'))))
    lines.append(R(merge(bMid(L1, R1, 'Root CA: VMCA self-signed'), bMid(L2, R2, 'Not issued by VMCA'))))
    lines.append(R(merge(bMid(L1, R1, 'Machine SSL: vCenter+ESXi+NSX'), bMid(L2, R2, 'Stored in VECS KeyStore'))))
    lines.append(R(merge(bMid(L1, R1, 'Solution user certs: SSO registered'), bMid(L2, R2, 'Expired STS: SSO failure'))))
    lines.append(R(merge(bMid(L1, R1, 'Subordinate CA mode: custom root'), bMid(L2, R2, 'Renewal: cert-manager CLI'))))
    lines.append(R(merge(bMid(L1, R1, 'Hybrid mode: 3rd-party Machine SSL'), bMid(L2, R2, 'STS key: separate renewal'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('VMCA auto-renews approaching-expiry certs; STS renewal is always manual.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Machine SSL Renewal'), bMid(L2, R2, 'Solution User Certs'))))
    lines.append(R(merge(bMid(L1, R1, 'cert-manager: renew --all'), bMid(L2, R2, 'vpxd: vCenter core service'))))
    lines.append(R(merge(bMid(L1, R1, 'VMCA auto-renews at 80% TTL'), bMid(L2, R2, 'vsphere-webclient: UI service'))))
    lines.append(R(merge(bMid(L1, R1, 'Subordinate: sign with custom CA'), bMid(L2, R2, 'machine: vpxd machine cert'))))
    lines.append(R(merge(bMid(L1, R1, 'ESXi: push new cert via vCenter'), bMid(L2, R2, 'Renew: cert-manager --solutionuser'))))
    lines.append(R(merge(bMid(L1, R1, 'Check: VECS-CLI list --store MACHINE'), bMid(L2, R2, 'Mismatch: service reg fails'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('VCSA appliance hosts VMCA, VECS, and STS; NTP sync required for cert validity;'))
    lines.append(txt_row('AD domain controllers for identity source validation.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('VMCA          = VMware Certificate Authority; built-in CA in VCSA'))
    lines.append(txt_row('VECS          = VMware Endpoint Certificate Store; in-memory cert store'))
    lines.append(txt_row('STS           = Security Token Service; issues SAML tokens for SSO'))
    lines.append(txt_row('Machine SSL   = SSL cert on HTTPS endpoints (vCenter, ESXi, NSX)'))
    lines.append(txt_row('Solution user = service accounts registered with SSO (vpxd, extensions)'))
    lines.append(txt_row('Subordinate CA= VMCA signed by external CA; chain includes corporate root'))
    lines.append(txt_row('Hybrid mode   = 3rd-party Machine SSL + VMCA solution user certs'))
    lines.append(txt_row('cert-manager  = /usr/lib/vmware-vmafd/bin/dir-cli; cert renewal CLI'))
    lines.append(txt_row('SAML token    = signed by STS cert; used across all SSO-aware services'))
    lines.append(txt_row('TTL           = cert validity period; VMCA default 2 years'))
    lines.append(txt_row('Lookup token  = SAML token for internal service calls; STS-issued'))
    lines.append(txt_row('vpxd          = core vCenter daemon; requires valid machine SSL + solution user'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-drs',
    'docs/virtualization/vmware/internals/drs-mechanics/index.md',
    'DRS Mechanics — Imbalance Score, Migration Bands, Predictive DRS',
)
def vmware_internals_drs():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'DRS Mechanics — Imbalance Score and Migration'))
    lines.append(txt_row())
    lines.append(txt_row('DRS calculates cluster imbalance every 5 min using per-host demand vs fair-share'))
    lines.append(txt_row('entitlement; migration priority bands 1-5 control which vMotions are triggered.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Imbalance Score Calculation'), bMid(L2, R2, 'Migration Priority Bands'))))
    lines.append(R(merge(bMid(L1, R1, 'Every 5 min: demand vs entitlement'), bMid(L2, R2, 'Band 1: benefit > threshold'))))
    lines.append(R(merge(bMid(L1, R1, 'Demand: actual resource usage'), bMid(L2, R2, 'Band 5: minimal benefit'))))
    lines.append(R(merge(bMid(L1, R1, 'Entitlement: fair share allocation'), bMid(L2, R2, 'Default: trigger band 1 only'))))
    lines.append(R(merge(bMid(L1, R1, 'Score 1-5: migration threshold'), bMid(L2, R2, 'Conservative: band 1 only'))))
    lines.append(R(merge(bMid(L1, R1, 'Memory: normalized consumed used'), bMid(L2, R2, 'Aggressive: bands 1-5'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Imbalance score is cluster-level aggregate; DRS moves VMs to reduce it below band.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Predictive DRS'), bMid(L2, R2, 'Reservation vs Entitlement'))))
    lines.append(R(merge(bMid(L1, R1, 'Requires Aria Operations plugin'), bMid(L2, R2, 'Reservation: guaranteed min'))))
    lines.append(R(merge(bMid(L1, R1, 'Predicts demand 60 min ahead'), bMid(L2, R2, 'Entitlement: DRS fair share'))))
    lines.append(R(merge(bMid(L1, R1, 'Pre-emptive vMotion before peak'), bMid(L2, R2, 'Shares: weight for excess'))))
    lines.append(R(merge(bMid(L1, R1, 'Threshold: user configurable'), bMid(L2, R2, 'Limit: hard cap on usage'))))
    lines.append(R(merge(bMid(L1, R1, 'Reduces peak-time imbalance'), bMid(L2, R2, 'DRS uses entitlement not limit'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ESXi hosts with physical CPU and RAM; DRS reads per-host performance counters'))
    lines.append(txt_row('via vmkernel; vCenter must be connected to all hosts for DRS to operate.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Imbalance score  = cluster-level metric; 1=balanced, 5=highly imbalanced'))
    lines.append(txt_row('Entitlement      = fair share of cluster resources allocated by DRS'))
    lines.append(txt_row('Demand           = actual resource consumption by VMs on a host'))
    lines.append(txt_row('Migration band   = 1-5 priority; DRS triggers when score > band threshold'))
    lines.append(txt_row('Normalized consumed= memory metric: active + ballooned + swapped'))
    lines.append(txt_row('Reservation      = guaranteed minimum resource; DRS must honour it'))
    lines.append(txt_row('Shares           = relative weight; used when cluster is in contention'))
    lines.append(txt_row('Limit            = hard cap; DRS does not use for imbalance calculation'))
    lines.append(txt_row('Predictive DRS   = Aria Operations plugin; sends demand forecast to DRS'))
    lines.append(txt_row('Conservative     = only most-needed vMotions (band 1 threshold)'))
    lines.append(txt_row('Aggressive       = all beneficial vMotions (bands 1-5 triggered)'))
    lines.append(txt_row('vMotion          = live migration of VM between hosts; zero downtime'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-ha',
    'docs/virtualization/vmware/internals/ha-deep-dive/index.md',
    'vSphere HA Deep Dive — Admission Control, Failover, APD/PDL',
)
def vmware_internals_ha():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'vSphere HA — Admission Control and Failover'))
    lines.append(txt_row())
    lines.append(txt_row('HA uses a master host elected from the cluster; master monitors heartbeats and'))
    lines.append(txt_row('restarts VMs on failure; admission control reserves capacity for failover.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Admission Control Policies'), bMid(L2, R2, 'Failover Behaviour'))))
    lines.append(R(merge(bMid(L1, R1, 'Percentage: % cluster reserved'), bMid(L2, R2, 'Restart priority: High/Med/Low'))))
    lines.append(R(merge(bMid(L1, R1, 'Slot-based: fixed CPU+mem slot'), bMid(L2, R2, 'Isolation: power-off or leave'))))
    lines.append(R(merge(bMid(L1, R1, 'Specify hosts: named failover'), bMid(L2, R2, 'APD: all-paths-down (transient)'))))
    lines.append(R(merge(bMid(L1, R1, 'Default: percentage (25%)'), bMid(L2, R2, 'PDL: permanent device loss'))))
    lines.append(R(merge(bMid(L1, R1, 'vSphere 7+: percentage preferred'), bMid(L2, R2, 'PDL response: aggressive needed'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Slot-based under-estimates capacity when VMs have large reservations.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'HA Master Election'), bMid(L2, R2, 'Heartbeat Datastores'))))
    lines.append(R(merge(bMid(L1, R1, 'Master: most datastores mounted'), bMid(L2, R2, '2 datastores per cluster'))))
    lines.append(R(merge(bMid(L1, R1, 'FDM agent on every ESXi host'), bMid(L2, R2, 'Path: /vmfs/volumes/.ha/'))))
    lines.append(R(merge(bMid(L1, R1, 'Heartbeat: 1s interval, 10s timeout'), bMid(L2, R2, 'Prefer: different storage paths'))))
    lines.append(R(merge(bMid(L1, R1, 'Isolation ping: cluster IP or GW'), bMid(L2, R2, 'If no ping + DS HB: isolated'))))
    lines.append(R(merge(bMid(L1, R1, 'Not isolated: master restarts VMs'), bMid(L2, R2, 'Isolated: response policy fires'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ESXi host hardware; shared storage for heartbeat datastores; management'))
    lines.append(txt_row('network for FDM agent communication; iLO/iDRAC for hardware status.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('FDM           = Fault Domain Manager; HA agent running on every ESXi host'))
    lines.append(txt_row('Master        = elected host; monitors slaves; restarts VMs on slave failure'))
    lines.append(txt_row('Admission ctrl= policy reserving cluster capacity for host failover'))
    lines.append(txt_row('Slot size     = largest CPU reservation + largest mem reservation in cluster'))
    lines.append(txt_row('Percentage    = % of cluster CPU+mem reserved; default 25%'))
    lines.append(txt_row('Restart prio  = High/Medium/Low/Disabled; determines VM restart order'))
    lines.append(txt_row('APD           = All-Paths-Down; storage lost but device still accessible'))
    lines.append(txt_row('PDL           = Permanent Device Loss; storage controller rejects all I/O'))
    lines.append(txt_row('Isolation response= what HA does when host loses management network'))
    lines.append(txt_row('Heartbeat DS  = shared datastore used for host-alive signalling'))
    lines.append(txt_row('Network partition= subset of hosts isolated from master but not from each other'))
    lines.append(txt_row('Proactive HA  = evacuates VMs from host before hardware failure (Aria required)'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-nsx-dp',
    'docs/virtualization/vmware/internals/nsx-data-plane/index.md',
    'NSX Data Plane — N-VDS, DFW, Geneve, TEPs, Edge Nodes',
)
def vmware_internals_nsx_dp():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'NSX Data Plane — N-VDS, DFW, Geneve, and Edge'))
    lines.append(txt_row())
    lines.append(txt_row('NSX data plane runs as ESXi kernel modules; DFW and Distributed Router operate'))
    lines.append(txt_row('per vNIC with zero bypass; overlay uses Geneve on UDP 6081 between TEPs.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'NSX Kernel Modules'), bMid(L2, R2, 'Overlay Network (Geneve)'))))
    lines.append(R(merge(bMid(L1, R1, 'N-VDS: replaces vSwitch in kernel'), bMid(L2, R2, 'Geneve: UDP port 6081'))))
    lines.append(R(merge(bMid(L1, R1, 'Distributed Router: kernel routing'), bMid(L2, R2, 'TEP: per-host tunnel endpoint'))))
    lines.append(R(merge(bMid(L1, R1, 'DFW: per-vNIC stateful firewall'), bMid(L2, R2, 'Segment ID: 24-bit VNI'))))
    lines.append(R(merge(bMid(L1, R1, 'Applied on vNIC: no bypass'), bMid(L2, R2, 'VTEP table: synced by NSX Mgr'))))
    lines.append(R(merge(bMid(L1, R1, 'Data plane survives NSX Mgr loss'), bMid(L2, R2, 'MTU: underlay >= 1600 bytes'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('E-W traffic: kernel path only — no L3 gateway hop needed for same-subnet VMs.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Edge Nodes (N-S Traffic)'), bMid(L2, R2, 'Fast Path Forwarding'))))
    lines.append(R(merge(bMid(L1, R1, 'N-S traffic exits via Edge nodes'), bMid(L2, R2, 'E-W: kernel, no vCenter needed'))))
    lines.append(R(merge(bMid(L1, R1, 'Edge VM or bare-metal form factor'), bMid(L2, R2, 'DFW: stateful 5-tuple match'))))
    lines.append(R(merge(bMid(L1, R1, 'SR lives on Edge: NAT, LB, VPN'), bMid(L2, R2, 'IDPS and NAT: also in kernel'))))
    lines.append(R(merge(bMid(L1, R1, 'T0 uplinks: BGP/OSPF to TOR'), bMid(L2, R2, 'Control plane: NSX Mgr only'))))
    lines.append(R(merge(bMid(L1, R1, 'ECMP: up to 8 paths N-S LB'), bMid(L2, R2, 'Forwarding table: TEP-distributed'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ESXi host pNICs carry Geneve-encapsulated overlay traffic on underlay VLAN;'))
    lines.append(txt_row('Edge nodes require dedicated pNIC or DPDK uplinks for N-S forwarding.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('N-VDS        = NSX Virtual Distributed Switch; ESXi kernel data-plane'))
    lines.append(txt_row('DFW          = Distributed Firewall; kernel-resident per-vNIC stateful FW'))
    lines.append(txt_row('DR           = Distributed Router; per-host kernel L3 forwarding'))
    lines.append(txt_row('SR           = Services Router; lives on Edge nodes; NAT/LB/VPN'))
    lines.append(txt_row('TEP          = Tunnel End Point; per-host IP for Geneve encapsulation'))
    lines.append(txt_row('Geneve       = Generic Network Virtualization Encapsulation; UDP 6081'))
    lines.append(txt_row('VNI          = Virtual Network Identifier; 24-bit segment ID'))
    lines.append(txt_row('VTEP table   = mapping of segment VNI to remote TEP IPs'))
    lines.append(txt_row('TOR          = Top-of-Rack switch; Edge T0 peers via BGP/OSPF'))
    lines.append(txt_row('ECMP         = Equal-Cost Multi-Path; up to 8 uplinks for N-S load balancing'))
    lines.append(txt_row('E-W traffic  = east-west; VM-to-VM same site; kernel path only'))
    lines.append(txt_row('N-S traffic  = north-south; VM to external network; via Edge node'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-vcha',
    'docs/virtualization/vmware/internals/vcha-internals/index.md',
    'vCenter HA — Active/Passive/Witness, Replication, Failover',
)
def vmware_internals_vcha():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'vCenter HA (VCHA) — Three-Node Topology'))
    lines.append(txt_row())
    lines.append(txt_row('VCHA deploys three vCenter instances — active, passive, and witness — with'))
    lines.append(txt_row('Postgres streaming replication; witness prevents split-brain; RTO ~4 min.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'VCHA Topology'), bMid(L2, R2, 'Failover Behaviour'))))
    lines.append(R(merge(bMid(L1, R1, 'Active: primary vCenter (R/W)'), bMid(L2, R2, 'Witness + passive detect loss'))))
    lines.append(R(merge(bMid(L1, R1, 'Passive: standby; Postgres sync'), bMid(L2, R2, 'Quorum: 2 of 3 nodes agree'))))
    lines.append(R(merge(bMid(L1, R1, 'Witness: tie-breaker (1 vCPU)'), bMid(L2, R2, 'Failover time: ~4 min RTO'))))
    lines.append(R(merge(bMid(L1, R1, 'HA network: private 1 GbE'), bMid(L2, R2, 'Split-brain: witness prevents'))))
    lines.append(R(merge(bMid(L1, R1, 'Replication: Postgres streaming'), bMid(L2, R2, 'Manual failover: VCHA admin UI'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('VCHA is HA only — it does not replace VCSA file-based backup for DR.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Network Requirements'), bMid(L2, R2, 'Maintenance and Backup'))))
    lines.append(R(merge(bMid(L1, R1, 'HA network: /24 dedicated VLAN'), bMid(L2, R2, 'VCHA != backup: HA only'))))
    lines.append(R(merge(bMid(L1, R1, 'Mgmt IP: all 3 on same subnet'), bMid(L2, R2, 'File-based backup: daily SFTP'))))
    lines.append(R(merge(bMid(L1, R1, 'Witness: can be on WAN (low BW)'), bMid(L2, R2, 'Restore: backup + new VCSA'))))
    lines.append(R(merge(bMid(L1, R1, 'Maintenance: disable VCHA first'), bMid(L2, R2, 'Rebuild VCHA after restore'))))
    lines.append(R(merge(bMid(L1, R1, 'All 3 nodes: must match build'), bMid(L2, R2, 'Patch: active -> passive -> wit'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Three VCSA VMs on separate hosts/clusters for fault isolation; HA network'))
    lines.append(txt_row('on isolated VLAN; witness can be on remote ESXi or vCenter-managed host.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('VCHA         = vCenter High Availability; 3-node active/passive/witness'))
    lines.append(txt_row('Active node  = runs vCenter services and responds to all requests'))
    lines.append(txt_row('Passive node = warm standby; Postgres replica; takes over on failover'))
    lines.append(txt_row('Witness node = tie-breaker only; no vCenter services; 1 vCPU/1 GB'))
    lines.append(txt_row('Postgres     = vCenter internal database; streamed from active to passive'))
    lines.append(txt_row('RTO          = Recovery Time Objective; ~4 min for VCHA automatic failover'))
    lines.append(txt_row('Split-brain  = both active and passive believe they are primary; prevented by witness'))
    lines.append(txt_row('Quorum       = 2 of 3 VCHA nodes must agree before promoting passive'))
    lines.append(txt_row('HA network   = private /24 VLAN for replication; not management network'))
    lines.append(txt_row('File-based backup= VCSA backup via SFTP/NFS/SMB; separate from VCHA'))
    lines.append(txt_row('Build match  = all 3 VCHA nodes must run identical vCenter build number'))
    lines.append(txt_row('Maintenance  = disable VCHA before patching; re-enable after all 3 updated'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-vlcm',
    'docs/virtualization/vmware/internals/vlcm-mechanics/index.md',
    'vLCM Mechanics — Image-Based Management, Drift Detection, Rolling Remediation',
)
def vmware_internals_vlcm():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'vLCM — Image-Based Lifecycle Management'))
    lines.append(txt_row())
    lines.append(txt_row('vLCM replaces VUM baselines with image-based management; a cluster image'))
    lines.append(txt_row('defines the complete ESXi software bill of materials; DRS coordinates patching.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Cluster Image'), bMid(L2, R2, 'Drift Detection and Compliance'))))
    lines.append(R(merge(bMid(L1, R1, 'Single desired-state definition'), bMid(L2, R2, 'Per-host compliance scan'))))
    lines.append(R(merge(bMid(L1, R1, 'Replaces VUM host baselines'), bMid(L2, R2, 'Diff: running vs desired image'))))
    lines.append(R(merge(bMid(L1, R1, 'Components: ESXi base + add-ons'), bMid(L2, R2, 'Non-compliant: blocked motion'))))
    lines.append(R(merge(bMid(L1, R1, 'Firmware: via HSM plugin'), bMid(L2, R2, 'Pre-check: storage + HA check'))))
    lines.append(R(merge(bMid(L1, R1, 'Declare state; vLCM enforces'), bMid(L2, R2, 'Compliance report: per host'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Rolling remediation evacuates one host at a time; VMs stay running via vMotion.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Rolling Remediation'), bMid(L2, R2, 'Image Depot Sources'))))
    lines.append(R(merge(bMid(L1, R1, 'DRS evacuates host first'), bMid(L2, R2, 'VMUG Advantage / MyVMware'))))
    lines.append(R(merge(bMid(L1, R1, 'Enter maintenance mode'), bMid(L2, R2, 'Vendor depot: OEM images'))))
    lines.append(R(merge(bMid(L1, R1, 'Apply image; reboot; exit MM'), bMid(L2, R2, 'Offline bundle: air-gap import'))))
    lines.append(R(merge(bMid(L1, R1, 'Next host: serial or parallel'), bMid(L2, R2, 'Security patch: depot sync'))))
    lines.append(R(merge(bMid(L1, R1, 'Zero-downtime: VMs migrate'), bMid(L2, R2, 'Custom VIB: manual import'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ESXi host hardware; HSM requires vendor management agent (iDRAC/iLO);'))
    lines.append(txt_row('depot sync requires outbound HTTPS to VMware depot or internal UMDS.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('vLCM         = vSphere Lifecycle Manager; image-based cluster management'))
    lines.append(txt_row('VUM          = vSphere Update Manager; superseded by vLCM in vSphere 7'))
    lines.append(txt_row('Cluster image= desired state: ESXi base + add-ons + firmware components'))
    lines.append(txt_row('HSM          = Hardware Support Manager; OEM plugin for firmware lifecycle'))
    lines.append(txt_row('Compliance   = per-host diff of running state vs cluster image'))
    lines.append(txt_row('Depot        = online or offline source of ESXi patches and add-ons'))
    lines.append(txt_row('UMDS         = Update Manager Download Service; local depot mirror'))
    lines.append(txt_row('Remediation  = vLCM applying image to non-compliant hosts; rolling'))
    lines.append(txt_row('Maintenance mode= host state where no VMs run; required for update'))
    lines.append(txt_row('Rolling      = one host at a time; DRS evacuates before each maintenance'))
    lines.append(txt_row('VIB          = VMware Installation Bundle; ESXi software package'))
    lines.append(txt_row('BOM          = Bill of Materials; complete component list for cluster image'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-vsan-health',
    'docs/virtualization/vmware/internals/vsan-cluster-health/index.md',
    'vSAN Cluster Health — Component State Machine, CLOM, Skyline Health',
)
def vmware_internals_vsan_health():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'vSAN Cluster Health — Component State Machine'))
    lines.append(txt_row())
    lines.append(txt_row('vSAN tracks component health via ACTIVE->ABSENT->DEGRADED->REBUILDING->ACTIVE;'))
    lines.append(txt_row('CLOM schedules rebuilds; resync throttle and proactive rebalance are tunable.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Component State Machine'), bMid(L2, R2, 'CLOM Rebuild Scheduling'))))
    lines.append(R(merge(bMid(L1, R1, 'ACTIVE: healthy + accessible'), bMid(L2, R2, 'CLOM: Cluster Level Object Mgr'))))
    lines.append(R(merge(bMid(L1, R1, 'ABSENT: node offline, 60 min grace'), bMid(L2, R2, 'Monitors object components'))))
    lines.append(R(merge(bMid(L1, R1, 'DEGRADED: data at risk (< FTT)'), bMid(L2, R2, 'Schedules rebuild on healthy disk'))))
    lines.append(R(merge(bMid(L1, R1, 'REBUILDING: resync in progress'), bMid(L2, R2, 'Resync throttle: IOPS limit'))))
    lines.append(R(merge(bMid(L1, R1, 'ABSENT -> DEGRADED at 60 min'), bMid(L2, R2, 'Default throttle: 0 (unlimited)'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Absent grace period: 60 min — host returning within 60 min skips rebuild.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Skyline Health Checks'), bMid(L2, R2, 'Stretched Cluster'))))
    lines.append(R(merge(bMid(L1, R1, 'vSAN -> Skyline Health in VCSA'), bMid(L2, R2, 'Witness host: 3rd site'))))
    lines.append(R(merge(bMid(L1, R1, 'Categories: Network/HW/Perf'), bMid(L2, R2, 'Preferred/secondary site'))))
    lines.append(R(merge(bMid(L1, R1, 'HCL checks: daily automated'), bMid(L2, R2, 'FTT=1 spans both sites RAID-1'))))
    lines.append(R(merge(bMid(L1, R1, 'Proactive rebalance: space var'), bMid(L2, R2, 'RPO=0: synchronous writes'))))
    lines.append(R(merge(bMid(L1, R1, 'Auto-remediate: disk re-add'), bMid(L2, R2, 'Witness quorum: site failure'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ESXi hosts with NVMe/SSD cache + capacity disks; vSAN VMkernel adapter on'))
    lines.append(txt_row('dedicated NIC; 10 GbE minimum for production; 25 GbE recommended.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('CLOM         = Cluster Level Object Manager; rebuild and placement engine'))
    lines.append(txt_row('Component    = smallest vSAN storage unit; replica or witness of an object'))
    lines.append(txt_row('DEGRADED     = component below FTT; rebuild scheduled immediately'))
    lines.append(txt_row('ABSENT       = component missing; 60 min grace before DEGRADED state'))
    lines.append(txt_row('FTT          = Failures to Tolerate; storage policy value (1, 2, or 3)'))
    lines.append(txt_row('Resync throttle= IOPS limit on rebuild I/O; 0=unlimited; set non-zero during peak'))
    lines.append(txt_row('Proactive rebalance= moves data when space usage diverges across nodes'))
    lines.append(txt_row('HCL          = Hardware Compatibility List; Skyline checks daily against VMware'))
    lines.append(txt_row('Witness      = tie-breaker component; quorum only; no data stored'))
    lines.append(txt_row('Stretched cluster= vSAN across 2 sites + witness; RPO=0 synchronous'))
    lines.append(txt_row('Skyline Health= vSAN built-in health framework; 200+ checks; in VCSA UI'))
    lines.append(txt_row('Space variance= difference in disk usage % across nodes; trigger for rebalance'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'vmware-internals-vsphere-net',
    'docs/virtualization/vmware/internals/vsphere-networking-internals/index.md',
    'vSphere Networking Internals — VDS, NIOC, Teaming, VMkernel',
)
def vmware_internals_vsphere_net():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'vSphere Networking — VDS, NIOC, and VMkernel'))
    lines.append(txt_row())
    lines.append(txt_row('VDS separates control plane (vCenter) from data plane (ESXi kernel module);'))
    lines.append(txt_row('NIOC traffic pools, teaming policy, and VMkernel adapters control all traffic.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'VDS Architecture'), bMid(L2, R2, 'NIOC Traffic Pools'))))
    lines.append(R(merge(bMid(L1, R1, 'Control plane: vCenter config'), bMid(L2, R2, 'NIOC version 3: vSphere 6+'))))
    lines.append(R(merge(bMid(L1, R1, 'Data plane: ESXi kernel module'), bMid(L2, R2, 'Traffic types: VM/vMotion/mgmt'))))
    lines.append(R(merge(bMid(L1, R1, 'Uplink: pNIC -> uplink portgroup'), bMid(L2, R2, 'vSAN/FT/NFS separate pools'))))
    lines.append(R(merge(bMid(L1, R1, 'dvPort: VM vNIC connects here'), bMid(L2, R2, 'Shares: relative weight'))))
    lines.append(R(merge(bMid(L1, R1, 'vCenter failure: data plane stays'), bMid(L2, R2, 'Reservation/Limit per type'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('VDS config state cached on ESXi; hosts retain networking if vCenter goes down.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Teaming Policies'), bMid(L2, R2, 'VMkernel Adapters'))))
    lines.append(R(merge(bMid(L1, R1, 'Active/standby: manual order'), bMid(L2, R2, 'vmk0: management (required)'))))
    lines.append(R(merge(bMid(L1, R1, 'Route by originating virt port'), bMid(L2, R2, 'vMotion: separate vmk needed'))))
    lines.append(R(merge(bMid(L1, R1, 'Route by source MAC hash'), bMid(L2, R2, 'vSAN: dedicated vmk per node'))))
    lines.append(R(merge(bMid(L1, R1, 'Route by IP hash (EtherChannel)'), bMid(L2, R2, 'NFC: storage traffic vmk'))))
    lines.append(R(merge(bMid(L1, R1, 'Failback: return to active pNIC'), bMid(L2, R2, 'Isolation: vmk per traffic type'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Physical NICs (pNICs) on ESXi hosts; TOR switch for inter-host traffic;'))
    lines.append(txt_row('LACP or EtherChannel required for IP hash teaming policy.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('VDS          = vSphere Distributed Switch; managed from vCenter'))
    lines.append(txt_row('VSS          = vSphere Standard Switch; per-host; no vCenter dependency'))
    lines.append(txt_row('dvPort       = distributed virtual port; persistent VM network connection'))
    lines.append(txt_row('Uplink port  = VDS port bound to physical NIC (pNIC)'))
    lines.append(txt_row('NIOC         = Network I/O Control; bandwidth scheduler on shared pNICs'))
    lines.append(txt_row('Traffic pool = NIOC classification: VM, vMotion, management, vSAN, FT, NFC'))
    lines.append(txt_row('Shares       = relative bandwidth weight in contention; higher = more'))
    lines.append(txt_row('Reservation  = guaranteed minimum bandwidth per traffic pool'))
    lines.append(txt_row('Limit        = maximum bandwidth cap per traffic pool'))
    lines.append(txt_row('VMkernel     = ESXi kernel adapter for host-initiated IP traffic'))
    lines.append(txt_row('vMotion vmk  = dedicated VMkernel for live migration traffic'))
    lines.append(txt_row('IP hash      = teaming policy requiring EtherChannel/LAG on the switch'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
