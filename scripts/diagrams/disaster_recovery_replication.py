...
def dr_recoverpoint():
    ...
    lines.append(txt_row())
    lines.append(txt_row('  Protected Site ──splitter intercepts writes──► RPA Cluster ──journal replication──► Recovery Site RPA'))
    lines.append(txt_row())
    ...
    lines.append(txt_row())
    lines.append(txt_row('  Physical: RPAs are VMs or physical appliances; journal vols on shared datastore; replication over IP'))
    lines.append(txt_row())
...
def dr_recoverpoint_architecture():
    ...
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('  RPA cluster      = 2–8 RecoverPoint Appliance VMs per site; active/active; no single point of failure'))
    lines.append(txt_row('  ESXi splitter    = Kernel module installed on each ESXi host; intercepts VM disk writes non-disruptively'))
    lines.append(txt_row('  Local copy       = Protection within same site (same cluster); journal on same or separate datastore'))
    ...
    lines.append(txt_row('  Active/active    = Both RPAs in cluster handle I/O simultaneously; failover is automatic on RPA loss'))
    ...
    lines.append(txt_row('  Bubble network   = Isolated portgroup for test failover VMs; no production traffic reaches test copies'))
    lines.append(txt_row())
...
def dr_recoverpoint_arch_how_it_works():
    ...
    lines.append(txt_row('  Bookmark            = Named timestamp in journal; enables recovery to a known-good application state'))
...
def dr_recoverpoint_arch_design():
    ...
    lines.append(R(merge(bBot(B1_L, B1_R), bBot(B2_L, B2_R), bBot(B3_L, B3_R))))
    lines.append(txt_row())
    lines.append(txt_row('  Physical: RPA VMs pinned to dedicated ESXi hosts; journal datastores on separate LUNs from production'))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
...
def dr_recoverpoint_arch_integrations():
    ...
    lines.append(R(merge(bBot(B1_L, B1_R), bBot(B2_L, B2_R))))
    lines.append(txt_row())
    lines.append(txt_row('  Physical: SRA plugin installed on SRM server; array splitter licence on PowerMax; VPLEX needs RP licence'))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('  SRA              = Storage Replication Adapter; plugin on SRM server; translates SRM calls to RP API'))
    lines.append(txt_row('  RP4VM plugin     = vCenter plugin; provides CG management, failover, and image access directly in UI'))
    lines.append(txt_row('  Array splitter   = Intercepts writes inside the storage array firmware; higher performance than ESXi'))
...
def dr_recoverpoint_ops_backup():
    ...
    lines.append(R(merge(bBot(B1_L, B1_R), bBot(B2_L, B2_R))))
    lines.append(txt_row())
    lines.append(txt_row('  Physical: failover powers on replica VMs at DR site; requires pre-configured VM networks and IP pools'))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
...
def dr_recoverpoint_ops_procedures():
    ...
    lines.append(R(merge(bBot(B1_L, B1_R), bBot(B2_L, B2_R), bBot(B3_L, B3_R))))
    lines.append(txt_row())
    lines.append(txt_row('  Physical: test copy VMs on bubble portgroup (no uplinks); failover requires DR network pre-configured'))
    lines.append(txt_row())
    lines.append(txt_row('  Key terms:'))
    lines.append(txt_row())
...
def _dr_recoverpoint_security():
    ...
    lines.append(R(bMid(3, 99, 'Encryption: AES-256 WAN compression+encryption; data at rest on journal volumes unencrypted by defau')))
...
def _dr_srdf_a_arch_design():
    ...
    lines.append(R(bMid(3, 99, 'Encryption enabled on all channels: SRDF encryption at the FA/RF port level; Unisphere HTTPS; SE service T')))
...
def _dr_srdf_a_ops_install():
    ...
    lines.append(R(bMid(3, 99, '  Auth: Symmetrix/PowerMax admin credentials; Solutions Enabler (SYMAPI); role-based Unisphere')))
    lines.append(R(bMid(3, 99, '  Storage: Two PowerMax arrays (production + DR site) · FC/FCIP SRDF link (dedicated bandwidth) · R')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
...
def _dr_srdf_a_security():
    ...
    lines.append(R(bMid(3, 99, 'Authentication: Symmetrix/PowerMax admin credentials; Solutions Enabler (SYMAPI); role-based Unisphere')))
    lines.append(R(bMid(3, 99, 'Encryption: SRDF encryption at the FA/RF port level; Unisphere HTTPS; SE service TLS')))
...
def _dr_srdf_s_overview():
    ...
    lines.append(R(bMid(3, 99, 'Management: Dark fiber FC (< 5 ms RTT) · Auth: Symmetrix admin credentials; Solutions Enabler; Un')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
...
def _dr_srdf_s_arch_design():
    ...
    lines.append(R(bMid(3, 99, 'RTO target drives recovery tier: instant, warm standby, or cold restore')))
    lines.append(R(bMid(3, 99, 'Dedicated backup network VLAN — no shared production traffic')))
    lines.append(R(bMid(3, 99, 'Encryption enabled on all channels: Data identical to R1 at R2; FA port encryption optional; Unisphere TLS')))
    lines.append(R(bMid(3, 99, 'Service accounts: minimum privilege; rotate credentials quarterly')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure:'))
...
def _dr_srdf_s_ops_install():
    ...
    lines.append(R(bMid(3, 99, '  Auth: Symmetrix admin credentials; Solutions Enabler; Unisphere role-based access')))
    lines.append(R(bMid(3, 99, '  Storage: Two PowerMax arrays · Dark fiber / DWDM FC link · Low-latency network (< 200 km) · RF di')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
...
def _dr_srm_overview():
    ...
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure:'))
    lines.append(txt_row('Two vCenter instances (protected + recovery site) · SRA installed on SRM server · Array replication link'))
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
...
def _dr_srm_arch_design():
    ...
    lines.append(R(bMid(3, 99, 'RTO target drives recovery tier: instant, warm standby, or cold restore')))
    lines.append(R(bMid(3, 99, 'Dedicated backup network VLAN — no shared production traffic')))
    lines.append(R(bMid(3, 99, 'Encryption enabled on all channels: SRM management TLS; replication encryption controlled by array/SRA lay')))
    lines.append(R(bMid(3, 99, 'Service accounts: minimum privilege; rotate credentials quarterly')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure:'))
...
def _dr_srm_ops_install():
    ...
    lines.append(R(bMid(3, 99, '  Auth: vCenter SSO / AD integration; SRM admin role; site-pairing certificate exchange')))
    lines.append(R(bMid(3, 99, '  Storage: Two vCenter instances (protected + recovery site) · SRA installed on SRM server · Array ')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
...
def _dr_superna_arch_how():
    ...
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(txt_row('  Changed data forwarded to the Superna Eyeglass engine — compression and encryption applied in transit'))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
    lines.append(R(bTop(3, 99)))
...
def _dr_superna_arch_design():
    ...
    lines.append(R(bMid(3, 99, 'RTO target drives recovery tier: instant, warm standby, or cold restore')))
    lines.append(R(bMid(3, 99, 'Dedicated backup network VLAN — no shared production traffic')))
    lines.append(R(bMid(3, 99, 'Encryption enabled on all channels: HTTPS/TLS for all management; SyncIQ data replication encryption (AES-')))
    lines.append(R(bMid(3, 99, 'Service accounts: minimum privilege; rotate credentials quarterly')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure:'))
...
def _dr_superna_arch_integrations():
    ...
    lines.append(R(bMid(3, 99, 'Monitoring: SNMP traps / syslog / REST API to ITSM and alerting systems')))
    lines.append(R(bMid(3, 99, 'Encryption: HTTPS/TLS for all management; SyncIQ data replication encryption (AES-256 in transit)')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([26, 51, 76])))
    lines.append(txt_row())
...
def _dr_superna_ops_install():
    ...
    lines.append(R(bMid(3, 99, '  Auth: Eyeglass admin roles; PowerScale admin credentials; AD integration for DFS-N management')))
    lines.append(R(bMid(3, 99, '  Storage: ESXi VM (Eyeglass appliance) · PowerScale cluster pair (production + DR) · SyncIQ replic')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
...
def _dr_superna_security():
    ...
    lines.append(R(bMid(3, 99, 'Authentication: Eyeglass admin roles; PowerScale admin credentials; AD integration for DFS-N management')))
    lines.append(R(bMid(3, 99, 'Encryption: HTTPS/TLS for all management; SyncIQ data replication encryption (AES-256 in transit)')))
...
def _dr_superna_sec_hardening():
    ...
    lines.append(R(bMid(3, 99, 'Network Hardening')))
    lines.append(R(bMid(3, 99, '  [ ] Separate backup VLAN — no direct production host access to repo')))
    lines.append(R(bMid(3, 99, '  [ ] Firewall: allow only 443 (Eyeglass web UI) · 8080 (REST API) · 8116 (Isilon/PowerScale mgmt)')))
    lines.append(R(bMid(3, 99, '  [ ] Disable unused ports and protocols on management interface')))
    lines.append(R(bMid(3, 99, '  [ ] Immutable repository: enable WORM or object lock on backup target')))
    lines.append(R(bMid(3, 99, '  [ ] Encryption in transit: disable TLS 1.0/1.1; enforce TLS 1.2+')))
    lines.append(R(bBot(3, 99)))
...
def _dr_veeam_arch_design():
    ...
    lines.append(R(bMid(3, 99, 'RTO target drives recovery tier: instant, warm standby, or cold restore')))
    lines.append(R(bMid(3, 99, 'Dedicated backup network VLAN — no shared production traffic')))
    lines.append(R(bMid(3, 99, 'Encryption enabled on all channels: AES-256 backup encryption (key stored in Veeam config DB); TLS on all ')))
    lines.append(R(bMid(3, 99, 'Service accounts: minimum privilege; rotate credentials quarterly')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure:'))
...
def _dr_veeam_arch_integrations():
    ...
    lines.append(R(bMid(3, 99, 'Monitoring: SNMP traps / syslog / REST API to ITSM and alerting systems')))
    lines.append(R(bMid(3, 99, 'Encryption: AES-256 backup encryption (key stored in Veeam config DB); TLS on all management; WORM r')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([26, 51, 76])))
    lines.append(txt_row())
...
def _dr_veeam_ops_install():
    ...
    lines.append(R(bMid(3, 99, '  Auth: Windows/AD auth for Veeam console; service account with vSphere admin; repo credentials')))
    lines.append(R(bMid(3, 99, '  Storage: Windows Server (Backup Server) · Proxy VMs on ESXi · Backup storage (NAS/SAN) · Manageme')))
    lines.append(R(bBot(3, 99)))
    lines.append(txt_row())
    lines.append(R(arrow([51])))
    lines.append(txt_row())
...
def _dr_veeam_security():
    ...
    lines.append(R(bMid(3, 99, 'Authentication: Windows/AD auth for Veeam console; service account with vSphere admin; repo credentials')))
    lines.append(R(bMid(3, 99, 'Encryption: AES-256 backup encryption (key stored in Veeam config DB); TLS on all management; WORM r')))
    lines.append(R(bMid(3, 99, 'Network: management VLAN separated; 443 (vCenter) management port')))
    lines.append(R(bMid(3, 99, 'Audit: all admin actions logged; log retention minimum 1 year')))
    lines.append(R(bBot(3, 99)))
