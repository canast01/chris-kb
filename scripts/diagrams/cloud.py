...
@kb_diagram('aws-trouble-common', 'docs/cloud/aws/troubleshooting/common-issues/index.md', 'AWS common issues — EC2, networking, IAM, and storage troubleshooting patterns')
def _aws_trouble_common():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'AWS Troubleshooting — Common Issues'))
    lines.append(txt_row())
    lines.append(txt_row('Common AWS issues and their resolution patterns across EC2, networking, IAM, and storage.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'EC2 Issues'), bMid(L2, R2, 'Networking Issues'))))
    lines.append(R(merge(bMid(L1, R1, 'Cannot connect SSH: check SG port 22'), bMid(L2, R2, 'No internet: check IGW + route'))))
    lines.append(R(merge(bMid(L1, R1, 'Instance unreachable: status checks'), bMid(L2, R2, 'Private cannot reach internet: NAT'))))
    lines.append(R(merge(bMid(L1, R1, 'High CPU: CloudWatch CPU metric'), bMid(L2, R2, 'Cross-VPC: peering + routes set'))))
    lines.append(R(merge(bMid(L1, R1, 'Disk full: extend EBS or clean logs'), bMid(L2, R2, 'DNS not resolving: check VPC attr'))))
    lines.append(R(merge(bMid(L1, R1, 'Out of memory: check processes'), bMid(L2, R2, 'ELB 502: target unhealthy check'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Check SG, NACL, route table, and IGW in order for networking issues.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'IAM Issues'), bMid(L2, R2, 'Storage Issues'))))
    lines.append(R(merge(bMid(L1, R1, 'AccessDenied: check CT + policy sim'), bMid(L2, R2, 'S3 403: check bucket policy + BPA'))))
    lines.append(R(merge(bMid(L1, R1, 'SCP blocking: check org SCPs'), bMid(L2, R2, 'EBS perf: IOPS exhausted check CW'))))
    lines.append(R(merge(bMid(L1, R1, 'Role assume fail: trust policy check'), bMid(L2, R2, 'RDS slow: Performance Insights'))))
    lines.append(R(merge(bMid(L1, R1, 'Credential expired: re-login SSO'), bMid(L2, R2, 'EFS mount fail: check SG port 2049'))))
    lines.append(R(merge(bMid(L1, R1, 'MFA required: check condition key'), bMid(L2, R2, 'Snapshot failed: IAM permission'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('AWS EC2 host hardware · Regional network fabric · CloudTrail audit infrastructure'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Status check    = EC2 system check (AWS infra) or instance check (OS); both must pass'))
    lines.append(txt_row('AccessDenied    = IAM explicit deny, missing allow, SCP block, or boundary restriction'))
    lines.append(txt_row('Policy simulator= IAM tool testing policy evaluation for a given principal and action'))
    lines.append(txt_row('BPA             = Block Public Access; S3 setting preventing public access'))
    lines.append(txt_row('Trust policy    = IAM role policy allowing specific principals to call AssumeRole'))
    lines.append(txt_row('SCP blocking    = Service Control Policy at org level blocking the API action'))
    lines.append(txt_row('ELB 502         = Bad Gateway; target returned invalid response; check app health'))
    lines.append(txt_row('VPC DNS attrs   = enableDnsHostnames + enableDnsSupport; must be on for DNS to work'))
    lines.append(txt_row('IOPS exhausted  = Volume throughput limit hit; upgrade to gp3 with higher IOPS'))
    lines.append(txt_row('Performance Insights= RDS tool showing wait events and top SQL for slow queries'))
    lines.append(txt_row('NFS port 2049   = EFS mount requires TCP 2049 open in mount target security group'))
    lines.append(txt_row('Credential expired= SSO tokens expire; run aws sso login to refresh'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('aws-trouble-diag', 'docs/cloud/aws/troubleshooting/diagnostics/index.md', 'AWS diagnostics — toolset for investigating infrastructure and application issues')
def _aws_trouble_diag():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'AWS Diagnostics — Investigation Toolset'))
    lines.append(txt_row())
    lines.append(txt_row('Diagnostic tools and investigation procedures for AWS infrastructure and application issues.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Log Sources'), bMid(L2, R2, 'Metrics & Traces'))))
    lines.append(R(merge(bMid(L1, R1, 'CloudTrail: who did what and when'), bMid(L2, R2, 'CloudWatch: CPU/mem/disk/net'))))
    lines.append(R(merge(bMid(L1, R1, 'VPC Flow Logs: network traffic'), bMid(L2, R2, 'Enhanced Monitoring: RDS OS'))))
    lines.append(R(merge(bMid(L1, R1, 'CloudWatch Logs: app/system logs'), bMid(L2, R2, 'X-Ray: distributed service map'))))
    lines.append(R(merge(bMid(L1, R1, 'ELB access logs: per-request data'), bMid(L2, R2, 'Performance Insights: RDS SQL'))))
    lines.append(R(merge(bMid(L1, R1, 'S3 server access logs: object ops'), bMid(L2, R2, 'CloudWatch Logs Insights query'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('CloudTrail is the first stop for any "who changed what" investigation.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Diagnostic CLI Commands'), bMid(L2, R2, 'Network Diagnostics'))))
    lines.append(R(merge(bMid(L1, R1, 'aws cloudtrail lookup-events'), bMid(L2, R2, 'VPC Reachability Analyzer'))))
    lines.append(R(merge(bMid(L1, R1, 'aws logs filter-log-events'), bMid(L2, R2, 'Network Access Analyzer'))))
    lines.append(R(merge(bMid(L1, R1, 'aws ec2 describe-instance-status'), bMid(L2, R2, 'Flow Logs: find REJECT entries'))))
    lines.append(R(merge(bMid(L1, R1, 'aws iam simulate-principal-policy'), bMid(L2, R2, 'Traceroute via SSM Session Mgr'))))
    lines.append(R(merge(bMid(L1, R1, 'aws support create-case'), bMid(L2, R2, 'ELB access logs: Athena query'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('CloudTrail storage infrastructure · CloudWatch Logs backend · X-Ray sampling plane'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('CloudTrail lookup= API returning recent management events for an account and region'))
    lines.append(txt_row('filter-log-events= CLI command searching CloudWatch Logs for a pattern in a time range'))
    lines.append(txt_row('Reachability Analyzer= VPC tool that traces the network path between two resources'))
    lines.append(txt_row('Network Access Analyzer= Identifies unintended network access paths to resources'))
    lines.append(txt_row('simulate-principal-policy= IAM CLI command checking if a principal can perform an action'))
    lines.append(txt_row('X-Ray service map= Visual graph of microservices and latency between them'))
    lines.append(txt_row('ELB access log  = Per-request record with response time, backend, status, client IP'))
    lines.append(txt_row('Flow Log REJECT = Indicates SG or NACL denied the packet; starting point for firewall debug'))
    lines.append(txt_row('SSM Session Manager= Browser/CLI shell to instance without SSH; useful for diagnostics'))
    lines.append(txt_row('Athena on logs  = SQL queries over ELB or S3 access logs stored in S3'))
    lines.append(txt_row('Logs Insights   = CloudWatch Logs query engine; supports time-series and aggregation'))
    lines.append(txt_row('aws support     = Open AWS support case via CLI with describe-trusted-advisor-checks'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('aws-trouble-esc', 'docs/cloud/aws/troubleshooting/escalation/index.md', 'AWS escalation — AWS support tiers, Trusted Advisor, and escalation procedures')
def _aws_trouble_esc():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'AWS Escalation — Support & Trusted Advisor'))
    lines.append(txt_row())
    lines.append(txt_row('AWS support tiers, escalation paths, and Trusted Advisor checks for proactive guidance.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Support Plans'), bMid(L2, R2, 'Escalation Path'))))
    lines.append(R(merge(bMid(L1, R1, 'Developer: 12h business response'), bMid(L2, R2, '1. Internal runbook attempt'))))
    lines.append(R(merge(bMid(L1, R1, 'Business: 1h production response'), bMid(L2, R2, '2. AWS console support case'))))
    lines.append(R(merge(bMid(L1, R1, 'Enterprise On-Ramp: 30min critical'), bMid(L2, R2, '3. Severity: Urgent/High/Normal'))))
    lines.append(R(merge(bMid(L1, R1, 'Enterprise: 15min + TAM + Concierge'), bMid(L2, R2, '4. TAM escalation if Enterprise'))))
    lines.append(R(merge(bMid(L1, R1, 'Business+: all Trusted Advisor checks'), bMid(L2, R2, '5. AWS executive escalation path'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Include CloudTrail events, CloudWatch metrics, and resource IDs when opening cases.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Trusted Advisor Checks'), bMid(L2, R2, 'AWS Health & Proactive'))))
    lines.append(R(merge(bMid(L1, R1, 'Cost: idle instances, underused RI'), bMid(L2, R2, 'AWS Health: account events'))))
    lines.append(R(merge(bMid(L1, R1, 'Security: MFA, open SGs, key age'), bMid(L2, R2, 'Proactive events: TAM notification'))))
    lines.append(R(merge(bMid(L1, R1, 'Fault tolerance: AZ coverage, DR'), bMid(L2, R2, 'Well-Architected Tool: review'))))
    lines.append(R(merge(bMid(L1, R1, 'Performance: overloaded instances'), bMid(L2, R2, 're:Post: community Q&A platform'))))
    lines.append(R(merge(bMid(L1, R1, 'Service limits: approaching quotas'), bMid(L2, R2, 'Service Quotas: self-service raise'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('AWS support infrastructure · TAM engagement portal · Trusted Advisor analysis plane'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('TAM             = Technical Account Manager; designated AWS engineer for Enterprise plans'))
    lines.append(txt_row('Severity Urgent  = Production system down; 15-min response time on Enterprise plan'))
    lines.append(txt_row('Severity High   = Significant business impact; 1h response on Business plan'))
    lines.append(txt_row('Trusted Advisor = AWS automated best-practice checks across 5 pillars'))
    lines.append(txt_row('Well-Architected = AWS framework review across 6 pillars; tool generates findings'))
    lines.append(txt_row('Service Quotas  = Service that tracks and enables self-service quota increase requests'))
    lines.append(txt_row('Concierge       = Enterprise support specialist for billing and account questions'))
    lines.append(txt_row('re:Post         = AWS community Q&A platform replacing forums; AWS experts answer'))
    lines.append(txt_row('Proactive event = TAM-initiated notification about upcoming maintenance or risk'))
    lines.append(txt_row('Support case    = Ticket opened in AWS console with issue details and severity'))
    lines.append(txt_row('Cost optimisation check= Trusted Advisor identifies idle/underutilised resources'))
    lines.append(txt_row('Service limit   = Account-level quota on AWS resources; some auto-raise, some manual'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('azure-arch-how', 'docs/cloud/azure/architecture/how-it-works/index.md', 'Azure architecture how it works — management groups, subscriptions, and resource hierarchy')
def _azure_arch_how():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Azure Architecture — How It Works'))
    lines.append(txt_row())
    lines.append(txt_row('Azure organises resources in a hierarchy: Management Groups → Subscriptions → RGs → Resources.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Resource Hierarchy'), bMid(L2, R2, 'Resource Manager (ARM)'))))
    lines.append(R(merge(bMid(L1, R1, 'Root Management Group: top level'), bMid(L2, R2, 'ARM: single control plane API'))))
    lines.append(R(merge(bMid(L1, R1, 'Management Groups: OU equivalent'), bMid(L2, R2, 'Resource Provider: register per svc'))))
    lines.append(R(merge(bMid(L1, R1, 'Subscriptions: billing + access unit'), bMid(L2, R2, 'ARM template: IaC JSON/Bicep'))))
    lines.append(R(merge(bMid(L1, R1, 'Resource Groups: lifecycle container'), bMid(L2, R2, 'RBAC: scope at any level'))))
    lines.append(R(merge(bMid(L1, R1, 'Resources: VMs, storage, databases'), bMid(L2, R2, 'Policy: enforced via ARM layer'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Policies assigned at MG inherit down; RBAC assigned at RG controls resource access.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Authentication Flow'), bMid(L2, R2, 'Service Deployment'))))
    lines.append(R(merge(bMid(L1, R1, 'Entra ID: identity provider'), bMid(L2, R2, 'Portal: web UI for all resources'))))
    lines.append(R(merge(bMid(L1, R1, 'Token: OAuth2 / OIDC flow'), bMid(L2, R2, 'CLI: az resource commands'))))
    lines.append(R(merge(bMid(L1, R1, 'ARM API: accepts token + validates'), bMid(L2, R2, 'SDK: .NET/Python/Java/JS'))))
    lines.append(R(merge(bMid(L1, R1, 'Policy evaluated at ARM layer'), bMid(L2, R2, 'IaC: Bicep / Terraform / ARM'))))
    lines.append(R(merge(bMid(L1, R1, 'RBAC: checked before resource op'), bMid(L2, R2, 'DevOps: GitHub Actions / ADO'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Azure regions · Availability Zones · Physical data centres · Global backbone network'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Management Group= Container above subscriptions for policy and RBAC inheritance'))
    lines.append(txt_row('Subscription    = Billing and access control boundary; contains resource groups'))
    lines.append(txt_row('Resource Group  = Logical container for related resources sharing a lifecycle'))
    lines.append(txt_row('ARM             = Azure Resource Manager; unified control plane for all Azure resources'))
    lines.append(txt_row('Resource Provider= Azure service that supplies resource types, e.g. Microsoft.Compute'))
    lines.append(txt_row('Bicep           = Domain-specific language that compiles to ARM templates'))
    lines.append(txt_row('RBAC            = Role-Based Access Control; assigned to identity at a scope'))
    lines.append(txt_row('Azure Policy    = Service enforcing governance rules at ARM layer; deny or audit'))
    lines.append(txt_row('Entra ID        = Azure Active Directory; cloud identity provider for all Azure auth'))
    lines.append(txt_row('OAuth2 token    = Short-lived bearer token issued by Entra ID for ARM API calls'))
    lines.append(txt_row('Root MG         = Top-level management group; all subscriptions under one tenant'))
    lines.append(txt_row('IaC             = Infrastructure as Code; define Azure resources in declarative templates'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('azure-arch-int', 'docs/cloud/azure/architecture/integrations/index.md', 'Azure architecture integrations — on-premises, identity federation, and hybrid connectivity')
def _azure_arch_int():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Azure Architecture — Integrations'))
    lines.append(txt_row())
    lines.append(txt_row('Azure integrates with on-premises via ExpressRoute/VPN, AD sync, and hybrid networking.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Identity Integration'), bMid(L2, R2, 'Network Integration'))))
    lines.append(R(merge(bMid(L1, R1, 'Entra ID Connect: AD sync'), bMid(L2, R2, 'ExpressRoute: private 1/10 Gbps'))))
    lines.append(R(merge(bMid(L1, R1, 'SAML/OIDC: federate to IdP'), bMid(L2, R2, 'Site-to-Site VPN: IPSec tunnel'))))
    lines.append(R(merge(bMid(L1, R1, 'Password hash sync or passthrough'), bMid(L2, R2, 'VNet Peering: cross-region VNets'))))
    lines.append(R(merge(bMid(L1, R1, 'Seamless SSO: transparent auth'), bMid(L2, R2, 'Virtual WAN: hub-and-spoke'))))
    lines.append(R(merge(bMid(L1, R1, 'B2B: external partner identities'), bMid(L2, R2, 'Private Endpoint: no internet'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('ExpressRoute bypasses internet; Private Endpoints restrict service access to VNet only.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data & App Integration'), bMid(L2, R2, 'Monitoring Integration'))))
    lines.append(R(merge(bMid(L1, R1, 'Azure Arc: manage on-prem in Azure'), bMid(L2, R2, 'Azure Monitor: unified telemetry'))))
    lines.append(R(merge(bMid(L1, R1, 'Logic Apps: workflow automation'), bMid(L2, R2, 'Log Analytics: central log sink'))))
    lines.append(R(merge(bMid(L1, R1, 'API Management: gateway + portal'), bMid(L2, R2, 'Defender for Cloud: posture mgmt'))))
    lines.append(R(merge(bMid(L1, R1, 'Event Grid: event routing service'), bMid(L2, R2, 'Sentinel: cloud-native SIEM'))))
    lines.append(R(merge(bMid(L1, R1, 'Service Bus: enterprise messaging'), bMid(L2, R2, 'Cost Management: billing view'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Azure global backbone · ExpressRoute partner peering locations · Regional POPs'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Entra ID Connect= Tool syncing on-premises AD users and groups to Entra ID'))
    lines.append(txt_row('Password hash sync= Syncs hashed passwords; allows cloud auth without on-prem dependency'))
    lines.append(txt_row('PassthroughAuth = Validates passwords against on-prem AD; requires agent online'))
    lines.append(txt_row('ExpressRoute    = Dedicated private circuit to Azure bypassing public internet'))
    lines.append(txt_row('S2S VPN         = IPSec/IKEv2 encrypted tunnel over internet to Azure VPN Gateway'))
    lines.append(txt_row('Virtual WAN     = Azure managed hub-and-spoke network for global connectivity'))
    lines.append(txt_row('Azure Arc       = Extends Azure management plane to on-prem and multi-cloud resources'))
    lines.append(txt_row('Private Endpoint= NIC in VNet with private IP for a PaaS service; no public exposure'))
    lines.append(txt_row('B2B federation  = Allows external Azure AD/Entra tenants to access your resources'))
    lines.append(txt_row('Seamless SSO    = Kerberos-based SSO for domain-joined machines accessing Azure apps'))
    lines.append(txt_row('Event Grid      = Publish-subscribe event routing service; connects Azure services'))
    lines.append(txt_row('Azure Sentinel  = Cloud-native SIEM/SOAR; ingests logs and generates security alerts'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines


@kb_diagram('azure-arch-design', 'docs/cloud/azure/architecture/design-standards/index.md', 'Azure architecture design standards — naming, tagging, regions, and landing zones')
def _azure_arch_design():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Azure Architecture — Design Standards'))
    lines.append(txt_row())
    lines.append(txt_row('Naming, tagging, region, and landing zone standards for consistent Azure governance.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Naming Conventions'), bMid(L2, R2, 'Tagging Standards'))))
    lines.append(R(merge(bMid(L1, R1, 'Format: {type}-{app}-{env}-{region}'), bMid(L2, R2, 'env: prod/dev/staging/test'))))
    lines.append(R(merge(bMid(L1, R1, 'Max length: varies by resource type'), bMid(L2, R2, 'owner: team or individual'))))
    lines.append(R(merge(bMid(L1, R1, 'Lowercase alphanumeric + hyphens'), bMid(L2, R2, 'cost-center: finance code'))))
    lines.append(R(merge(bMid(L1, R1, 'Storage: no hyphens in names'), bMid(L2, R2, 'project: workload identifier'))))
    lines.append(R(merge(bMid(L1, R1, 'Global unique: storage + ACR names'), bMid(L2, R2, 'Inherit from RG: tag auto-apply'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Landing zones provide pre-configured subscription patterns enforcing design standards.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Region & Availability'), bMid(L2, R2, 'Landing Zone Design'))))
    lines.append(R(merge(bMid(L1, R1, 'Primary + secondary region pair'), bMid(L2, R2, 'Platform subscriptions: mgmt'))))
    lines.append(R(merge(bMid(L1, R1, 'Paired regions: Azure-defined pairs'), bMid(L2, R2, 'Application landing zones: workload'))))
    lines.append(R(merge(bMid(L1, R1, 'AZs: 3 per supported region'), bMid(L2, R2, 'Connectivity sub: hub VNet'))))
    lines.append(R(merge(bMid(L1, R1, 'Availability Set: update + fault dom'), bMid(L2, R2, 'Identity sub: Entra + DCs'))))
    lines.append(R(merge(bMid(L1, R1, 'SLA: varies per service + zone use'), bMid(L2, R2, 'Policy: enforce at MG level'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Azure physical regions · Availability Zone data centres · Region-pair replication links'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Landing zone    = Pre-configured subscription with policies, networking, and RBAC baseline'))
    lines.append(txt_row('CAF             = Cloud Adoption Framework; Microsoft guidance for Azure governance'))
    lines.append(txt_row('Region pair     = Two Azure regions paired for sequential updates and data residency'))
    lines.append(txt_row('Availability Zone= Physically separate DC within a region; 99.99% SLA when used'))
    lines.append(txt_row('Update domain   = Availability Set grouping protecting VMs from simultaneous updates'))
    lines.append(txt_row('Fault domain    = Availability Set grouping on separate hardware/power/network racks'))
    lines.append(txt_row('Hub VNet        = Central network for shared services: firewall, DNS, VPN gateway'))
    lines.append(txt_row('Spoke VNet      = Workload VNet peered to hub; isolated per application or team'))
    lines.append(txt_row('MG policy scope = Policies assigned at MG apply to all child subscriptions and RGs'))
    lines.append(txt_row('Tag inheritance = Configuring RG tag inheritance propagates tags to child resources'))
    lines.append(txt_row('Global unique   = Some Azure resource names (storage, ACR) must be globally unique'))
    lines.append(txt_row('Platform sub    = Dedicated subscriptions for management, connectivity, and identity'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
