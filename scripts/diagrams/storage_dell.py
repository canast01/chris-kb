...
@kb_diagram(
    'dell-apex-arch-index',
    'docs/storage/dell/apex-storage-as-a-service/architecture/index.md',
    'Dell APEX Storage-as-a-Service Architecture — consumption-based, APEX Console, underlying arrays',
)
def dell_apex_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell APEX Storage-as-a-Service — Architecture Overview'))
    lines.append(txt_row())
    lines.append(txt_row('Consumption-based storage delivered to your data center or AWS; no upfront CapEx;'))
    lines.append(txt_row('Dell provisions and manages hardware; customer manages data and workloads.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Service Model'), bMid(L2, R2, 'Underlying Storage Platforms'))))
    lines.append(R(merge(bMid(L1, R1, 'Pay per GB/month consumed'), bMid(L2, R2, 'PowerStore X: block workloads'))))
    lines.append(R(merge(bMid(L1, R1, 'Hardware delivered to DC'), bMid(L2, R2, 'PowerMax: mission-critical'))))
    lines.append(R(merge(bMid(L1, R1, 'No upfront capital cost'), bMid(L2, R2, 'PowerFlex: hyper-converged'))))
    lines.append(R(merge(bMid(L1, R1, 'Dell manages HW lifecycle'), bMid(L2, R2, 'ECS: object storage (S3)'))))
    lines.append(R(merge(bMid(L1, R1, 'Elastic burst: +/- capacity'), bMid(L2, R2, 'PowerScale: scale-out NAS'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Customer retains control of data placement, access, and security policies.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'APEX Console (Management)'), bMid(L2, R2, 'Operational Model'))))
    lines.append(R(merge(bMid(L1, R1, 'Single portal: all services'), bMid(L2, R2, 'Dell: HW monitoring + support'))))
    lines.append(R(merge(bMid(L1, R1, 'REST API for automation'), bMid(L2, R2, 'Customer: VM/app workloads'))))
    lines.append(R(merge(bMid(L1, R1, 'Subscription management'), bMid(L2, R2, 'CloudIQ: performance insights'))))
    lines.append(R(merge(bMid(L1, R1, 'Consumption dashboards'), bMid(L2, R2, 'APEX Observer: on-prem agent'))))
    lines.append(R(merge(bMid(L1, R1, 'Multi-site view: one pane'), bMid(L2, R2, 'SLA: 99.9999% availability'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Dell-supplied arrays in customer rack; dedicated rack power and cooling by customer;'))
    lines.append(txt_row('network connectivity to APEX Console via internet (TCP 443); APEX Observer VM on-prem.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('APEX           = Dell Storage-as-a-Service brand; on-prem hardware, cloud billing'))
    lines.append(txt_row('APEX Console   = SaaS management portal for all APEX services; REST API'))
    lines.append(txt_row('OpEx model     = operational expenditure; no large upfront hardware purchase'))
    lines.append(txt_row('Commitment period= minimum contract term (1, 3, or 5 years)'))
    lines.append(txt_row('Burst capacity = temporary capacity above committed level; billed at higher rate'))
    lines.append(txt_row('APEX Observer  = on-prem VM that reports telemetry to APEX Console'))
    lines.append(txt_row('CloudIQ        = Dell AIOps analytics; integrated with APEX for health scoring'))
    lines.append(txt_row('PowerStore     = Dell mid-range AFA; common APEX block storage platform'))
    lines.append(txt_row('PowerMax       = Dell high-end AFA; APEX option for mission-critical block'))
    lines.append(txt_row('PowerFlex      = Dell hyper-converged / software-defined storage option'))
    lines.append(txt_row('ECS            = Elastic Cloud Storage; Dell object storage; S3-compatible'))
    lines.append(txt_row('Elastic scaling= add or reduce committed capacity via APEX Console request'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-cloudiq-arch-index',
    'docs/storage/dell/cloudiq/architecture/index.md',
    'Dell CloudIQ Architecture — AIOps SaaS, telemetry, anomaly detection, capacity forecasting',
)
def dell_cloudiq_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell CloudIQ — AIOps Analytics Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('SaaS-hosted AIOps platform; collects telemetry from Dell storage via call-home;'))
    lines.append(txt_row('ML-based anomaly detection, capacity forecasting, and health scoring per system.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Collection'), bMid(L2, R2, 'Supported Platforms'))))
    lines.append(R(merge(bMid(L1, R1, 'Call-home: API telemetry'), bMid(L2, R2, 'PowerStore, PowerMax'))))
    lines.append(R(merge(bMid(L1, R1, 'No agent on most platforms'), bMid(L2, R2, 'PowerScale, Unity XT'))))
    lines.append(R(merge(bMid(L1, R1, '15-minute granularity'), bMid(L2, R2, 'Data Domain, ECS'))))
    lines.append(R(merge(bMid(L1, R1, '13 months: historical data'), bMid(L2, R2, 'PowerFlex, SC Series'))))
    lines.append(R(merge(bMid(L1, R1, 'SaaS: Dell-hosted, no on-prem'), bMid(L2, R2, 'PowerProtect DD'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('CloudIQ analyzes patterns across millions of array data points using ML models.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Analytics Features'), bMid(L2, R2, 'Integration Points'))))
    lines.append(R(merge(bMid(L1, R1, 'Health score: 0-100 per system'), bMid(L2, R2, 'REST API: northbound'))))
    lines.append(R(merge(bMid(L1, R1, 'Anomaly: deviation from baseline'), bMid(L2, R2, 'SIEM: event export'))))
    lines.append(R(merge(bMid(L1, R1, 'Capacity forecast: 90-day'), bMid(L2, R2, 'ServiceNow integration'))))
    lines.append(R(merge(bMid(L1, R1, 'Workload intelligence: IO type'), bMid(L2, R2, 'Splunk: telemetry export'))))
    lines.append(R(merge(bMid(L1, R1, 'What-if: right-size scenarios'), bMid(L2, R2, 'Email/Slack: alert notify'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('CloudIQ is SaaS — no on-prem server required; arrays send telemetry outbound to'))
    lines.append(txt_row('Dell cloud on TCP 443; arrays need internet access or ESRS proxy.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('CloudIQ        = Dell AIOps SaaS platform; storage health and analytics'))
    lines.append(txt_row('Health score   = 0-100 score per array; below 80 triggers recommended action'))
    lines.append(txt_row('Anomaly        = ML-detected deviation from normal performance baseline'))
    lines.append(txt_row('Capacity forecast= 90-day prediction of when storage will fill up'))
    lines.append(txt_row('Workload intelligence= classifies IO patterns: random, sequential, mixed'))
    lines.append(txt_row('What-if        = scenario: what happens to latency if I add X GB of workload?'))
    lines.append(txt_row('Call-home      = array-to-Dell telemetry channel; also used for ESRS/support'))
    lines.append(txt_row('SaaS           = Software as a Service; Dell hosts, no customer infra required'))
    lines.append(txt_row('Connector      = per-platform CloudIQ plugin; enabled on array config'))
    lines.append(txt_row('REST API       = CloudIQ northbound API; pull metrics into custom dashboards'))
    lines.append(txt_row('ESRS           = EMC Secure Remote Services; call-home connectivity mechanism'))
    lines.append(txt_row('Alert policy   = user-defined rule; sends email/Slack when threshold exceeded'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    ...
    'Dell COD Architecture — PowerMax capacity on demand, ESRS activation, license model',
)
def dell_cod_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell COD — Capacity on Demand Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Software-defined capacity expansion for PowerMax; no hardware shipment needed;'))
    lines.append(txt_row('activated via ESRS connectivity to Dell cloud; license key unlocks pre-installed capacity.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Licensing Model'), bMid(L2, R2, 'Activation Process'))))
    lines.append(R(merge(bMid(L1, R1, 'Base capacity: shipped with array'), bMid(L2, R2, 'Order COD units from Dell'))))
    lines.append(R(merge(bMid(L1, R1, 'COD: pre-installed, locked'), bMid(L2, R2, 'License key sent by email'))))
    lines.append(R(merge(bMid(L1, R1, 'Tiers: increments of capacity'), bMid(L2, R2, 'Import in Unisphere: COD'))))
    lines.append(R(merge(bMid(L1, R1, 'Pay: only activated capacity'), bMid(L2, R2, 'ESRS validates entitlement'))))
    lines.append(R(merge(bMid(L1, R1, 'Grace period: 30 days offline'), bMid(L2, R2, 'Capacity available in minutes'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('COD is capacity only; FOD is for software features; both require ESRS connectivity.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'ESRS Integration'), bMid(L2, R2, 'COD vs FOD'))))
    lines.append(R(merge(bMid(L1, R1, 'ESRS: call-home to esrs.dell.com'), bMid(L2, R2, 'COD: raw capacity unlock'))))
    lines.append(R(merge(bMid(L1, R1, 'TCP 443 outbound required'), bMid(L2, R2, 'FOD: feature activation'))))
    lines.append(R(merge(bMid(L1, R1, 'ESRS gateway: optional proxy'), bMid(L2, R2, 'Both: same ESRS channel'))))
    lines.append(R(merge(bMid(L1, R1, 'Offline: 30-day grace period'), bMid(L2, R2, 'Both: supported on PowerMax'))))
    lines.append(R(merge(bMid(L1, R1, 'ConnectEMC: legacy ESRS name'), bMid(L2, R2, 'Both: instant activation'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('PowerMax array (all capacity physically installed); ESRS gateway VM optional;'))
    lines.append(txt_row('management network with internet access to esrs.dell.com on TCP 443.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('COD            = Capacity on Demand; pre-installed capacity unlocked by license'))
    lines.append(txt_row('FOD            = Features on Demand; software feature activation (SRDF, TimeFinder)'))
    lines.append(txt_row('ESRS           = EMC Secure Remote Services; call-home + license validation channel'))
    lines.append(txt_row('Unisphere      = PowerMax web management UI; where COD is activated'))
    lines.append(txt_row('License key    = alphanumeric string delivered by Dell email after order'))
    lines.append(txt_row('Capacity tier  = fixed increment of COD capacity available for activation'))
    lines.append(txt_row('Grace period   = 30 days ESRS offline before COD locks; contact Dell to extend'))
    lines.append(txt_row('ConnectEMC     = legacy name for ESRS call-home service'))
    lines.append(txt_row('ESRS gateway   = optional on-prem VM that proxies array call-home traffic'))
    lines.append(txt_row('PowerMax       = Dell high-end AFA; COD applies to this platform'))
    lines.append(txt_row('Entitlement    = Dell contract record matching license key to serial number'))
    lines.append(txt_row('esrs.dell.com  = Dell ESRS validation endpoint; TCP 443 must be reachable'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-data-domain-arch-index',
    'docs/storage/dell/data-domain/architecture/index.md',
    'Dell Data Domain Architecture — PBBA, inline dedup, DD Boost, replication',
)
def dell_data_domain_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell Data Domain — Backup Appliance Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Purpose-built backup appliance (PBBA) with inline global deduplication (65:1 typical);'))
    lines.append(txt_row('DD Boost protocol for direct backup app integration; DD Replicator for AIR.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Core Architecture'), bMid(L2, R2, 'Data Paths'))))
    lines.append(R(merge(bMid(L1, R1, 'PBBA: optimized for backup IO'), bMid(L2, R2, 'DD Boost: OST plugin on client'))))
    lines.append(R(merge(bMid(L1, R1, 'Inline dedup: before writing'), bMid(L2, R2, 'NFS/CIFS: legacy backup apps'))))
    lines.append(R(merge(bMid(L1, R1, 'Typical ratio: 20:1 to 65:1'), bMid(L2, R2, 'DD VTL: virtual tape library'))))
    lines.append(R(merge(bMid(L1, R1, 'DDVE: virtual edition (VM)'), bMid(L2, R2, 'Boost FC: FC-attached option'))))
    lines.append(R(merge(bMid(L1, R1, 'DD OS: purpose-built OS'), bMid(L2, R2, 'S3: object interface (DDVE)'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('DD Boost offloads dedup processing to client (backup server) for faster throughput.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Replication (DD Replicator)'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'AIR: Automatic Image Repl'), bMid(L2, R2, 'DD OS CLI: ddsh shell'))))
    lines.append(R(merge(bMid(L1, R1, 'Dir-to-dir or MTree replica'), bMid(L2, R2, 'DD System Manager: web UI'))))
    lines.append(R(merge(bMid(L1, R1, 'Collection replication: full'), bMid(L2, R2, 'DD Mgmt Center: multi-system'))))
    lines.append(R(merge(bMid(L1, R1, 'Cascade: A → B → C chain'), bMid(L2, R2, 'REST API v2.1: automation'))))
    lines.append(R(merge(bMid(L1, R1, 'Cloud tier: S3 cold tier'), bMid(L2, R2, 'SNMP: monitoring integration'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('DD appliance (DD6300/DD9300/DD9900) or DDVE VM; FC or Ethernet to backup servers;'))
    lines.append(txt_row('replication link: WAN between primary and DR site DD appliances.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PBBA           = Purpose-Built Backup Appliance; optimized HW + SW for backup'))
    lines.append(txt_row('DD Boost       = Dell Data Domain Boost; dedup-aware backup app protocol'))
    lines.append(txt_row('OST            = OpenStorage Technology; Veritas API that DD Boost uses'))
    lines.append(txt_row('Deduplication  = removing duplicate data; write once, reference many times'))
    lines.append(txt_row('MTree          = logical partition on Data Domain; like a volume; each client uses one'))
    lines.append(txt_row('AIR            = Automatic Image Replication; replicates backup images to remote DD'))
    lines.append(txt_row('DDVE           = Data Domain Virtual Edition; runs as VM (VMware or AWS)'))
    lines.append(txt_row('DD Replicator  = DD feature that performs async replication between DD systems'))
    lines.append(txt_row('DD VTL         = Virtual Tape Library; presents DD as tape drives to legacy apps'))
    lines.append(txt_row('Collection replica= full DD system replication; used for DR of the DD itself'))
    lines.append(txt_row('StreamFusion   = DD technology for parallel stream dedup during backup'))
    lines.append(txt_row('Cloud tier     = cold tier to S3/Azure Blob; reduces on-prem footprint'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-aiops-arch-index',
    'docs/storage/dell/dell-aiops/architecture/index.md',
    'Dell AIOps Architecture — CloudIQ, anomaly detection, capacity intelligence, ML analytics',
)
def dell_aiops_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell AIOps — AI-Driven Storage Operations'))
    lines.append(txt_row())
    lines.append(txt_row('AIOps umbrella: CloudIQ as the analytics engine; ML trained on millions of metrics;'))
    lines.append(txt_row('anomaly detection, capacity forecasting, and workload intelligence reduce manual effort.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'AIOps Platform'), bMid(L2, R2, 'AI Capabilities'))))
    lines.append(R(merge(bMid(L1, R1, 'CloudIQ: core analytics SaaS'), bMid(L2, R2, 'Anomaly: vs trained baseline'))))
    lines.append(R(merge(bMid(L1, R1, 'ML model: fleet-trained'), bMid(L2, R2, 'Capacity forecast: 90 days'))))
    lines.append(R(merge(bMid(L1, R1, 'Health score: 0-100 per array'), bMid(L2, R2, 'Workload class: IO pattern'))))
    lines.append(R(merge(bMid(L1, R1, 'Real-time: 15-min granularity'), bMid(L2, R2, 'Right-size: advisor recs'))))
    lines.append(R(merge(bMid(L1, R1, 'Historical: 13 months'), bMid(L2, R2, 'Proactive: auto SR creation'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('ML models trained across the entire Dell storage fleet; individual anomalies detected'))
    lines.append(txt_row('by comparing against what is normal for that array type and workload pattern.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Sources'), bMid(L2, R2, 'Outcomes'))))
    lines.append(R(merge(bMid(L1, R1, 'Array telemetry via call-home'), bMid(L2, R2, 'Fewer reactive incidents'))))
    lines.append(R(merge(bMid(L1, R1, 'No agent on most platforms'), bMid(L2, R2, 'Optimized capacity spend'))))
    lines.append(R(merge(bMid(L1, R1, 'API connectors per product'), bMid(L2, R2, 'Proactive SR before failure'))))
    lines.append(R(merge(bMid(L1, R1, 'APEX: STaaS consumption data'), bMid(L2, R2, 'Cross-tier visibility'))))
    lines.append(R(merge(bMid(L1, R1, 'Compute + network context'), bMid(L2, R2, 'API: Splunk/ServiceNow'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Entirely SaaS — no on-prem server; arrays connect outbound to Dell cloud on TCP 443;'))
    lines.append(txt_row('ESRS or ESRS gateway required for call-home; APEX Observer optional on-prem VM.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('AIOps          = AI for IT Operations; ML applied to infrastructure management'))
    lines.append(txt_row('CloudIQ        = Dell AIOps SaaS; storage health, anomaly, capacity analytics'))
    lines.append(txt_row('ML model       = trained on fleet-wide telemetry; detects per-array anomalies'))
    lines.append(txt_row('Anomaly        = metric significantly deviating from its historical normal range'))
    lines.append(txt_row('Capacity intelligence= predicts when arrays will reach threshold, not just current %'))
    lines.append(txt_row('Workload class = IO pattern label (random read, sequential write, mixed) per LUN'))
    lines.append(txt_row('Proactive SR   = CloudIQ can auto-open Dell support case before admin notices'))
    lines.append(txt_row('Health score   = composite score per array; accounts for performance + capacity'))
    lines.append(txt_row('Cross-tier     = visibility linking compute CPU pressure to storage latency cause'))
    lines.append(txt_row('Call-home      = telemetry path from array to Dell cloud; same as ESRS channel'))
    lines.append(txt_row('Right-size     = recommendation to rebalance workloads across arrays'))
    lines.append(txt_row('APEX Observer  = on-prem agent that enriches telemetry with local context'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-ecs-arch-index',
    'docs/storage/dell/ecs/architecture/index.md',
    'Dell ECS Architecture — scale-out object storage, S3/Swift/CAS, erasure coding, geo-distribution',
)
def dell_ecs_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell ECS — Elastic Cloud Storage Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Software-defined object storage on commodity x86 nodes; S3/Swift/CAS protocols;'))
    lines.append(txt_row('Reed-Solomon erasure coding across nodes; geo-distribution across multiple sites.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Cluster Architecture'), bMid(L2, R2, 'Supported Protocols'))))
    lines.append(R(merge(bMid(L1, R1, '3+ nodes minimum per VDC'), bMid(L2, R2, 'S3: primary; widest support'))))
    lines.append(R(merge(bMid(L1, R1, 'RAIN: node-level redundancy'), bMid(L2, R2, 'Swift: OpenStack object API'))))
    lines.append(R(merge(bMid(L1, R1, 'Scale: add nodes horizontally'), bMid(L2, R2, 'CAS: content-addressable'))))
    lines.append(R(merge(bMid(L1, R1, 'VDC: virtual data center'), bMid(L2, R2, 'NFS: via S3-NFS gateway'))))
    lines.append(R(merge(bMid(L1, R1, 'Geo: multiple VDCs linked'), bMid(L2, R2, 'HDFS: Hadoop connector'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Namespace and bucket hierarchy controls access; no traditional filesystem paths.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Protection'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'Erasure coding: 12/4 or 10/4'), bMid(L2, R2, 'ECS Portal: web UI'))))
    lines.append(R(merge(bMid(L1, R1, 'No RAID: node-level coding'), bMid(L2, R2, 'ECS REST API: automation'))))
    lines.append(R(merge(bMid(L1, R1, 'Geo-replication across VDCs'), bMid(L2, R2, 'Namespace + bucket structure'))))
    lines.append(R(merge(bMid(L1, R1, 'WORM: immutable object lock'), bMid(L2, R2, 'S3 ACLs + bucket policies'))))
    lines.append(R(merge(bMid(L1, R1, 'Object versioning supported'), bMid(L2, R2, 'IAM-compatible user model'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('ECS commodity x86 nodes (ECS 300/500 series); 25GbE networking between nodes;'))
    lines.append(txt_row('ECS Appliance or ECS Software on commodity servers; 3-node minimum per VDC.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('ECS            = Elastic Cloud Storage; Dell object storage platform'))
    lines.append(txt_row('Object storage = stores data as objects (ID + data + metadata); no file hierarchy'))
    lines.append(txt_row('S3             = Simple Storage Service; Amazon API now the object storage standard'))
    lines.append(txt_row('RAIN           = Redundant Array of Independent Nodes; erasure coding at node level'))
    lines.append(txt_row('Erasure coding = like RAID 6 but across nodes; 12 data + 4 parity = 25% overhead'))
    lines.append(txt_row('WORM           = Write Once Read Many; immutable object lock for compliance'))
    lines.append(txt_row('VDC            = Virtual Data Center; an ECS cluster unit; multiple VDCs = geo'))
    lines.append(txt_row('Namespace      = top-level ECS tenant container; like an S3 account'))
    lines.append(txt_row('Bucket         = object container within a namespace; like an S3 bucket'))
    lines.append(txt_row('CAS            = Content Addressable Storage; Dell Centera-compatible API'))
    lines.append(txt_row('Swift          = OpenStack object storage API; ECS-supported for OpenStack'))
    lines.append(txt_row('Geo-distribution= objects replicated across multiple VDCs in different sites'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-fod-arch-index',
    'docs/storage/dell/fod/architecture/index.md',
    'Dell FOD Architecture — PowerMax software feature licensing, SRDF/TimeFinder activation',
)
def dell_fod_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell FOD — Features on Demand Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Software feature licensing for PowerMax; activates SRDF, TimeFinder, and other features'))
    lines.append(txt_row('without hardware changes; license key via ESRS or offline import in Unisphere.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Licensing Model'), bMid(L2, R2, 'Available Features'))))
    lines.append(R(merge(bMid(L1, R1, 'Software-defined activation'), bMid(L2, R2, 'SRDF: synchronous replication'))))
    lines.append(R(merge(bMid(L1, R1, 'License key: term or permanent'), bMid(L2, R2, 'TimeFinder: snapshots+clones'))))
    lines.append(R(merge(bMid(L1, R1, 'ESRS: online validation'), bMid(L2, R2, 'SRDF/Metro: active-active'))))
    lines.append(R(merge(bMid(L1, R1, 'Offline: import license file'), bMid(L2, R2, 'Analytics: AI-based recs'))))
    lines.append(R(merge(bMid(L1, R1, 'No hardware change needed'), bMid(L2, R2, 'Cloud tiering: to S3'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('SRDF and TimeFinder are most commonly licensed via FOD; both need ESRS for validation.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Activation Steps'), bMid(L2, R2, 'FOD vs COD'))))
    lines.append(R(merge(bMid(L1, R1, 'Order feature from Dell'), bMid(L2, R2, 'FOD: software feature unlock'))))
    lines.append(R(merge(bMid(L1, R1, 'License key sent by email'), bMid(L2, R2, 'COD: raw capacity unlock'))))
    lines.append(R(merge(bMid(L1, R1, 'Unisphere: System → FOD'), bMid(L2, R2, 'Same ESRS channel for both'))))
    lines.append(R(merge(bMid(L1, R1, 'Import key → ESRS validates'), bMid(L2, R2, 'Can combine on same array'))))
    lines.append(R(merge(bMid(L1, R1, 'Feature active in minutes'), bMid(L2, R2, 'Both: PowerMax only'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('PowerMax array (all features physically capable but software-locked);'))
    lines.append(txt_row('ESRS connectivity to esrs.dell.com on TCP 443 required for online activation.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('FOD            = Features on Demand; PowerMax software feature licensing'))
    lines.append(txt_row('SRDF           = Symmetrix Remote Data Facility; PowerMax replication technology'))
    lines.append(txt_row('TimeFinder     = PowerMax snapshot and clone technology'))
    lines.append(txt_row('SRDF/Metro     = synchronous active-active replication; zero RPO Metro stretch'))
    lines.append(txt_row('ESRS           = EMC Secure Remote Services; Dell license validation channel'))
    lines.append(txt_row('Unisphere      = PowerMax web management UI; FOD license import location'))
    lines.append(txt_row('Term license   = time-limited feature license; must be renewed'))
    lines.append(txt_row('Permanent license= one-time purchase; no renewal; tied to array serial number'))
    lines.append(txt_row('COD            = Capacity on Demand; capacity extension (different from FOD)'))
    lines.append(txt_row('Feature lock   = feature code in array firmware; FOD key unlocks it'))
    lines.append(txt_row('Offline import = load license file via USB/SFTP without ESRS connectivity'))
    lines.append(txt_row('PowerMax       = Dell high-end AFA; only platform supporting FOD'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-powermax-arch-index',
    'docs/storage/dell/powermax/architecture/index.md',
    'Dell PowerMax Architecture — all-NVMe AFA, SRDF, TimeFinder, mainframe support, 5-nines',
)
def dell_powermax_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell PowerMax — High-End AFA Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('All-NVMe all-flash array for mission-critical workloads; DirectPath I/O to NVMe;'))
    lines.append(txt_row('SRDF replication (sync/async/Metro); TimeFinder snapshots; FICON for mainframe.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Platform'), bMid(L2, R2, 'Data Services'))))
    lines.append(R(merge(bMid(L1, R1, 'All-NVMe AFA (2500/8500)'), bMid(L2, R2, 'SRDF: synchronous replication'))))
    lines.append(R(merge(bMid(L1, R1, 'DirectPath I/O: host→NVMe'), bMid(L2, R2, 'TimeFinder: snap + clone'))))
    lines.append(R(merge(bMid(L1, R1, 'NVMe-oF: fabric attachment'), bMid(L2, R2, 'FAST VP: auto-tiering'))))
    lines.append(R(merge(bMid(L1, R1, 'FICON: mainframe FC support'), bMid(L2, R2, 'Inline dedup + compression'))))
    lines.append(R(merge(bMid(L1, R1, '5-nines: 99.9999% HA target'), bMid(L2, R2, 'Thin provisioning: dynamic'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('SRDF/Metro enables active-active stretched clusters with zero RPO across two sites.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Replication Topologies'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'SRDF/S: sync; 0 RPO metro'), bMid(L2, R2, 'Unisphere for PowerMax'))))
    lines.append(R(merge(bMid(L1, R1, 'SRDF/A: async; low RPO remote'), bMid(L2, R2, 'SYMCLI: command line'))))
    lines.append(R(merge(bMid(L1, R1, 'SRDF/Metro: active-active'), bMid(L2, R2, 'REST API: v100+'))))
    lines.append(R(merge(bMid(L1, R1, 'Concurrent: SRDF/S + SRDF/A'), bMid(L2, R2, 'PowerMax OS: embedded'))))
    lines.append(R(merge(bMid(L1, R1, 'Cascade: 3-site protection'), bMid(L2, R2, 'CloudIQ: AIOps analytics'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('PowerMax 2500 (entry) or 8500 (enterprise); dual-engine active-active internally;'))
    lines.append(txt_row('FC/NVMe-oF SAN fabric connections; dedicated management network for Unisphere.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PowerMax       = Dell high-end AFA; successor to VMAX; mission-critical storage'))
    lines.append(txt_row('SRDF           = Symmetrix Remote Data Facility; synchronous/async replication'))
    lines.append(txt_row('TimeFinder     = PowerMax snap (instant, no copy) and clone (full copy) tech'))
    lines.append(txt_row('SRDF/Metro     = synchronous active-active; both sides serve IO simultaneously'))
    lines.append(txt_row('NVMe-oF        = NVMe over Fabrics; FC-NVMe or RoCE; lower latency than SCSI'))
    lines.append(txt_row('DirectPath I/O = host OS bypasses storage controller; direct PCIe to NVMe'))
    lines.append(txt_row('FICON          = IBM mainframe FC protocol; PowerMax is mainframe-compatible'))
    lines.append(txt_row('5-nines        = 99.9999% availability; ~32 seconds unplanned downtime per year'))
    lines.append(txt_row('FAST VP        = Fully Automated Storage Tiering; auto-moves data to right tier'))
    lines.append(txt_row('Unisphere      = PowerMax web management UI; also manages multiple arrays'))
    lines.append(txt_row('SYMCLI         = command-line interface for PowerMax/VMAX operations'))
    lines.append(txt_row('Concurrent SRDF= same volume replicated to two different sites simultaneously'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-powerpath-arch-index',
    'docs/storage/dell/powerpath/architecture/index.md',
    'Dell PowerPath Architecture — host multipathing, load balancing, FC and iSCSI paths',
)
def dell_powerpath_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell PowerPath — Host Multipathing Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Host-based multipathing software managing FC and iSCSI paths to Dell storage;'))
    lines.append(txt_row('Adaptive load balancing across paths; replaces native MPIO for Dell arrays.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Function'), bMid(L2, R2, 'Supported Platforms'))))
    lines.append(R(merge(bMid(L1, R1, 'Manages multiple SAN paths'), bMid(L2, R2, 'PowerMax, PowerStore'))))
    lines.append(R(merge(bMid(L1, R1, 'Presents single virtual device'), bMid(L2, R2, 'Unity, VNX, VMAX legacy'))))
    lines.append(R(merge(bMid(L1, R1, 'Balances IO across FC paths'), bMid(L2, R2, 'Windows, Linux, AIX'))))
    lines.append(R(merge(bMid(L1, R1, 'Fails over on path loss'), bMid(L2, R2, 'Solaris, HP-UX'))))
    lines.append(R(merge(bMid(L1, R1, 'Replaces native OS MPIO'), bMid(L2, R2, 'FC, iSCSI, NVMe-oF'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('PowerPath is installed on each host; one instance manages all paths from that host.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Load Balancing Policies'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'Adaptive: queue-depth aware'), bMid(L2, R2, 'powermt display: show paths'))))
    lines.append(R(merge(bMid(L1, R1, 'Least-Blocks: IO count'), bMid(L2, R2, 'powermt manage: add/remove'))))
    lines.append(R(merge(bMid(L1, R1, 'Round-Robin: equal share'), bMid(L2, R2, 'emcpowervt: path test'))))
    lines.append(R(merge(bMid(L1, R1, 'Symm-Optimized: array-aware'), bMid(L2, R2, 'PPMA: multi-host GUI'))))
    lines.append(R(merge(bMid(L1, R1, 'No-Rebalance: manual'), bMid(L2, R2, 'REST API: PPMA automation'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('HBA cards in host servers; FC cables to SAN switches; zoned to storage ports;'))
    lines.append(txt_row('minimum 2 paths per LUN (dual-fabric A+B) required for true redundancy.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PowerPath      = Dell host multipathing software; manages paths from host to array'))
    lines.append(txt_row('Multipathing   = using multiple physical connections to same LUN; HA + load balance'))
    lines.append(txt_row('Path           = one physical connection: HBA port → FC switch → storage port'))
    lines.append(txt_row('MPIO           = Microsoft/OS native multipath; PowerPath replaces this for Dell'))
    lines.append(txt_row('Adaptive       = default policy; monitors queue depth per path; moves busy load'))
    lines.append(txt_row('powermt        = PowerPath CLI tool; display, save, restore, manage paths'))
    lines.append(txt_row('PPMA           = PowerPath Management Appliance; central multi-host management'))
    lines.append(txt_row('Failover       = automatic reroute when a path goes down; transparent to host IO'))
    lines.append(txt_row('Trespass       = legacy VNX path ownership transfer; PowerPath handles silently'))
    lines.append(txt_row('FC path        = Fibre Channel path from HBA WWN to storage port WWPN'))
    lines.append(txt_row('iSCSI path     = TCP/IP path from initiator IQN to storage target IQN'))
    lines.append(txt_row('emcpowervt     = PowerPath path verification tool; tests individual paths'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-powerscale-arch-index',
    'docs/storage/dell/powerscale/architecture/index.md',
    'Dell PowerScale Architecture — OneFS scale-out NAS, NFS/SMB/HDFS, SyncIQ, SmartConnect',
)
def dell_powerscale_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell PowerScale — OneFS Scale-Out NAS Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Single filesystem namespace across all nodes; horizontal scale by adding nodes;'))
    lines.append(txt_row('NFS/SMB/HDFS/S3 protocols; SyncIQ async replication; SmartConnect DNS load balance.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'OneFS Architecture'), bMid(L2, R2, 'Protocols'))))
    lines.append(R(merge(bMid(L1, R1, 'Single filesystem namespace'), bMid(L2, R2, 'NFS v3/v4.1: Linux/Unix'))))
    lines.append(R(merge(bMid(L1, R1, 'H-series: HDD capacity'), bMid(L2, R2, 'SMB 2.1/3.0: Windows'))))
    lines.append(R(merge(bMid(L1, R1, 'F-series: NVMe performance'), bMid(L2, R2, 'HDFS: Hadoop workloads'))))
    lines.append(R(merge(bMid(L1, R1, 'A-series: AFA nodes'), bMid(L2, R2, 'S3: via S3 gateway'))))
    lines.append(R(merge(bMid(L1, R1, 'No LUN abstraction: files only'), bMid(L2, R2, 'Multi-protocol: same data'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('SmartConnect assigns DNS names per access zone; clients distributed across nodes.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Protection'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'N+M FEC: RAID across nodes'), bMid(L2, R2, 'OneFS CLI: isi commands'))))
    lines.append(R(merge(bMid(L1, R1, 'SyncIQ: async replication'), bMid(L2, R2, 'OneFS web UI: browser'))))
    lines.append(R(merge(bMid(L1, R1, 'SnapshotIQ: point-in-time'), bMid(L2, R2, 'REST API: v12+'))))
    lines.append(R(merge(bMid(L1, R1, 'SmartLock: WORM compliance'), bMid(L2, R2, 'InsightIQ: analytics'))))
    lines.append(R(merge(bMid(L1, R1, 'CloudPools: S3 cold tier'), bMid(L2, R2, 'SmartConnect: DNS LB'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('PowerScale nodes in rack; InfiniBand or 25GbE backend between nodes; dual-port'))
    lines.append(txt_row('front-end Ethernet to client network; dedicated management Ethernet port.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PowerScale     = Dell scale-out NAS; formerly Isilon; runs OneFS OS'))
    lines.append(txt_row('OneFS          = PowerScale operating system; presents single namespace across nodes'))
    lines.append(txt_row('FEC            = Forward Error Correction; like RAID 5/6 but across multiple nodes'))
    lines.append(txt_row('SyncIQ         = async policy-based replication between PowerScale clusters'))
    lines.append(txt_row('SnapshotIQ     = point-in-time snapshot; no performance impact; schedule-based'))
    lines.append(txt_row('SmartLock      = WORM (Write Once Read Many) compliance; object immutability'))
    lines.append(txt_row('CloudPools     = auto-tier cold data to S3 (AWS, Azure, or ECS)'))
    lines.append(txt_row('SmartConnect   = DNS-based client distribution across OneFS front-end nodes'))
    lines.append(txt_row('Access zone    = logical tenant partition; own auth, protocols, IP range'))
    lines.append(txt_row('H-series       = HDD-based capacity nodes; high density, moderate performance'))
    lines.append(txt_row('HDFS           = Hadoop Distributed Filesystem; PowerScale presents as HDFS'))
    lines.append(txt_row('isi commands   = OneFS CLI; isi status, isi volumes, isi sync policies'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-powerstore-arch-index',
    'docs/storage/dell/powerstore/architecture/index.md',
    'Dell PowerStore Architecture — mid-range AFA, AppsON, dynamic storage class, Kubernetes',
)
def dell_powerstore_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell PowerStore — Mid-Range AFA Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Mid-range all-flash array; AppsON runs containerized apps on array OS;'))
    lines.append(txt_row('Dynamic Storage Class auto-tiers NVMe→NVMe SCM→SAS; native Kubernetes integration.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Platform'), bMid(L2, R2, 'Data Services'))))
    lines.append(R(merge(bMid(L1, R1, 'T-model: AFA or hybrid'), bMid(L2, R2, 'Inline dedup + compression'))))
    lines.append(R(merge(bMid(L1, R1, 'X-model: extreme performance'), bMid(L2, R2, 'Snapshots: space-efficient'))))
    lines.append(R(merge(bMid(L1, R1, 'AppsON: Docker on array OS'), bMid(L2, R2, 'Metro volume: active-active'))))
    lines.append(R(merge(bMid(L1, R1, 'Dynamic Storage Class: auto'), bMid(L2, R2, 'Async replication: remote'))))
    lines.append(R(merge(bMid(L1, R1, 'Kubernetes CSI: native'), bMid(L2, R2, 'Thin provisioning: default'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('AppsON use cases: PowerStore analytics app, custom metrics, VAAI agent hosting.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Dynamic Storage Class'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'Tier 1: NVMe SCM (fastest)'), bMid(L2, R2, 'PowerStore Manager: web UI'))))
    lines.append(R(merge(bMid(L1, R1, 'Tier 2: NVMe SSD (standard)'), bMid(L2, R2, 'PowerStore CLI: psscli'))))
    lines.append(R(merge(bMid(L1, R1, 'Tier 3: SAS HDD (capacity)'), bMid(L2, R2, 'REST API: automation'))))
    lines.append(R(merge(bMid(L1, R1, 'Auto-move: IO heat map'), bMid(L2, R2, 'Ansible/PowerShell modules'))))
    lines.append(R(merge(bMid(L1, R1, 'Adaptive Optimization: policy'), bMid(L2, R2, 'CloudIQ: AIOps analytics'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('PowerStore appliance (rackmount 2U); dual node (active-active internally); FC or'))
    lines.append(txt_row('iSCSI/NVMe-oF for host connectivity; expansion drawer for capacity growth.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('PowerStore     = Dell mid-range AFA; block + file; replaces Unity and SC Series'))
    lines.append(txt_row('AppsON         = run Docker containers directly on the PowerStore OS'))
    lines.append(txt_row('Dynamic Storage Class= auto-tiering across NVMe SCM, NVMe SSD, and SAS drives'))
    lines.append(txt_row('NVMe SCM       = NVMe Storage Class Memory; highest performance tier (Optane)'))
    lines.append(txt_row('Metro volume   = synchronous active-active stretch between two PowerStore arrays'))
    lines.append(txt_row('Kubernetes CSI = Container Storage Interface; PowerStore as K8s persistent volume'))
    lines.append(txt_row('Adaptive Optimization= tiering policy engine; promotes/demotes by IO heat'))
    lines.append(txt_row('PowerStore Manager= web-based management console for PowerStore'))
    lines.append(txt_row('psscli         = PowerStore command-line interface'))
    lines.append(txt_row('REST API       = PowerStore northbound API; JSON; used by Ansible module'))
    lines.append(txt_row('T-model        = standard PowerStore; AFA or hybrid configuration'))
    lines.append(txt_row('X-model        = PowerStore X; extreme performance with NVMe SCM'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-unity-arch-index',
    'docs/storage/dell/unity/architecture/index.md',
    'Dell Unity Architecture — mid-range unified storage, file + block, Unisphere XT',
)
def dell_unity_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell Unity — Mid-Range Unified Storage Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Mid-range unified array: block (FC, iSCSI) and file (NFS, SMB) from same pool;'))
    lines.append(txt_row('managed via Unisphere XT; successor platform is PowerStore (migration path exists).'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Platform'), bMid(L2, R2, 'Protocols'))))
    lines.append(R(merge(bMid(L1, R1, 'Unity XT: all-flash model'), bMid(L2, R2, 'FC: block SAN (LUNs)'))))
    lines.append(R(merge(bMid(L1, R1, 'Unity hybrid: SSD+HDD tier'), bMid(L2, R2, 'iSCSI: block over Ethernet'))))
    lines.append(R(merge(bMid(L1, R1, 'Dual SP (service processor)'), bMid(L2, R2, 'NFS: Linux/Unix file access'))))
    lines.append(R(merge(bMid(L1, R1, 'Active-passive SP failover'), bMid(L2, R2, 'SMB/CIFS: Windows shares'))))
    lines.append(R(merge(bMid(L1, R1, 'FCoE: optional FC over Eth'), bMid(L2, R2, 'FCoE: block over 10GbE'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Block and file data can share the same storage pool; FAST VP auto-tiers both.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Data Services'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'Thin provisioning: default'), bMid(L2, R2, 'Unisphere XT: web UI'))))
    lines.append(R(merge(bMid(L1, R1, 'Snapshots: point-in-time'), bMid(L2, R2, 'UEMCLI: REST-based CLI'))))
    lines.append(R(merge(bMid(L1, R1, 'Async replication: remote DR'), bMid(L2, R2, 'REST API: JSON'))))
    lines.append(R(merge(bMid(L1, R1, 'Sync replication: metro'), bMid(L2, R2, 'vSphere plugin: VASA/vCenter'))))
    lines.append(R(merge(bMid(L1, R1, 'FAST VP: auto-tiering'), bMid(L2, R2, 'NAS server: file tenant unit'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('Unity chassis with two service processors (SP A, SP B); DAE disk shelves for'))
    lines.append(txt_row('expansion; FC or Ethernet switches for host connectivity.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('Unity          = Dell mid-range unified storage; block + file from shared pools'))
    lines.append(txt_row('Unity XT       = all-flash variant; AFA with NVMe-ready design'))
    lines.append(txt_row('SP             = Service Processor; the compute controller inside Unity chassis'))
    lines.append(txt_row('Unisphere XT   = Unity web management console; REST-based; no thick client'))
    lines.append(txt_row('UEMCLI         = Unity CLI; runs over REST API; scripting-friendly'))
    lines.append(txt_row('FAST VP        = Fully Automated Storage Tiering for Virtual Pools; auto-tier'))
    lines.append(txt_row('NAS server     = virtual file server; multiple per Unity; own IP and auth'))
    lines.append(txt_row('VASA           = VMware storage APIs; Unity registers as a VASA provider'))
    lines.append(txt_row('Sync replication= Metro synchronous: write confirmed on both arrays before ack'))
    lines.append(txt_row('DAE            = Disk Array Enclosure; expansion shelf for Unity chassis'))
    lines.append(txt_row('FCoE           = Fibre Channel over Ethernet; consolidates SAN+LAN on 10GbE'))
    lines.append(txt_row('PowerStore     = Unity successor; migration tool available (Import+Migrate)'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
...
@kb_diagram(
    'dell-vplex-arch-index',
    'docs/storage/dell/vplex/architecture/index.md',
    'Dell VPLEX Architecture — storage virtualization, Metro active-active, non-disruptive migration',
)
def dell_vplex_arch_index():
    W2 = 103
    R, txt_row = make_helpers(W2)
    L1, R1, L2, R2 = 3, 50, 53, 99
    lines = []
    lines.append(title_border(W2, 'Dell VPLEX — Storage Virtualization Architecture'))
    lines.append(txt_row())
    lines.append(txt_row('Storage virtualization appliance between hosts and arrays; presents virtual volumes'))
    lines.append(txt_row('abstracting underlying LUNs; Metro active-active stretch; non-disruptive migration.'))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Function'), bMid(L2, R2, 'Deployment Topologies'))))
    lines.append(R(merge(bMid(L1, R1, 'Storage virtualization layer'), bMid(L2, R2, 'Local: single site; migrate'))))
    lines.append(R(merge(bMid(L1, R1, 'Sits between hosts + arrays'), bMid(L2, R2, 'Metro: active-active 2-site'))))
    lines.append(R(merge(bMid(L1, R1, 'Presents virtual volumes'), bMid(L2, R2, 'Geo: async distant sites'))))
    lines.append(R(merge(bMid(L1, R1, 'Abstracts underlying LUNs'), bMid(L2, R2, 'Cluster: 2 directors per site'))))
    lines.append(R(merge(bMid(L1, R1, 'Cache coherency: distributed'), bMid(L2, R2, 'FC: only connectivity'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('VPLEX Metro enables VMware vMSC (Metro Storage Cluster) for cross-site vMotion.'))
    lines.append(txt_row())
    lines.append(R(arrow([26, 76])))
    lines.append(txt_row())
    lines.append(R(merge(bTop(L1, R1), bTop(L2, R2))))
    lines.append(R(merge(bMid(L1, R1, 'Key Use Cases'), bMid(L2, R2, 'Management'))))
    lines.append(R(merge(bMid(L1, R1, 'Non-disruptive migration'), bMid(L2, R2, 'VPLEX Management Console'))))
    lines.append(R(merge(bMid(L1, R1, 'Heterogeneous array support'), bMid(L2, R2, 'vscli: command line'))))
    lines.append(R(merge(bMid(L1, R1, 'Active-active Metro stretch'), bMid(L2, R2, 'REST API: automation'))))
    lines.append(R(merge(bMid(L1, R1, 'Host transparent migration'), bMid(L2, R2, 'VPLEX Witness: tiebreaker'))))
    lines.append(R(merge(bMid(L1, R1, 'vMSC: VMware stretched cluster'), bMid(L2, R2, 'vCenter plugin optional'))))
    lines.append(R(merge(bBot(L1, R1), bBot(L2, R2))))
    lines.append(txt_row())
    lines.append(txt_row('Physical Infrastructure (the hardware everything above runs on):'))
    lines.append(txt_row('VPLEX VS2 appliance (2U rackmount); FC fabric connections to both hosts and arrays;'))
    lines.append(txt_row('Inter-cluster link (ICL): FC between VPLEX clusters at both Metro sites.'))
    lines.append(txt_row())
    lines.append(txt_row('Key terms:'))
    lines.append(txt_row())
    lines.append(txt_row('VPLEX          = Dell storage virtualization appliance; brand name'))
    lines.append(txt_row('Virtual volume = logical volume presented to hosts; maps to one or more LUNs'))
    lines.append(txt_row('Virtual device = internal VPLEX construct mapping virtual vol to storage extents'))
    lines.append(txt_row('Metro          = synchronous active-active VPLEX across 2 sites; shared volume'))
    lines.append(txt_row('Local          = single-site VPLEX; non-disruptive migration use case'))
    lines.append(txt_row('Cache coherency= VPLEX distributed cache ensures consistency across both copies'))
    lines.append(txt_row('ICL            = Inter-Cluster Link; FC link between the two VPLEX clusters'))
    lines.append(txt_row('Director       = VPLEX compute module; two per cluster for HA'))
    lines.append(txt_row('vMSC           = vSphere Metro Storage Cluster; VMware validated with VPLEX Metro'))
    lines.append(txt_row('Witness        = VPLEX tiebreaker VM; determines surviving site on network partition'))
    lines.append(txt_row('Non-disruptive migration= move LUNs between arrays while host IO continues'))
    lines.append(txt_row('Heterogeneous  = VPLEX virtualizes LUNs from any vendor (PowerMax, NetApp, Pure)'))
    lines.append(txt_row())
    lines.append('└' + '─' * W2 + '┘')
    return lines
