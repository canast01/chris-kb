...
```mermaid
graph TB
  GV["SANnav Global View\n(multi-portal aggregation)"] --> PA["SANnav Portal A\n(Fabric A — DC1)"]
  GV --> PB["SANnav Portal B\n(Fabric B — DC2)"]
  PA -->|"HTTPS + SNMP v3"| SW["Managed Brocade FC Switches\n(Gen 6 / Gen 7 directors and edge)"]
  PB -->|"HTTPS + SNMP v3"| SW
  ADMIN(["SAN Admin"]) -->|"browser"| GV
