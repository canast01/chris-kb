...
Navigate to **Admin Console > Security > Local Users > admin > Change Password** for the ND application admin account. The OS `ndadmin` and the application `admin` are separate accounts.
...
# Import and activate via acs CLI:
acs certificates import --key /tmp/nd.key --cert /tmp/nd-bundle.crt --name nd-corp-cert
acs certificates activate --name nd-corp-cert
```
