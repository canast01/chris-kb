...
**vSAN-relevant objects:**

| Object | Relevant For |
|---|---|
| vCenter root | Global administration, KMS configuration |
| Datacenter | Cluster creation, datacenter-level policies |
| Cluster | vSAN configuration, health management, policy management |
| Host | Disk group management, maintenance mode |
| Datastore (vSAN) | VM provisioning on vSAN, storage policy assignment |
| VM Storage Policy | Creating and assigning storage policies |

Assign permissions at the **cluster level** for operators who manage vSAN. Use `Propagate to children` so permissions extend to hosts and datastores within the cluster.

### Built-in Roles

| Role | Relevant vSAN Access |
|---|---|
| Administrator | Full access to all vSAN operations |
| Read Only | View-only — can see health, capacity, object status |
| No Access | No access (used to explicitly block inheritance) |
| Virtual Machine User | No vSAN configuration access — can use vSAN-backed datastores |
| Datastore Consumer | Create VMs on datastores — includes vSAN datastores |

Built-in roles cannot be edited. Create custom roles for operator access models.

---

## Custom Roles for vSAN

### vSAN Operations Role (Storage Operator)

Minimum privileges for a storage operator who manages disk groups, monitors health, and handles day-to-day vSAN operations without access to VM configuration or security settings.

```powershell
Connect-VIServer <vcenter>

$vsanOperatorPrivileges = @(
    # Host and cluster management
    "Host.Inventory.EditCluster",
    "Host.Config.Storage",
    "Host.Config.MaintenanceMode",

    # vSAN specific
    "VsanHealth.ClusterHealth.Execute",
...
New-VIRole -Name "vSAN-StorageOperator" `
    -Privilege (Get-VIPrivilege -Id $vsanOperatorPrivileges)
```
