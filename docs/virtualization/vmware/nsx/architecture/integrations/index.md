...
From the NSX Manager UI: **System → Fabric → Compute Managers → Add**
...
```bash
# List all registered compute managers
curl -sk -u 'admin:password' \
  https://<nsx-manager>/api/v1/fabric/compute-managers | python3 -m json.tool

# Check connection status — should be REGISTERED
curl -sk -u 'admin:password' \
  https://<nsx-manager>/api/v1/fabric/compute-managers/<manager-id>/status
```
