...
```
┌───────────────────────────── VM Encryption — Key Hierarchy (KMS / vTPM) ──────────────────────────────┐
│                                                                                                       │
│  VM encryption uses a two-tier key model. Keys never leave the KMS in plaintext;                      │
│  vCenter only holds the encrypted DEK (wrapped by the KEK stored in the KMS).                         │
│                                                                                                       │
│   ┌──────────────────────────────────────────────┐  ┌─────────────────────────────────────────────┐   │
│   │         Key Management Server (KMS)          │  │              VM Storage Layout              │   │
│  ─────────────────────────────────────────────────────────────────────────────────────────            │
│   │ External KMIP-compliant server               │  │ .vmx file — VM config (encrypted)           │   │
│   │ Stores Key Encrypting Keys (KEKs)            │  │ .vmdk — virtual disk (encrypted)            │   │
│   │ vCenter connects via KMIP/TLS                │  │ .nvram — firmware vars (encrypted)          │   │
│   │ vCenter requests KEK per VM                  │  │ .vswp swap excluded from encryption         │   │
│   │ KEK never leaves KMS in plaintext            │  │ DEK generated by vSphere per VM             │   │
│   │ Revoke KEK = VM cannot power on              │  │ DEK wrapped by KEK → stored in vmx          │   │
│   │                                              │  │                                             │   │
│   └──────────────────────────────────────────────┘  └─────────────────────────────────────────────┘   │
│                                                                                                       │
│    Encrypt / decrypt flow:                                                                            │
│      1.  Admin enables VM encryption in storage policy                                                │
│      2.  vCenter fetches KEK from KMS (KMIP GET key)                                                  │
│      3.  vSphere generates a random DEK for this VM                                                   │
│      4.  DEK is encrypted with KEK → cipher-DEK stored in .vmx                                        │
│      5.  All I/O (read/write) encrypted/decrypted transparently using DEK in RAM                      │
│      6.  On power-off DEK is discarded; power-on requires fresh KEK from KMS                          │
│                                                                                                       │
│    KMS    = Key Management Server; external; holds KEKs; accessed via KMIP                            │
│    KMIP   = Key Management Interoperability Protocol (port 5696)                                      │
│    KEK    = Key Encrypting Key; stored in KMS; wraps the DEK                                          │
│    DEK    = Data Encrypting Key; generated by vSphere; used for actual I/O                            │
│    vTPM   = virtual Trusted Platform Module; requires vCenter KMS to unseal                           │
│                                                                                                       │
└───────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
