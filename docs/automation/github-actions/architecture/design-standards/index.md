...
```yaml
name: CI — Build and Test        # human-readable, includes context
# Not: ci, build, test

jobs:
  build-and-test:                 # kebab-case job IDs
    name: Build and Test (Node ${{ matrix.node-version }})  # readable job name

  deploy-to-staging:              # descriptive job ID
...
on:
  push:
    ...
    paths-ignore:             # skip CI for docs-only changes
      - "docs/**"
      - "*.md"
  pull_request:
    branches: [main]
  ...
  workflow_dispatch:          # manual trigger — always include
    inputs:
      environment:
