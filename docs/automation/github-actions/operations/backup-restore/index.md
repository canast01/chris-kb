# GitHub Actions — Backup & Restore

> Part of the [GitHub Actions Operations](../index.md) reference.

## What to Back Up

| Item | Location | Backup Method |
|---|---|---|
| Workflow files | `.github/workflows/` in Git | Git push to remote / mirror |
| Composite actions | `.github/actions/` in Git | Git push to remote / mirror |
| Reusable workflows | `.github/workflows/` in Git | Git push to remote / mirror |
| Repository secrets | GitHub API | Export names only (values never retrievable) |
| Environment secrets | GitHub API | Export names only |
| Environment protection rules | GitHub API | Export via API |
| Self-hosted runner registration | GitHub API | Re-register from runner host |
| Organization-level workflows | `.github/` org repo | Git push to remote |

!!! note "Secrets are not recoverable"
    GitHub never exposes secret values after creation — not via the UI, API, or export. Document where each secret value comes from (e.g., "generated by HashiCorp Vault") so you can regenerate rather than recover.

## Git Repository Backup (Primary)

Workflow files are code — they live in Git and are backed up with the rest of the repository.

```bash
# Mirror repository to backup location
git remote add backup git@backup-gitlab.example.com:org/repo.git
git push backup --mirror

# Automate daily mirror
cat > /usr/local/bin/github-repo-mirror.sh <<'EOF'
#!/bin/bash
set -euo pipefail
REPOS=(
  "git@github.com:org/infra.git"
  "git@github.com:org/app.git"
)
BACKUP_DIR="/backups/github"

for repo in "${REPOS[@]}"; do
  name=$(basename "$repo" .git)
  if [ -d "$BACKUP_DIR/$name.git" ]; then
    git -C "$BACKUP_DIR/$name.git" fetch --all
  else
    git clone --mirror "$repo" "$BACKUP_DIR/$name.git"
  fi
done
EOF
chmod +x /usr/local/bin/github-repo-mirror.sh
echo "0 2 * * * /usr/local/bin/github-repo-mirror.sh" | crontab -
```

## Export Repository Configuration via GitHub CLI

```bash
# Export repository settings
gh api repos/ORG/REPO > repo-config-$(date +%F).json

# Export all workflows (metadata — files are in Git)
gh api repos/ORG/REPO/actions/workflows | jq '.workflows[]' > workflows.json

# Export environments
gh api repos/ORG/REPO/environments | jq '.environments[]' > environments.json

# Export environment protection rules
for env in $(gh api repos/ORG/REPO/environments | jq -r '.environments[].name'); do
  gh api repos/ORG/REPO/environments/$env >> env-rules.json
done

# List secrets (names only — values not exposed)
gh secret list > secret-names.txt
gh secret list --env production >> secret-names.txt
```

## Backup GitHub Actions Self-Hosted Runners

Runner registration tokens are ephemeral — the runner binary and its configuration need to be preserved.

```bash
# Document runner configuration
gh api orgs/ORG/actions/runners | jq '.runners[] | {id, name, os, labels}' > runners.json
gh api repos/ORG/REPO/actions/runners | jq '.runners[]' >> runners.json

# On the runner host — back up runner config
tar czf /backups/github/runner-config-$(date +%F).tar.gz \
  /home/github-runner/actions-runner/.credentials \
  /home/github-runner/actions-runner/.env \
  /home/github-runner/actions-runner/.runner

# Save runner service config
systemctl cat github-runner > /backups/github/runner-service.txt
```

## Restore Procedures

### Restore Workflow Files from Git

```bash
# Files are in Git — restore is just a checkout
git clone git@github.com:org/repo.git
# or restore specific file
git checkout HEAD -- .github/workflows/deploy.yml

# From mirror backup
git clone /backups/github/repo.git /tmp/restore
cp -r /tmp/restore/.github/workflows/ /opt/repo/.github/
```

### Recreate Secrets After Repository Loss

```bash
# Recreate secrets from source systems
# SSH deploy keys — from control node
gh secret set DEPLOY_SSH_KEY < ~/.ssh/deploy_ed25519

# AWS credentials — from IAM (or use OIDC — no secret needed)
gh secret set AWS_ACCESS_KEY_ID --body "$AWS_ACCESS_KEY_ID"
gh secret set AWS_SECRET_ACCESS_KEY --body "$AWS_SECRET_ACCESS_KEY"

# Vault token — from HashiCorp Vault
gh secret set VAULT_TOKEN --body "$(vault token create -field=token -policy=github-actions)"

# Slack bot token — from Slack app settings
gh secret set SLACK_BOT_TOKEN --body "$SLACK_BOT_TOKEN"
```

### Recreate Environments and Protection Rules

```bash
# Create environment
gh api --method PUT repos/ORG/REPO/environments/production \
  --field wait_timer=0 \
  --field prevent_self_review=true

# Add required reviewers
gh api --method PUT repos/ORG/REPO/environments/production \
  -f 'reviewers[][type]=User' -f 'reviewers[][id]=123'

# Set environment secrets
gh secret set PROD_DB_PASSWORD --env production --body "$PROD_DB_PASSWORD"
```

### Re-register Self-Hosted Runner

```bash
# Get registration token
REG_TOKEN=$(gh api --method POST repos/ORG/REPO/actions/runners/registration-token | jq -r .token)

# Re-register runner
cd /home/github-runner/actions-runner
./config.sh \
  --url https://github.com/ORG/REPO \
  --token "$REG_TOKEN" \
  --name "prod-runner-01" \
  --labels "self-hosted,linux,prod" \
  --unattended

# Start service
sudo ./svc.sh install
sudo ./svc.sh start
```

## Organization-Level Backup

```bash
# Export org-level secrets (names only)
gh secret list --org ORG > org-secret-names.txt

# Export org-level workflow files (stored in .github repo)
git clone git@github.com:ORG/.github.git /backups/github/org-github-repo

# Export runner groups
gh api orgs/ORG/actions/runner-groups | jq '.runner_groups[]' > runner-groups.json
```

## Disaster Recovery Checklist

| Step | Action |
|---|---|
| 1 | Clone repository from backup mirror |
| 2 | Verify `.github/workflows/` files present |
| 3 | Recreate GitHub environments + protection rules |
| 4 | Recreate all secrets from source systems |
| 5 | Re-register self-hosted runners |
| 6 | Trigger `workflow_dispatch` on CI workflow to verify |
| 7 | Trigger `workflow_dispatch` on deploy workflow to staging |
| 8 | Verify notifications working |
