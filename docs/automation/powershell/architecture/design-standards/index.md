...
Common approved verbs by group:

| Group | Verbs (selection) |
|---|---|
| Common | `Add`, `Clear`, `Copy`, `Get`, `Move`, `New`, `Remove`, `Set`, `Show`, `Hide` |
| Data | `Backup`, `Export`, `Import`, `Restore`, `Save`, `Sync` |
| Lifecycle | `Disable`, `Enable`, `Install`, `Start`, `Stop`, `Uninstall`, `Update` |
| Diagnostic | `Debug`, `Measure`, `Resolve`, `Test`, `Trace` |
| Security | `Block`, `Grant`, `Lock`, `Protect`, `Revoke`, `Unlock`, `Unprotect` |

Never invent verbs like `Check-`, `Validate-`, `Process-`, or `Run-`. Use `Test-`, `Confirm-`, `Invoke-`, or `Start-` instead.
...
│   └── ConvertTo-WidgetPayload.ps1
├── Tests/                  # Pester tests mirror Public structure
│   ├── Get-Widget.Tests.ps1
│   └── Set-Widget.Tests.ps1
├── en-US/                  # MAML help (optional, generated by PlatyPS)
│   └── MyModule-help.xml
└── CHANGELOG.md
```
