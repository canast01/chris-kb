...
> **Execution policy:** `RemoteSigned` is the standard setting for managed servers. `Restricted` blocks all script execution and will cause automation failures. Investigate any scope showing an unexpected policy.

---

## Module Health

Modules provide the cmdlets used by automation scripts. Missing or outdated modules cause `CommandNotFoundException` errors and script failures.

**List all available modules**

```powershell
Get-Module -ListAvailable | Sort-Object Name | Select-Object Name, Version, ModuleType
```
