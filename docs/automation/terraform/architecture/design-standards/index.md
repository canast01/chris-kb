...
> Each environment directory has its own backend configuration and state file. This provides full isolation — a `terraform destroy` in dev cannot touch prod state.
...
|---|---|---|
| AWS VPC | `vpc-{env}-{region-short}` | `vpc-prod-euw1` |
| AWS Subnet | `sn-{visibility}-{env}-{az}` | `sn-priv-prod-euw1a` |
| AWS Security Group | `sg-{service}-{env}` | `sg-app-prod` |
| AWS IAM Role | `role-{service}-{purpose}` | `role-lambda-execution` |
| Azure Resource Group | `rg-{service}-{env}-{region-short}` | `rg-platform-prod-weu` |
| Azure Storage Account | `st{org}{service}{env}` (no hyphens, max 24 chars) | `stmyorgplatformprod` |
| VM / compute instance | `{service}-{env}-{index:02d}` | `app-prod-01` |
| Terraform module | `{organisation}/{name}/{provider}` | `my-org/vpc/aws` |

### Terraform identifier naming

```hcl
# Resource labels: snake_case, descriptive, no redundancy
resource "aws_vpc" "main" { }            # Good
resource "aws_vpc" "aws_vpc_main" { }    # Bad — provider name redundant
...
# Output names: snake_case, describe the value
output "vpc_id" { }
