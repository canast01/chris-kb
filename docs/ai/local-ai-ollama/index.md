...
Ollama runs open-weight large language models locally, exposing a REST API that is compatible with the OpenAI API format. It handles model download, quantisation, and GPU layer offloading automatically — supporting NVIDIA CUDA and Apple Silicon Metal backends. Key operational concerns are VRAM availability for larger models, choosing the right quantisation level (Q4_K_M is the standard trade-off), and network binding when exposing the API beyond localhost.
</div>
