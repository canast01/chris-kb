...
Veeam uses ports 2500–3300 (TCP) for data channel communication between VBR, proxies, and repositories. Any firewall blocking this range will cause timeout failures.
...
4. **Finalize to production** — if the VM stays in instant recovery state for too long, Veeam will not free the backup chain. Migrate the disk to a production datastore once the VM is confirmed working.
...
Common causes:
- Veeam configuration database (PostgreSQL or SQL Server) connection failure
- Insufficient disk space on the VBR server's OS drive
- Memory leak after extended uptime (schedule a service restart during off-peak)
- Corrupt job metadata (identified by error mentioning a specific job ID)
