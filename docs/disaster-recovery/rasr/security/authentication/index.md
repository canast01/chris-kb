...
```bash
# Verify LDAP/AD integration is configured on the array
# (Array CLI — uemcli for Unity, pstcli for PowerStore)
uemcli /sys/auth/ldap show

# View local management accounts
uemcli /user show

# Verify session timeout is configured (15–30 minutes)
...
- Assign LDAP groups to iDRAC roles (Administrator, Operator, ReadOnly)
- Enable two-factor authentication (smart card or RADIUS-based MFA)
