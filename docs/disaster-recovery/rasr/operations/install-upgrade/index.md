...
### Post-Installation — Start and Configure the Service
...
## RASR Boot Media Creation

RASR boot media is required for bare-metal recovery. Create media immediately after installation; do not wait until a recovery event.

### Create Bootable USB

1. Insert a USB drive (minimum 4 GB, all data will be erased).
2. Open **Dell RASR Console** → **Create Recovery Media** → select **USB Drive**.
3. Choose the target USB drive → click **Create**.

**Via CLI:**

```cmd
"C:\Program Files\Dell\RASR\rasrutil.exe" /createmedia /dest F:
```
