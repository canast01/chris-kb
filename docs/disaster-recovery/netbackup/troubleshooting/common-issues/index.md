...
Check for any firewall rules blocking TCP 13782 (bpcd), 13724 (bprd), and 1556 (pbx_exchange) between master and client.

### Status 59 — Access denied to client

```bash
# Verify client name matches what is in bp.conf
cat /usr/openv/netbackup/bp.conf | grep CLIENT_NAME

# On master: check server list accepted for this client
bpgetconfig -g <client-hostname> -L | grep -E "CLIENT|SERVER"

# Confirm hostname resolution is consistent
