...
For production use, replace Basic Auth with OAuth 2.0 tokens.

---

## Table API

The Table API provides CRUD operations on any ServiceNow table.

Base URL: `$INSTANCE/api/now/table/{tableName}`

### GET — Query Records

```bash
# List open P1 incidents (limit 10)
curl -s -u "$USER:$PASS" \
  "$INSTANCE/api/now/table/incident?sysparm_query=priority%3D1%5Estate%21%3D6&sysparm_limit=10&sysparm_fields=number,short_description,state,assigned_to" \
