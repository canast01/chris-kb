...
**Never disable SSL certificate verification** (`glide.http.ssl_check_cert = false`) in production. This creates a man-in-the-middle vulnerability for all outbound integrations.

### MID Server TLS Configuration

The MID Server establishes an encrypted tunnel to the ServiceNow instance.

```properties
# MID Server config.xml — TLS configuration
# Location: /opt/servicenow/mid/agent/config.xml

<parameter name="mid.instance.ssl.verify" value="true"/>

# Minimum TLS version for MID outbound connections
<parameter name="mid.ssl.minimum.protocol" value="TLSv1.2"/>

# Cipher suite restriction
<parameter name="mid.ssl.cipher.suites"
