...
- [ ] Take a fresh database backup: `pg_dump` (see Backup & Restore)
- [ ] Snapshot shared home filesystem
- [ ] Verify backup integrity before proceeding
- [ ] Confirm all cluster nodes are healthy (Data Center)
- [ ] Download the target Confluence installer from [Atlassian download page](https://www.atlassian.com/software/confluence/download-archives)
- [ ] Verify installer checksum

```bash
# Verify installer checksum (SHA-256 from Atlassian download page)
sha256sum atlassian-confluence-9.x.x-x64.bin
