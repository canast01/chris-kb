...
|---|---|---|
| Jira Issues macro | Jira → Confluence | Embed live JQL query results in pages |
| Confluence page links | Confluence → Jira | Pages linked from Jira issue fields |
| Jira project page | Jira → Confluence | Auto-created space per Jira project |
| Roadmap / Retrospective templates | Confluence → Jira | Template macros create Jira epics/issues |
| Release notes | Both | Confluence pages linked in Jira version notes |
...
Confluence pages can be linked to Jira issues programmatically:

```bash
# Create a remote issue link from Jira to a Confluence page
curl -u user:token \
  -X POST \
  -H "Content-Type: application/json" \
  https://jira.example.com/rest/api/2/issue/OPS-123/remotelink \
  -d '{
    "object": {
      "url": "https://confluence.example.com/display/OPS/Runbook",
      "title": "OPS Runbook",
