...
| Type | Scope | Creation Wizard | Best For |
|------|-------|----------------|----------|
| Page template | Global or space | No | Meeting notes, ADRs |
| Blueprint | Global | Yes | Project briefs, incident reports |
| Custom blueprint | Space | Yes | Team-specific structured content |

## Creating a Page Template

```bash
# Create a space-level template via API
curl -u user:token -X POST \
  ...
  -d '{
    ...
    "space": {"key": "OPS"},
    "description": {
      "value": "Standard runbook format for operational procedures",
      "representation": "plain"
    },
    "body": {
      "storage": {
        "value": "<h2>Overview</h2><p><ac:placeholder>Describe the service</ac:placeholder></p><h2>Prerequisites</h2><p><ac:placeholder>List required access and tools</ac:placeholder></p><h2>Procedure</h2><p><ac:placeholder>Step-by-step instructions</ac:placeholder></p><h2>Rollback</h2><p><ac:placeholder>How to undo this procedure</ac:placeholder></p>",
