...
| Log File | Location | Key Patterns |
|---|---|---|
| Main application log | `<CONF_HOME>/logs/atlassian-confluence.log` | ERROR, WARN, exceptions, upgrade tasks |
| Tomcat stdout / JVM | `<INSTALL>/logs/catalina.out` | JVM startup, OOM, thread dumps (if `kill -3`) |
| Security log | `<CONF_HOME>/logs/atlassian-confluence-security.log` | Auth failures, permission denied, admin actions |
| Index recovery log | `<CONF_HOME>/logs/atlassian-confluence-index-recovery.log` | Index corruption and rebuild events |
| GC log | Configured in `setenv.sh` | GC pause durations, heap occupancy |

### Enable GC Logging

Add to `setenv.sh` (JDK 11+):

```bash
JAVA_OPTS="$JAVA_OPTS \
  -Xlog:gc*:file=/var/atlassian/application-data/confluence/logs/gc.log:time,uptime:filecount=5,filesize=20m"
```
