...
1. Shut down Jira: `systemctl stop jira`
2. Navigate to: `Admin → System → Restore System`
3. Upload the `.zip` file or place it in `<jira-home>/import/`
4. Click **Restore**
5. After restore, reindex: `Admin → System → Indexing → Full Re-index`

---

## Database-Level Backup (PostgreSQL)

### Prerequisites

```bash
# Verify pg_dump version matches PostgreSQL server version
pg_dump --version
psql --version
```
