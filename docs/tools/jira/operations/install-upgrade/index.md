...
Cross-reference each plugin key against the target version on the Marketplace. Flag any plugins without a compatible version — either obtain an updated version or plan to disable them during upgrade.

---

## Fresh Installation (Data Center, Linux)

### System Preparation

```bash
# Create dedicated user
useradd -m -d /var/atlassian/application-data -s /bin/bash jira
...
# Set system limits
cat >> /etc/security/limits.d/jira.conf << 'EOF'
jira soft nofile 32768
jira hard nofile 32768
jira soft nproc  32768
