...
Jira and Confluence are connected via **Application Links** (OAuth 1.0a or OAuth 2.0). Configure in:
`Jira Admin → System → Application Links → Add application link`

Enter the Confluence base URL. Both instances must be able to reach each other on their configured base URLs.

### Issue Links in Confluence Pages
