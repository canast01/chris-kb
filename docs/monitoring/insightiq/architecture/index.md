...
| Retention Period | Estimated Disk Usage (5 nodes, 30-day sample rate) |
|---|---|
| 30 days | ~20 GB |
...
| Additional data disk | Per retention requirements above | Separate VMDK for data |
...
InsightIQ version compatibility with OneFS must be validated using the NetApp Interoperability Matrix Tool (IMT) before any OneFS upgrade. As of recent releases, InsightIQ supports OneFS 8.x and 9.x. For OneFS 9.5+, consider whether native OneFS performance views (via the OneFS web UI or CLI) may supplement InsightIQ for simpler use cases.
