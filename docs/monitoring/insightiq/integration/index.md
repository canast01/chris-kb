...
OneFS service account setup (run on OneFS CLI):

```bash
# Create read-only role for InsightIQ
isi auth roles create --name=IIQ_ReadOnly --description="InsightIQ read-only role"
isi auth roles modify IIQ_ReadOnly --add-priv-ro ISI_PRIV_LOGIN_PAPI
isi auth roles modify IIQ_ReadOnly --add-priv-ro ISI_PRIV_STATISTICS

# Create service account
isi auth users create svc-insightiq --password <password> --enabled yes

# Assign role
isi auth roles modify IIQ_ReadOnly --add-user svc-insightiq
```
