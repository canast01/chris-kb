...
Verify site connectivity: **Admin > Sites > [Site] > Status = Connected**

## NDFC (NX-OS Fabric Controller) Integration

NDFC manages NX-OS fabric switches directly. Switches are discovered and onboarded via NDFC fabric creation.

```text
NDFC > Fabric Builder > [Fabric] > Actions > Add Switches
- Switch IP range or individual management IPs
- Discovery credentials (read-write for provisioning; read-only for monitoring)
- SNMP community / v3 credentials
```
