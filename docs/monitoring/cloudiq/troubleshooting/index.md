...
### Stale Telemetry / Old Last-Seen Timestamp

**Symptoms**: CloudIQ shows metrics that are hours or days out of date.

```text
1. Check SCG service status:
   - SCG admin UI > System Status — verify all services show Running
   - If a service is stopped: restart from the admin UI

2. Verify SCG-to-array connectivity:
   - SCG admin UI > Systems > [System] > Test Connection
   - Resolve any network or credential errors reported

3. Restart SCG telemetry collection:
   - SCG admin UI > System Settings > Restart Collection Service
   - Allow 15–30 minutes for data to repopulate in CloudIQ

4. If SCG is unreachable (VM-level issue):
   - vCenter: power-cycle the SCG VM
   - Re-verify services after reboot
```
