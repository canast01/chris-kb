...
Always verify the exact compatibility matrix in the [NetApp Interoperability Matrix Tool (IMT)](https://mysupport.netapp.com/matrix) before upgrading. SnapCenter also has separate compatibility tables for each application plugin (Oracle, SQL Server, Exchange, SAP HANA, VMware).
