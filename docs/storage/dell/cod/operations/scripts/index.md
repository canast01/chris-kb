...
A table listing each storage pool with its name, percentage used, and a status of OK, WARNING, or CRITICAL. The final line shows the overall worst status. Use this in a cron job to get alerts before COD capacity is exhausted.
...
- An inventory entry for your Solutions Enabler server under the group `symcli_host`
- SYMCLI (`symcfg`, `symlicense`) installed on the target host

**Step 1 — Save the file**

1. Copy the code block above
2. Save it as `cod_capacity_audit.yml` in your Ansible working directory

**Step 2 — Fill in your details**

| Variable | What to put | How to find it |
|---|---|---|
| `sid` | Your PowerMax system ID | Run `symcfg list` on the Solutions Enabler server |
| `symcli_path` | Path to SYMCLI binaries | Default is `/usr/symcli/bin` |
| `warn_pct` | Pool usage % warning threshold | Default is `80` |
