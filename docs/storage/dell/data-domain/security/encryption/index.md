...
D@RE encrypts all data written to the DDFS container store using AES-256. Deduplication and compression operate on plaintext data in memory before encryption occurs, so dedup ratios are not impacted by enabling encryption. The DDOS D@RE implementation is FIPS 140-2 validated.

### Check Current Encryption Status

```bash
# Show encryption state, mode, and key manager type
encryption status
