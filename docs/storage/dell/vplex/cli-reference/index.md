...
```bash
# --- Navigate the virtual volumes context interactively ---
cd /virtual-volumes
ls
...
# Remove from storage view first (see Storage Views section), then:
...
| Status | Meaning |
|---|---|
| `in-progress` | Data copy running |
| `complete` | Copy done; not yet committed |
| `committed` | Source detached; migration finalised |
| `paused` | Manually paused or error |
...
# Show hardware inventory for all directors
