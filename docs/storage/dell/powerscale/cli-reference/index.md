...
Nodes are the physical servers that make up the cluster. Each node contributes storage, compute, and network resources. These commands let you inspect individual nodes, check their drives and sensors, and manage node additions or removals.
...
Storage pools group nodes of similar hardware together. Tiers let you organize pools into performance layers (e.g., fast SSD vs. large-capacity spinning disk). SmartPools (licensed feature) automatically moves files between tiers based on access patterns.
...
PowerScale's filesystem lives at `/ifs`. Everything — NFS exports, SMB shares, snapshots — is a subdirectory under `/ifs`. Quotas let you set hard limits on how much space a directory can use.
...
NFS (Network File System) is the standard protocol for sharing files to Linux/Unix clients. An NFS export makes a directory under `/ifs` available to mount over the network.
...
SMB (Server Message Block) is the protocol Windows uses to access file shares — it's what you use when you map a network drive. PowerScale supports SMB for Windows clients.
...
# Enable Access Based Enumeration (users only see files they have permission to)
...
PowerScale uses SmartConnect to load-balance client connections across nodes. IP pools group node interfaces together, and DNS-based SmartConnect zone names distribute clients automatically.
...
Access zones partition the cluster into separate namespaces — each with its own NFS exports, SMB shares, and authentication providers. This is how you serve different tenants or departments from one cluster.
...
Snapshots are instant point-in-time copies of a directory. They don't consume extra space until data changes, and they're accessible via the `.snapshot` folder inside any directory. Great for quick recovery of deleted files.
...
SyncIQ replicates directories from one PowerScale cluster to another. It creates a policy that defines what to sync, where to sync it, and on what schedule. Used for disaster recovery and data distribution.
...
OneFS uses a job engine to run background tasks automatically. FlexProtect is the most important — it re-protects your data after a node or drive fails. Understanding these jobs helps you monitor cluster health during and after hardware events.
...
These commands give you live visibility into how hard the cluster is working — throughput, latency, and IOPS across the whole cluster, per node, per protocol, or per client.
...
Events are the cluster's way of telling you something needs attention. They have severity levels — critical events need immediate action, warnings can wait. Alert channels let you receive notifications by email or SNMP.
...
Keep the cluster's OneFS OS and drive firmware up to date for security and performance. OneFS upgrades are rolling — each node upgrades one at a time so the cluster stays online.
