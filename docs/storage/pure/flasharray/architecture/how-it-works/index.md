...
Host write
    │
    ▼
NVRAM (CT active + CT peer mirror)
    │  ACK returned to host here
    ▼
[Background destage to flash]
    │
    ├── Step 1: Zero-block detection
    │     Blocks containing all zeroes are stored as metadata only (no physical write)
    │
    ├── Step 2: Global deduplication
    │     SHA-based fingerprint computed per chunk
    │     Fingerprint looked up in global dedup hash table
    │     If match found: reference pointer written, no duplicate physical write
    │
    ├── Step 3: Inline compression (LZ4 → zstd)
    │     Unique (post-dedup) data compressed with LZ4
    │     Further compressible segments passed to zstd
    │
    └── Step 4: Write to DirectFlash Modules
          Purity manages NAND block allocation, wear-levelling, and garbage collection directly
```
