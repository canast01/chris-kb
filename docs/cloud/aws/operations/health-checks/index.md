...
Verify compute instance health across system checks, instance checks, and Auto Scaling group status.

```bash
# All instances and their status checks
aws ec2 describe-instance-status \
  --include-all-instances \
  --query 'InstanceStatuses[*].{ID:InstanceId,AZ:AvailabilityZone,System:SystemStatus.Status,Instance:InstanceStatus.Status}' \
  --output table

# System check failures (AWS hardware / network issues)
aws ec2 describe-instance-status \
  --filters Name=system-status.status,Values=impaired \
  --query 'InstanceStatuses[*].[InstanceId,SystemStatus.Status,SystemStatus.Details[0].Name,SystemStatus.Details[0].Status]' \
  --output table

# Instances not in running state
aws ec2 describe-instances \
  --filters Name=instance-state-name,Values=stopped,stopping,shutting-down,terminated \
  --query 'Reservations[*].Instances[*].{ID:InstanceId,State:State.Name,Name:Tags[?Key==`Name`]|[0].Value}' \
  --output table

# Auto Scaling group instance health
aws autoscaling describe-auto-scaling-groups \
  --query 'AutoScalingGroups[*].{Name:AutoScalingGroupName,Desired:DesiredCapacity,InService:Instances[?HealthStatus==`Healthy`]|length(@),Total:Instances|length(@)}' \
  --output table

# Recent EC2 Auto Recovery actions
